...
启动脚本默认监听 `0.0.0.0:8879`，可直接通过 `http://服务器地址:8879` 检查，也可由 Nginx/Caddy 反向代理到 HTTPS 域名；仍可通过 `WEB_HOST=127.0.0.1` 限制为仅本机访问。网页提供注册、登录、邮箱验证重置密码和退出功能，会话保存在 HttpOnly Cookie 中。
