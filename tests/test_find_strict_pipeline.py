...
def test_parallel_title_filter_result_processing_progress_uses_processed_count(monkeypatch):
    ...
    assert not any("processed title result" in call[3] for call in progress_calls)
    processing = [call for call in progress_calls if "processed 3 title batches" in call[3]]
    assert processing
    assert processing[-1][1:3] == (3, 3)
    assert all(current <= total for phase, current, total, _message in progress_calls if phase == "llm_title_filter")


def test_title_filter_backfills_omitted_rows_even_when_scored_pool_exceeds_target(monkeypatch):
    monkeypatch.setenv("TITLE_DETAIL_CANDIDATE_TARGET", "5")
    monkeypatch.setenv("MIN_TITLE_CANDIDATES", "5")
    monkeypatch.delenv("TITLE_FILTER_SEQUENTIAL", raising=False)

    def fake_json_or_error(_llm, prompt: str, timeout_sec=None, temperature=None):
        ids = [item_id for item_id in re.findall(r"paper_\d+", prompt) if item_id != "paper_999"]
        return {
            "ok": True,
            "error": "",
            "data": {
    ...
    items = [
        {
            "id": "paper_999",
            "title": "Adaptive preference learning for LLM recommender systems",
            "abstract": "This work studies LLM recommender systems, preference learning, and cold-start recommendation.",
        }
    ] + [
        {"id": f"paper_{index}", "title": f"Generic unrelated model {index}"}
        for index in range(7)
    ]
    logs = []
    cfg = AppConfig(
        provider="mock",
        research_interest="LLM recommender systems preference learning cold-start recommendation",
        max_fetch_papers=5,
        llm_concurrency=4,
    )
    ...
    kept = {item["id"]: item for item in selected}
    assert "paper_999" in kept
    assert kept["paper_999"].get("title_llm_missing") is True
    assert any("title LLM omitted 1 title rows" in message for message in logs)


def test_detail_titles_adds_presentation_recall_without_replacing_base(monkeypatch):
    monkeypatch.setenv("PRESENTATION_RECALL_DETAIL_PER_TYPE", "2")
    monkeypatch.setenv("PRESENTATION_RECALL_TITLE_SCORE_FLOOR", "5.0")
    rows = [
        {"id": f"poster_{index}", "title": f"Poster {index}", "fit_score": 8.0, "title_llm_fit_score": 8.0, "track": "Poster"}
        for index in range(4)
    ] + [
        {"id": "oral_1", "title": "Relevant oral", "fit_score": 5.5, "title_llm_fit_score": 5.5, "track": "Oral"},
        {"id": "oral_low", "title": "Weak oral", "fit_score": 4.0, "title_llm_fit_score": 4.0, "track": "Oral"},
        {"id": "spotlight_1", "title": "Relevant spotlight", "fit_score": 6.0, "title_llm_fit_score": 6.0, "track": "Spotlight"},
        {"id": "spotlight_2", "title": "Second spotlight", "fit_score": 5.0, "title_llm_fit_score": 5.0, "track": "Spotlight"},
    ]
    logs = []

    selected = find_pipeline._detail_titles_with_presentation_recall(rows, 3, "TestVenue", logs.append)

    assert [item["id"] for item in selected[:3]] == ["poster_0", "poster_1", "poster_2"]
    assert {item["id"] for item in selected[3:]} == {"oral_1", "spotlight_1", "spotlight_2"}
    assert "oral_low" not in {item["id"] for item in selected}
    assert any("presentation recall added 3" in message for message in logs)


def test_detail_recall_allows_weak_local_oral_evidence_without_final_promotion(monkeypatch):
    monkeypatch.setenv("PRESENTATION_RECALL_DETAIL_PER_TYPE", "1")
    monkeypatch.setenv("PRESENTATION_RECALL_TITLE_SCORE_FLOOR", "5.0")
    rows = [
        {"id": "poster_0", "title": "Strong poster", "fit_score": 8.0, "title_llm_fit_score": 8.0, "track": "Poster"},
        {"id": "poster_1", "title": "Second poster", "fit_score": 7.5, "title_llm_fit_score": 7.5, "track": "Poster"},
        {
            "id": "local_oral",
            "title": "Adaptive preference learning with limited feedback",
            "fit_score": 5.25,
            "retrieval_fit_score": 5.25,
            "topic_evidence": "weak: adaptive topic evidence",
            "reason_source": "local title ranking after incomplete title LLM",
            "track": "ICLR 2026 Oral",
            "presentation_type": "oral",
        },
    ]
    llm_weak = dict(rows[-1], id="llm_weak", topic_evidence_source="llm_adaptive")
    logs = []

    selected = find_pipeline._detail_titles_with_presentation_recall(rows, 2, "ICLR", logs.append)

    assert "local_oral" in {item["id"] for item in selected}
    assert find_pipeline._screening_quality_bonus(rows[-1]) > 0
    assert find_pipeline._screening_quality_bonus(llm_weak) == 0
    assert any("presentation recall added" in message and "oral" in message for message in logs)


def test_title_prefilter_prompt_includes_available_abstract_snippets():
    items = [
        {
            "id": "paper_001",
            "title": "Large-scale graph inference with language models",
            "abstract": "This abstract mentions user behavior graphs and recommendation systems as a concrete evaluation setting.",
        }
    ]
    llm = BatchLLM()
    cfg = AppConfig(provider="mock", research_interest="LLM recommender systems user behavior graphs", max_fetch_papers=5)

    _prefilter_titles(items, cfg, llm, "ICLR", log=lambda _msg: None, should_cancel=lambda: False, scan_all=True)

    assert "Paper titles and available abstract snippets" in llm.prompts[0]
    assert "abstract: This abstract mentions user behavior graphs and recommendation systems" in llm.prompts[0]


def test_title_filter_full_backfills_large_llm_omission_window(monkeypatch):
    monkeypatch.setenv("TITLE_DETAIL_CANDIDATE_TARGET", "5")
    monkeypatch.setenv("MIN_TITLE_CANDIDATES", "5")
    monkeypatch.setenv("TITLE_FILTER_SEQUENTIAL", "1")

    def sparse_json_or_error(_llm, prompt: str, timeout_sec=None, temperature=None):
        ids = []
        for item_id in re.findall(r"paper_\d+|paper_999", prompt):
            if item_id == "paper_999":
                continue
    ...
    monkeypatch.setattr(find_pipeline, "_json_or_error_wall_timeout", sparse_json_or_error)
    items = [
        {"id": f"paper_{index}", "title": f"Generic unrelated model {index}", "abstract": ""}
        for index in range(40)
    ]
    items.append({
        "id": "paper_999",
        "title": "LLM recommender systems with user feedback and cold-start recommendation",
        "abstract": "This work studies LLM recommender systems, user feedback, and cold-start recommendation.",
    })
    cfg = AppConfig(provider="mock", research_interest="LLM recommender systems with user feedback", max_fetch_papers=5)
    rows = _prefilter_titles(items, cfg, BatchLLM(), "ICLR", log=lambda _msg: None, should_cancel=lambda: False, scan_all=True, result_limit=5)

    selected_ids = {row["id"] for row in rows}
    assert "paper_999" in selected_ids
    assert next(row for row in rows if row["id"] == "paper_999")["title_llm_missing"] is True


def test_detail_presentation_recall_uses_best_abstract_aware_oral(monkeypatch):
    monkeypatch.setenv("PRESENTATION_RECALL_DETAIL_PER_TYPE", "1")
    monkeypatch.setenv("PRESENTATION_RECALL_TITLE_SCORE_FLOOR", "5.0")
    rows = [
        {"id": "poster_0", "title": "Strong poster", "fit_score": 8.0, "title_llm_fit_score": 8.0, "track": "Poster"},
        {"id": "base_oral", "title": "Base oral", "fit_score": 5.0, "title_llm_fit_score": 5.0, "track": "Oral"},
        {"id": "poster_1", "title": "Second poster", "fit_score": 7.5, "title_llm_fit_score": 7.5, "track": "Poster"},
        {
            "id": "abstract_oral",
            "title": "Generic graph inference title",
            "fit_score": 2.0,
            "title_llm_fit_score": 2.0,
            "abstract_fit_score": 7.2,
            "retrieval_fit_score": 7.2,
            "local_profile_phrase_match_count": 2,
            "track": "ICLR 2026 Oral",
        },
    ]
    logs = []
    ...
    assert [item["id"] for item in selected[:2]] == ["poster_0", "base_oral"]
    assert "abstract_oral" in {item["id"] for item in selected}
    assert find_pipeline._screening_quality_bonus(rows[-1]) > 0
    assert any("presentation recall added" in message and "oral" in message for message in logs)


def test_quality_local_title_recall_reaches_detail_fetch(monkeypatch):
    monkeypatch.setenv("QUALITY_LOCAL_TITLE_RECALL_PER_TYPE", "1")
    monkeypatch.setenv("PRESENTATION_RECALL_DETAIL_PER_TYPE", "1")
    selected = [
        {"id": f"poster_{index}", "title": f"Selected baseline {index}", "fit_score": 8.0, "diversity_score": 7.0, "track": "Poster"}
        for index in range(3)
    ]
    oral = {
        "id": "local_oral",
        "title": "Adaptive preference learning from user feedback",
        "abstract": "This paper studies adaptive preference learning from user feedback signals.",
        "local_score": 0.12,
        "local_rank": 42,
        "local_profile_phrase_match_count": 1,
        "track": "ICLR 2026 Oral",
        "venue": "ICLR",
        "year": 2026,
    }
    ...
    supplemented = find_pipeline._quality_local_title_recall_supplement(selected + [oral], selected, "ICLR", logs.append)
    detail = find_pipeline._detail_titles_with_presentation_recall(supplemented, 2, "ICLR", logs.append)

    assert any(item["id"] == "local_oral" and item.get("quality_local_title_recall") for item in supplemented)
    assert "local_oral" in {item["id"] for item in detail}
    assert any("high-quality local recall added 1 title candidates" in message for message in logs)
    assert any("presentation recall added" in message and "local_quality" in message for message in logs)


def test_tfidf_screening_applies_quality_bonus_before_global_cutoff():
    papers = [
        {"id": "poster", "title": "Recommendation preference modeling", "category": "rec", "track": "Poster"},
        {"id": "oral", "title": "Recommendation preference modeling", "category": "rec", "track": "Oral"},
    ]

    ranked, report = rank_papers_tfidf(
        papers,
        "recommendation preference modeling",
        global_limit=1,
        per_category_limit=10,
        ranking_bonus=lambda paper: 0.05 if paper.get("id") == "oral" else 0.0,
    )

    assert report["ranking_bonus_applied"] is True
    assert [item["id"] for item in ranked] == ["oral"]
    assert ranked[0]["local_quality_bonus"] == 0.05
    assert ranked[0]["local_rank_score"] > ranked[0]["local_score"]



def test_tfidf_quality_bonus_can_use_computed_local_relevance_fields():
    papers = [
        {"id": "poster", "title": "generic baseline", "category": "topic", "track": "Poster"},
        {"id": "oral", "title": "Adaptive preference learning from user feedback", "category": "topic", "track": "Oral"},
    ]

    ranked, _report = rank_papers_tfidf(
        papers,
        "adaptive preference learning with user feedback",
        global_limit=1,
        per_category_limit=10,
        ranking_bonus=lambda row: 0.05
        if row.get("track") == "Oral" and row.get("local_profile_phrase_match_count", 0) > 0
        else 0.0,
    )

    assert [item["id"] for item in ranked] == ["oral"]
    assert ranked[0]["local_profile_phrase_match_count"] > 0
    assert ranked[0]["local_quality_bonus"] == 0.05
    assert ranked[0]["local_rank_score"] > ranked[0]["local_score"]


def test_title_ranking_uses_screening_quality_bonus_without_rewriting_fit_score(monkeypatch):
    monkeypatch.setenv("SCREENING_QUALITY_RELEVANCE_FLOOR", "5.0")
    oral = {
        "id": "oral",
        "title": "Oral recommendation candidate",
        "venue": "ICLR",
        "year": 2026,
        "track": "ICLR 2026 Oral",
        "presentation_type": "oral",
        "fit_score": 5.5,
        "title_llm_fit_score": 5.5,
        "diversity_score": 5.0,
        "topic_evidence": "passed:title route",
        "topic_evidence_supported": True,
    }
    poster = {
        "id": "poster",
        "title": "Poster recommendation candidate",
        "venue": "ICLR",
        "year": 2026,
        "track": "ICLR 2026 Poster",
        "presentation_type": "poster",
        "fit_score": 5.8,
        "title_llm_fit_score": 5.8,
        "diversity_score": 5.0,
        "topic_evidence": "passed:title route",
        "topic_evidence_supported": True,
    }
    unrelated_oral = dict(oral, id="unrelated_oral", category="unrelated", reason="irrelevant to the profile")

    assert find_pipeline._screening_quality_bonus(oral) >= 0.45
    assert find_pipeline._title_rank_key(oral) < find_pipeline._title_rank_key(poster)
    assert oral["fit_score"] == 5.5
    assert find_pipeline._screening_quality_bonus(unrelated_oral) == 0.0
...
class ZhOnlyReasonLLM(BatchLLM):
    def json_or_error(self, prompt: str, temperature=None):
        self.prompts.append(prompt)
        self.temperatures.append(temperature)
        ids = re.findall(r"paper_\d+", prompt)
                ...
                "evaluations": [
                    {
                        ...
                        "fit_score": 8.2,
                        "diversity_score": 7.4,
                        "hit_directions_zh": ["生成式AI", "材料物理"],
                        "hit_directions_en": ["generative AI", "materials physics"],
                        "topic_evidence": "passed:adaptive_llm_topic_route",
                        "topic_evidence_supported": True,
                        "matched_topic_route": "adaptive route from current research profile",
                        "topic_evidence_basis": "abstract",
                        "missing_topic_evidence": [],
                        "fit_explanation_zh": "摘要明确说明生成模型用于材料发现，并包含可复用的评测线索。",
                        "fit_explanation_en": "The abstract explicitly connects generative models to materials discovery.",
                        "reason_zh": "该论文直接服务当前研究画像中的生成式AI科学发现方向。摘要给出了可复用的方法和评测价值，因此适合作为后续调研候选。",
                        "reason_en": "This paper directly serves the current research profile and offers reusable method and evaluation value.",
                    }
                    for item_id in ids
                ]
            },
...
def test_final_scoring_syncs_primary_reason_from_reason_zh_when_batch_schema_omits_reason():
    llm = ZhOnlyReasonLLM()
    ...
    items = [
        {
            ...
            "abstract": "This paper studies generative models for materials discovery with benchmark evaluation.",
            "classification_source": "llm_inferred",
            "reason": "Local title/abstract ranking before final relevance scoring. Configure an LLM API key for model-based relevance scoring.",
        }
    ]

    evaluated = _evaluate_items(items, cfg, llm, "articles", log=lambda _msg: None)

    assert evaluated[0]["reason"] == evaluated[0]["reason_zh"]
    assert "Configure an LLM API key" not in evaluated[0]["reason"]


def test_venue_title_index_cache_can_bootstrap_from_adopted_find_results(monkeypatch, tmp_path):
    monkeypatch.setenv("WORKSPACE_ROOT", str(tmp_path))
    monkeypatch.setenv("PROJECT_ID", "demo_project")
    finding_dir = tmp_path / "projects" / "demo_project" / "planning" / "finding"
    finding_dir.mkdir(parents=True)
    rows = [
        {"id": "paper_1", "title": "Adopted official paper", "venue": "ICLR", "year": 2026, "metadata": {"venue_id": "iclr"}},
        {"id": "paper_2", "title": "Other venue paper", "venue": "ICML", "year": 2026, "metadata": {"venue_id": "icml"}},
    ]
    (finding_dir / "find_results.json").write_text(json.dumps({"raw_title_index": rows}), encoding="utf-8")
    venue = {"id": "iclr", "name": "ICLR"}
    monkeypatch.setattr(find_pipeline, "fetch_venue_title_index_all", lambda _venue, _years: ([], "none"))

    cached, adapter = find_pipeline._fetch_venue_title_index_for_find(venue, [2026], None)

    assert [row["id"] for row in cached] == ["paper_1"]
    assert adapter == "project_find_results_cache"
    stored, stored_adapter = find_pipeline._load_venue_title_index_cache(venue, [2026], None)
    assert [row["id"] for row in stored] == ["paper_1"]
    assert stored_adapter == "project_find_results_cache"


def test_venue_title_index_cache_recovers_transient_empty_fetch(monkeypatch, tmp_path):
    monkeypatch.setenv("WORKSPACE_ROOT", str(tmp_path))
    monkeypatch.setenv("PROJECT_ID", "demo_project")
    venue = {"id": "iclr", "name": "ICLR"}
    rows = [
        {"id": "paper_1", "title": "Cached official paper", "venue": "ICLR", "year": 2026, "classification_source": "official"}
    ]
    find_pipeline._store_venue_title_index_cache(venue, [2026], rows, "openreview")
    monkeypatch.setattr(find_pipeline, "fetch_venue_title_index_all", lambda _venue, _years: ([], "none"))

    cached, adapter = find_pipeline._fetch_venue_title_index_for_find(venue, [2026], None)

    assert cached == rows
    assert adapter == "openreview_cache"


def test_venue_title_index_cache_stores_successful_online_fetch(monkeypatch, tmp_path):
    monkeypatch.setenv("WORKSPACE_ROOT", str(tmp_path))
    monkeypatch.setenv("PROJECT_ID", "demo_project")
    venue = {"id": "iclr", "name": "ICLR"}
    rows = [
        {"id": "paper_1", "title": "Online official paper", "venue": "ICLR", "year": 2026, "classification_source": "official"}
    ]
    monkeypatch.setattr(find_pipeline, "fetch_venue_title_index_all", lambda _venue, _years: (rows, "openreview"))

    fetched, adapter = find_pipeline._fetch_venue_title_index_for_find(venue, [2026], None)
    cached, cached_adapter = find_pipeline._load_venue_title_index_cache(venue, [2026], None)

    assert fetched == rows
    assert adapter == "openreview"
    assert cached == rows
    assert cached_adapter == "openreview_cache"


def test_venue_title_index_timeout_uses_verified_cache(monkeypatch, tmp_path):
    monkeypatch.setenv("WORKSPACE_ROOT", str(tmp_path))
    monkeypatch.setenv("PROJECT_ID", "demo_project")
    monkeypatch.setenv("FIND_VENUE_TITLE_FETCH_TIMEOUT_SEC", "0.01")
    venue = {"id": "icml", "name": "ICML"}
    rows = [
        {"id": "paper_1", "title": "Cached official paper", "venue": "ICML", "year": 2026, "classification_source": "official"}
    ]
    find_pipeline._store_venue_title_index_cache(venue, [2026], rows, "icml_downloads")

    def slow_fetch(_venue, _years):
        time.sleep(0.2)
        return [], "icml_downloads"

    monkeypatch.setattr(find_pipeline, "fetch_venue_title_index_all", slow_fetch)

    fetched, adapter = find_pipeline._fetch_venue_title_index_for_find(venue, [2026], None)

    assert fetched == rows
    assert adapter == "icml_downloads_cache"


def test_venue_title_index_timeout_returns_timeout_without_cache(monkeypatch, tmp_path):
    monkeypatch.setenv("WORKSPACE_ROOT", str(tmp_path))
    monkeypatch.setenv("PROJECT_ID", "demo_project")
    monkeypatch.setenv("FIND_VENUE_TITLE_FETCH_TIMEOUT_SEC", "0.01")
    venue = {"id": "unknown", "name": "SlowConf"}
    ...
    assert fetched == []
    assert adapter == "timeout"


def test_venue_year_resolution_uses_short_probe_timeout(monkeypatch):
    monkeypatch.setenv("FIND_VENUE_YEAR_PROBE_TIMEOUT_SEC", "3.5")
    venue = {"id": "sigir", "name": "SIGIR"}
    calls = []
    monkeypatch.setattr(find_pipeline, "load_local_venue_year", lambda _venue, _year: None)

    def fake_fetch(_venue, years, limit, **kwargs):
        calls.append({
            "years": years,
            "limit": limit,
            "timeout_sec": kwargs.get("timeout_sec"),
            "prefer_cache": kwargs.get("prefer_cache"),
        })
        if years == [2025]:
    ...
    years, reason = find_pipeline._resolve_venue_years(venue, [2026])

    assert years == [2025]
    assert calls
    assert all(call["limit"] == 1 for call in calls)
    assert all(call["timeout_sec"] == 3.5 for call in calls)
    assert all(call["prefer_cache"] is True for call in calls)
    assert "latest available SIGIR title index year 2025 via dblp" in reason


def test_venue_title_index_prefer_cache_skips_slow_online_probe(monkeypatch, tmp_path):
    monkeypatch.setenv("WORKSPACE_ROOT", str(tmp_path))
    monkeypatch.setenv("PROJECT_ID", "demo_project")
    monkeypatch.setenv("FIND_VENUE_TITLE_FETCH_TIMEOUT_SEC", "0.2")
    venue = {"id": "icml", "name": "ICML"}
    rows = [{"id": "paper_1", "title": "Cached paper", "venue": "ICML", "year": 2026}]
    find_pipeline._store_venue_title_index_cache(venue, [2026], rows, "icml_downloads")

    def slow_fetch(_venue, _years):
        raise AssertionError("online fetch should not run when prefer_cache has a verified cache")

    monkeypatch.setattr(find_pipeline, "fetch_venue_title_index_all", slow_fetch)

    fetched, adapter = find_pipeline._fetch_venue_title_index_for_find(venue, [2026], None, prefer_cache=True)

    assert fetched == rows
    assert adapter == "icml_downloads_cache"


def test_stage0_profile_cache_reuses_successful_project_profile(monkeypatch, tmp_path):
    monkeypatch.setenv("WORKSPACE_ROOT", str(tmp_path))
    monkeypatch.setenv("PROJECT_ID", "demo_project")
    cfg = AppConfig(provider="mock", model="mock-find", research_interest="生成式AI 科学发现 材料物理")
    profile = {
        "explicit_profile": {"research_interest_summary": "生成式AI 科学发现 材料物理"},
        ...
        "filtering_hints": {"hard_exclusions": [], "conditional_exclusions": [], "soft_penalties": [], "must_keep_if_present": [], "preference_hints": []},
        "uncertainty": {"ambiguous_terms": [], "needs_clarification": False},
    }

    find_pipeline._store_stage0_profile_cache(cfg, profile)
    loaded = find_pipeline._load_stage0_profile_cache(cfg)

    assert loaded == profile
    assert (tmp_path / "projects" / "demo_project" / "planning" / "finding" / "cache" / "stage0_profiles.json").exists()


def test_cached_final_scoring_syncs_primary_reason_from_reason_zh():
    item = {
        "id": "paper_001",
        "title": "Generative materials discovery",
        "abstract": "This paper studies generative models for materials discovery with benchmark evaluation.",
    ...
    entry = {
        "schema": find_pipeline.FINAL_LLM_SCORE_CACHE_SCHEMA_VERSION,
        "scoring_policy": find_pipeline.SCORING_POLICY_VERSION,
        "prompt_policy": find_pipeline.FINAL_LLM_SCORE_CACHE_PROMPT_POLICY,
        "fit_score": 8.2,
        "diversity_score": 7.4,
        "reason": "Local title/abstract ranking before final relevance scoring. Configure an LLM API key for model-based relevance scoring.",
        "reason_zh": "该论文直接服务当前研究画像中的生成式AI科学发现方向。摘要给出了可复用的方法和评测价值，因此适合作为后续调研候选。",
    ...
    assert find_pipeline._apply_cached_final_llm_score(item, entry, "生成式AI 科学发现 材料物理") is True

    assert item["reason"] == item["reason_zh"]
    assert "Configure an LLM API key" not in item["reason"]
...
def test_screened_ranking_uses_same_find_recommendation_gate_and_sorts_by_score():
    items = [
        {"id": "low", "fit_score": 6, "recommendation_score": 6.0, "score": 6.0, "reason_source": "llm abstract evaluation", "topic_evidence": "passed:adaptive_llm_topic_route", "abstract": "real abstract"},
        {"id": "middle", "fit_score": 7, "recommendation_score": 7.2, "score": 7.2, "reason_source": "llm abstract evaluation", "topic_evidence": "passed:adaptive_llm_topic_route", "abstract": "real abstract"},
        {"id": "top", "fit_score": 8, "recommendation_score": 8.4, "score": 8.4, "reason_source": "llm abstract evaluation", "topic_evidence": "passed:adaptive_llm_topic_route", "abstract": "real abstract"},
...
def test_final_ranking_breaks_exact_ties_by_semantic_retrieval_fit():
    base = {
        "fit_score": 8.0,
        "diversity_score": 7.0,
        "combined_score": 7.75,
        "recommendation_score": 7.83,
        "score": 7.83,
        "quality_bonus": 0.08,
        "reason_source": "llm abstract evaluation",
        "topic_evidence": "passed:adaptive_llm_topic_route",
        "topic_evidence_supported": True,
        "abstract": "real abstract with enough topic evidence",
    }
    items = [
        {
            **base,
            "id": "alphabetic_first",
            "title": "A alphabetic but weaker semantic tie",
            "title_fit_score": 4.0,
            "retrieval_fit_score": 4.0,
            "abstract_fit_score": 4.0,
            "title_llm_fit_score": 9.0,
        },
        {
            **base,
            "id": "semantic_first",
            "title": "Z stronger semantic tie",
            "title_fit_score": 5.5,
            "retrieval_fit_score": 6.5,
            "abstract_fit_score": 6.5,
            "title_llm_fit_score": 8.5,
        },
        {
            **base,
            "id": "lower_final_score",
            "title": "Best local signal but lower final score",
            "recommendation_score": 7.82,
            "score": 7.82,
            "title_fit_score": 9.0,
            "retrieval_fit_score": 9.0,
            "abstract_fit_score": 9.0,
            "title_llm_fit_score": 9.0,
        },
    ]
    ...
    assert [item["id"] for item in ranked] == ["semantic_first", "alphabetic_first", "lower_final_score"]
    assert [item["id"] for item in recommended] == ["semantic_first", "alphabetic_first", "lower_final_score"]
...
def test_final_scoring_prompt_requires_topic_evidence_schema_and_routes():
    ...
    assert "core routes only" in final_prompt
    assert "modifiers, not standalone topic routes" in final_prompt
    ...
    assert "Shared surface terms are not enough" in final_prompt
    assert "concrete target problem" in final_prompt
    assert "profile-specific evidence" in final_prompt
    assert "possible application, benchmark/dataset domain" in final_prompt
    assert "concretely reusable for the research profile" in final_prompt
    assert "missing profile evidence" in final_prompt
    assert "one decimal place" in final_prompt
    assert "avoid default integer or x.5 scores" in final_prompt
    assert '"fit_score":7.3' in final_prompt


def test_stage0_term_fragments_and_preference_hints_are_not_standalone_topic_routes():
    interest = "\n".join([
        "LLM-enhanced recommender systems: using large language models to improve user/item representations, preference reasoning, cold-start recommendation, and controllable recommendation explanations with reproducible experiments.",
        "large language models",
        "recommender systems",
        "user/item representations",
        "preference reasoning",
        "cold-start recommendation",
        "controllable recommendation explanations",
        "reproducible experiments",
        "LLM integration",
        "Preference hints: reproducible experiments, controllable recommendation explanations",
    ])

    routes = find_pipeline._adaptive_topic_route_lines(interest)
    block = find_pipeline._adaptive_topic_routes_block(AppConfig(research_interest=interest), interest)

    assert routes == ["LLM-enhanced recommender systems: using large language models to improve user/item representations, preference reasoning, cold-start recommendation, and controllable recommendation explanations with reproducible experiments."]
    assert "Preference hints" not in block
    assert "2. reproducible experiments" not in block
    assert "core routes only" in block


def test_final_scoring_demotes_route_fragment_without_complete_route_support():
    class FragmentRouteLLM(BatchLLM):
        def json_or_error(self, prompt: str, temperature=None):
            self.prompts.append(prompt)
            self.temperatures.append(temperature)
            return {
                    ...
                    "evaluations": [
                        {
                            ...
                            "fit_score": 8.4,
                            "diversity_score": 7.0,
                            "hit_directions": ["preference reasoning"],
                            "topic_evidence": "passed: direct title+abstract evidence for a current topic route",
                            "topic_evidence_supported": True,
                            "matched_topic_route": "preference reasoning",
                            "topic_evidence_basis": "The abstract studies human preferences and reward features.",
                            "missing_topic_evidence": [],
                            "fit_explanation": "The paper studies generic preference reasoning.",
                            "fit_explanation_zh": "论文研究通用偏好推理。",
                            "fit_explanation_en": "The paper studies generic preference reasoning.",
                            "reason": "It is useful background for preference reasoning, but the abstract does not establish the complete route.",
                            "reason_zh": "它可作为偏好推理背景，但摘要没有建立完整路线。",
                            "reason_en": "It is useful background for preference reasoning, but the abstract does not establish the complete route.",
                        }
                    ]
                },
            }

    route = "LLM-enhanced recommender systems: using large language models to improve user/item representations and preference reasoning."
    cfg = AppConfig(provider="mock", research_interest=route, max_recommended_papers=5, llm_concurrency=1)
    items = [
        {
            "id": "paper_001",
            "title": "Capturing Individual Human Preferences with Reward Features",
            "abstract": "This paper studies reward features for capturing individual human preferences from feedback.",
            "classification_source": "llm_inferred",
        }
    ]

    evaluated = _evaluate_items(items, cfg, FragmentRouteLLM(), "articles", log=lambda _msg: None)

    assert evaluated[0]["topic_evidence_supported"] is False
    assert evaluated[0]["llm_complete_route_guard_failed"] is True
    assert evaluated[0]["fit_score"] <= 4.8
    assert _recommended(evaluated, cfg) == []


def test_final_scoring_demotes_high_score_that_admits_missing_complete_route_core():
    class MissingCoreLLM(BatchLLM):
        def json_or_error(self, prompt: str, temperature=None):
            self.prompts.append(prompt)
            self.temperatures.append(temperature)
            route = "LLM-enhanced recommender systems: using large language models to improve user/item representations and preference reasoning."
            return {
                ...
                "data": {
                    "evaluations": [
                        {
                            "id": "paper_001",
                            "category": "Recommendation representation learning",
                            "fit_score": 8.6,
                            "diversity_score": 7.0,
                            "hit_directions": ["representation learning", "recommendation"],
                            "topic_evidence": "passed: direct title+abstract evidence for a current topic route",
                            "topic_evidence_supported": True,
                            "matched_topic_route": route,
                            "topic_evidence_basis": "The abstract studies representation learning for recommendation.",
                            "missing_topic_evidence": [],
                            "fit_explanation": "It improves recommendation representations.",
                            "fit_explanation_zh": "它改进推荐表示。",
                            "fit_explanation_en": "It improves recommendation representations.",
                            "reason": "The method could be adapted to the route, but it does not involve LLMs.",
                            "reason_zh": "该方法可迁移到这一路线，但未涉及 LLM。",
                            "reason_en": "The method could be adapted to the route, but it does not involve LLMs.",
                        }
                    ]
                },
            }
    ...
    items = [
        {
            "id": "paper_001",
            "title": "Informative Contrastive Learning for Recommendation",
            "abstract": "This paper proposes a contrastive learning framework for recommender systems and user preference representation.",
            "classification_source": "llm_inferred",
        }
    ]

    evaluated = _evaluate_items(items, cfg, MissingCoreLLM(), "articles", log=lambda _msg: None)
...
def test_final_scoring_canonicalizes_fragment_when_source_supports_complete_route():
    class SupportedFragmentLLM(BatchLLM):
        def json_or_error(self, prompt: str, temperature=None):
            self.prompts.append(prompt)
            self.temperatures.append(temperature)
            return {
                ...
                "data": {
                    "evaluations": [
                        {
                            "id": "paper_001",
                            "category": "LLM recommendation",
                            "fit_score": 8.1,
                            "diversity_score": 6.7,
                            "hit_directions": ["user/item representations"],
                            "topic_evidence": "passed: direct title+abstract evidence for a current topic route",
                            "topic_evidence_supported": True,
                            "matched_topic_route": "user/item representations",
                            "topic_evidence_basis": "The abstract studies LLM recommender representations.",
                            "missing_topic_evidence": [],
                            "fit_explanation": "It uses large language models for recommender representations.",
                            "fit_explanation_zh": "它使用大语言模型改进推荐表示。",
                            "fit_explanation_en": "It uses large language models for recommender representations.",
                            "reason": "The paper directly studies LLM-based recommender systems and representation learning.",
                            "reason_zh": "该论文直接研究基于大语言模型的推荐系统和表示学习。",
                            "reason_en": "The paper directly studies LLM-based recommender systems and representation learning.",
                        }
                    ]
                },
            }
    ...
    items = [
        {
            "id": "paper_001",
            "title": "Large Language Models for Recommender System Representations",
            "abstract": "This paper uses large language models to improve user and item representations in recommender systems with preference reasoning experiments.",
            "classification_source": "llm_inferred",
        }
    ]

    evaluated = _evaluate_items(items, cfg, SupportedFragmentLLM(), "articles", log=lambda _msg: None)
    recommended = _recommended(evaluated, cfg)

    assert evaluated[0]["topic_evidence_supported"] is True
    assert evaluated[0]["matched_topic_route"] == route.rstrip(".")
    assert not evaluated[0].get("llm_complete_route_guard_failed")
    assert [item["id"] for item in recommended] == ["paper_001"]


def test_final_scoring_accepts_core_route_without_all_colon_axes():
    class CoreRouteLLM(BatchLLM):
        def json_or_error(self, prompt: str, temperature=None):
            self.prompts.append(prompt)
            self.temperatures.append(temperature)
            route = "LLM-enhanced recommender systems: using large language models to improve user/item representations, preference reasoning, cold-start recommendation, and controllable recommendation explanations with reproducible experiments."
            return {
                "ok": True,
                "error": "",
                "data": {
                    "evaluations": [
                        {
                            ...
                            "fit_score": 8.0,
                            "diversity_score": 6.8,
                            "hit_directions": ["LLM recommender representations"],
                            "topic_evidence": "passed: direct title+abstract evidence for a current topic route",
                            "topic_evidence_supported": True,
                            "matched_topic_route": route,
                            "topic_evidence_basis": "The abstract directly studies large language models for recommender systems and user/item representations.",
                            "missing_topic_evidence": ["cold-start recommendation", "controllable recommendation explanations"],
                            "fit_explanation": "It directly studies LLM recommender representations.",
                            "fit_explanation_zh": "它直接研究 LLM 推荐系统表示。",
                            "fit_explanation_en": "It directly studies LLM recommender representations.",
                            "reason": "The paper directly supports the core route even though it does not cover every post-colon evidence axis.",
                            "reason_zh": "它直接支持核心路线，但不覆盖冒号后的全部证据方向。",
                            "reason_en": "It directly supports the core route without covering every post-colon evidence axis.",
                        }
                    ]
                },
            }
    ...
    items = [
        {
            "id": "paper_001",
            "title": "Mapping User and Item Representations into Large Language Models for Recommender Systems",
            "abstract": "This paper studies large language models for recommender systems by mapping user and item representations into LLM token spaces and evaluating recommendation accuracy.",
            "classification_source": "llm_inferred",
        }
    ]

    evaluated = _evaluate_items(items, cfg, CoreRouteLLM(), "articles", log=lambda _msg: None)
    recommended = _recommended(evaluated, cfg)
...
def test_complete_route_guard_keeps_source_supported_core_route_when_optional_axis_is_missing():
    class OptionalAxisMissingLLM(BatchLLM):
        def json_or_error(self, prompt: str, temperature=None):
            self.prompts.append(prompt)
            self.temperatures.append(temperature)
            route = "LLM-enhanced recommender systems: using large language models to improve user/item representations, preference reasoning, cold-start recommendation, and controllable recommendation explanations with reproducible experiments."
            return {
                ...
                "data": {
                    "evaluations": [
                        {
                            "id": "paper_001",
                            "category": "LLM recommendation reasoning",
                            "fit_score": 8.1,
                            "diversity_score": 6.9,
                            "hit_directions": ["LLM recommender reasoning"],
                            "topic_evidence": "passed: direct title+abstract evidence for a current topic route",
                            "topic_evidence_supported": True,
                            "matched_topic_route": route,
                            "topic_evidence_basis": "The abstract studies LLM reasoning for recommender systems.",
                            "missing_topic_evidence": ["cold-start recommendation", "controllable recommendation explanations"],
                            "fit_explanation": "It directly studies LLM reasoning for recommender systems.",
                            "fit_explanation_zh": "它直接研究 LLM 推荐系统推理。",
                            "fit_explanation_en": "It directly studies LLM reasoning for recommender systems.",
                            "reason": "The paper directly supports the core route, but it does not mention cold-start recommendation or controllable explanations.",
                            "reason_zh": "它直接支持核心路线，但未涉及冷启动推荐或可控解释。",
                            "reason_en": "The paper directly supports the core route, but it does not mention cold-start recommendation or controllable explanations.",
                        }
                    ]
                },
            }
    ...
    items = [
        {
            "id": "paper_001",
            "title": "Reasoning-Enhanced Recommender Systems with Large Language Models",
            "abstract": "This paper uses large language models to improve reasoning in recommender systems and evaluates recommendation performance on user-item interaction benchmarks.",
            "classification_source": "llm_inferred",
        }
    ]

    evaluated = _evaluate_items(items, cfg, OptionalAxisMissingLLM(), "articles", log=lambda _msg: None)
    recommended = _recommended(evaluated, cfg)
...
def test_topic_evidence_contradiction_allows_positive_core_component_language():
    item = {
        "topic_evidence": "passed: direct title+abstract evidence for a current topic route",
        "topic_evidence_supported": True,
        "matched_topic_route": "LLM-enhanced recommender systems: using large language models to improve user/item representations, preference reasoning, cold-start recommendation, and controllable recommendation explanations with reproducible experiments",
        "topic_evidence_basis": "Title and abstract explicitly address item representation for LLM-based recommender systems.",
        "category": "Item Representation",
        "missing_topic_evidence": [],
        "reason_en": "This paper directly focuses on user/item representations, a core component, and proposes an LLM-based profile extrapolation method. However, it does not cover preference reasoning, cold-start, or controllable explanations.",
        "fit_explanation_en": "It provides concrete reusable value for improving item representations, which is a core component of the research profile.",
    }

    assert find_pipeline._has_topic_evidence_contradiction(item) is False


def test_supported_topic_evidence_is_required_for_recommended_pool():
    cfg = AppConfig(provider="openai", api_key="test-key", model="test-model", max_recommended_papers=3)
    items = [
        {
            "id": "strong_supported",
            "title": "Supported final paper",
            "abstract": "This real abstract provides direct method evidence for the current research route.",
            "reason_source": "llm abstract evaluation",
            "fit_score": 7.8,
            "diversity_score": 6.5,
            "score": 7.5,
            "recommendation_score": 7.5,
            "topic_evidence": "passed: direct title and abstract evidence",
            "topic_evidence_supported": True,
        },
        {
            "id": "weak_fill",
            "title": "Weak filler paper",
            "abstract": "This real abstract is only a boundary match.",
            "reason_source": "llm abstract evaluation",
            "fit_score": 4.8,
            "diversity_score": 4.8,
            "score": 4.8,
    ...
    assert [item["id"] for item in recommended] == ["strong_supported"]
    assert items[1]["find_recommendation_reject_reason"] == "unsupported_final_topic_evidence"


def test_find_recommendation_ignores_read_stage_full_text_flags(tmp_path):
    def item(item_id: str, score: float, **extra) -> dict:
        row = {
            "id": item_id,
            "title": f"Current profile method paper {item_id}",
            "abstract": "This real abstract gives concrete method, data, and evaluation evidence for the current research route.",
            "reason_source": "llm abstract evaluation",
            "fit_score": score,
            "diversity_score": 6.0,
            "score": score,
    ...
    items = [
        item(
            "top_unreadable",
            9.3,
            find_full_text_unavailable_for_read_stage=True,
            find_full_text_required_for_read_stage=True,
            find_full_text_status="missing",
            find_recommendation_reject_reason="full_text_unavailable_for_read_stage",
        ),
        item("second_readable", 8.8),
        item("third_readable", 8.1),
    ]
    cfg = AppConfig(provider="openai", api_key="test-key", model="test-model", max_recommended_papers=2)

    selected = find_pipeline._recommended(items, cfg, source_count=1)

    assert [row["id"] for row in selected] == ["top_unreadable", "second_readable"]
    assert items[0]["find_recommendation"] is True
    assert "find_recommendation_reject_reason" not in items[0]
    assert not (tmp_path / "find_run" / "full_text_reading" / "full_text_packet.json").exists()


def test_attach_abstract_language_fields_preserves_latex_segments_with_placeholders():
    class LatexTranslationLLM(BatchLLM):
        def json_or_error(self, prompt: str, temperature=None):
            self.prompts.append(prompt)
            self.temperatures.append(temperature)
            assert r"\textit{\underline{R}einforced" not in prompt
            assert "extit{" not in prompt
            assert "[[LATEX_1]]" in prompt
            assert "[[LATEX_2]]" in prompt
            assert "[[LATEX_3]]" in prompt
            return {
                "ok": True,
                "error": "",
                "data": {
                    "translations": [
                        {
                            "id": "paper_001",
                            "abstract_zh": "本文提出 [[LATEX_1]]（LatentR[[LATEX_2]]），并优化 [[LATEX_3]]，完整保留原摘要中的方法、动机、实验和结果陈述。",
                        }
                    ]
                },
            }

    llm = LatexTranslationLLM()
    item = {
        "id": "paper_001",
        "title": "LatentR3",
        "abstract": r"We propose extit{\underline{R}einforced \underline{Latent} \underline{R}easoning for \underline{R}ecommendation} (LatentR$^3$), a method for LLM-enhanced recommendation. The objective $L_{rec}=L_1+L_2$ improves ranking quality.",
        "_user_visible_recommendation": True,
    }

    result = _attach_abstract_language_fields([item], llm, log=lambda _msg: None, should_cancel=lambda: False)

    assert result["status"] == "completed"
    assert item["abstract_en"].startswith(r"We propose \textit{\underline{R}einforced")
    assert r"\textit{\underline{R}einforced \underline{Latent} \underline{R}easoning for \underline{R}ecommendation}" in item["abstract_zh"]
    assert r"$^3$" in item["abstract_zh"]
    assert r"$L_{rec}=L_1+L_2$" in item["abstract_zh"]
    assert "[[LATEX_" not in item["abstract_zh"]
    assert re.search(r"(?<!t)extit\{", item["abstract_en"]) is None
    assert llm.temperatures == [0.0]


def test_attach_abstract_language_fields_preserves_scientific_numbers_and_metrics():
    class ScientificNotationTranslationLLM(BatchLLM):
        def json_or_error(self, prompt: str, temperature=None):
            self.prompts.append(prompt)
            self.temperatures.append(temperature)
            assert "1.65×" not in prompt
            assert "$+130\\%$" not in prompt
            assert "Qwen2.5-3B" not in prompt
            placeholders = list(dict.fromkeys(re.findall(r"\[\[LATEX_\d+\]\]", prompt)))
            ...
            return {
                ...
                "data": {
                    "translations": [
                        {
                            "id": "paper_001",
                            "abstract_zh": "本文保留关键科学记号 " + "、".join(placeholders) + "，并忠实翻译其方法、实验协议、主要结果和边界条件。",
                        }
                    ]
                },
            }

    llm = ScientificNotationTranslationLLM()
    item = {
        "id": "paper_001",
        "title": "Scientific notation preservation",
        "abstract": "We report NDCG@10 gains of 1.65× and $+130\\%$ on Qwen2.5-3B with p < 0.05. The method keeps R$^2$ec identifiers, top-K sampling, HR@5, and 0.0264 scores in the original scientific notation while explaining the full experimental protocol and limitations for reproducibility.",
        "_user_visible_recommendation": True,
    }
    ...
    assert result["status"] == "completed"
    assert "1.65×" in item["abstract_zh"]
    assert "$+130\\%$" in item["abstract_zh"]
    assert "Qwen2.5-3B" in item["abstract_zh"]
    assert "NDCG@10" in item["abstract_zh"]
    assert "HR@5" in item["abstract_zh"]
    assert "0.0264" in item["abstract_zh"]
    assert "[[LATEX_" not in item["abstract_zh"]
...
class MissingReasonRetryLLM(BatchLLM):
    def __init__(self):
        super().__init__()
        self.single_calls = 0
    ...
    def json_or_error(self, prompt: str, temperature=None):
        ...
        for item_id in ids:
            row = {
                ...
                "fit_score": 8.1,
                "diversity_score": 7.0,
                "hit_directions_zh": ["生成式AI"],
                "hit_directions_en": ["generative AI"],
                "topic_evidence": "passed:adaptive_llm_topic_route",
                "topic_evidence_supported": True,
                "matched_topic_route": "adaptive route from current research profile",
                "topic_evidence_basis": "abstract",
                "missing_topic_evidence": [],
                "fit_explanation_zh": "摘要提供了可复用的生成式AI方法证据。",
                "fit_explanation_en": "The abstract provides reusable generative AI method evidence.",
                "reason_en": "This paper has reusable method value for the current profile.",
            }
            if include_reason:
                row["reason_zh"] = "该论文提供了可复用的方法证据，并与当前研究画像直接相关。"
            rows.append(row)
        return {"ok": True, "error": "", "data": {"evaluations": rows}}


def test_final_scoring_retries_rows_missing_required_reason(monkeypatch):
    monkeypatch.setenv("OMITTED_ITEM_RETRY_ATTEMPTS", "2")
    llm = MissingReasonRetryLLM()
    cfg = AppConfig(provider="mock", research_interest="生成式AI 科学发现 材料物理", max_recommended_papers=5, llm_concurrency=1)
    items = [
        {
            "id": "paper_001",
    ...
    assert evaluated[0]["reason_source"] == "llm abstract evaluation"
    assert evaluated[0]["llm_retry_attempts"] == 1
    assert evaluated[0]["reason"] == evaluated[0]["reason_zh"]
    assert "Configure an LLM API key" not in evaluated[0]["reason"]
...
def test_recommendation_tie_break_uses_combined_score_for_high_fit_ties():
    cfg = AppConfig(provider="mock", max_recommended_papers=2)
    items = [
        {
            "id": "alpha",
...
def test_topic_evidence_uses_source_text_not_llm_explanation():
    ...
    assert recommended == []
    assert evaluated[0]["find_recommendation_reject_reason"] == "unsupported_final_topic_evidence"


def test_weak_topic_audit_fields_do_not_enter_find_recommendations():
    ...
    assert [item["id"] for item in recommended] == ["low_llm_not_recommended"]
    assert items[0]["find_recommendation_reject_reason"] == "unsupported_final_topic_evidence"
    assert items[0]["not_positive_support"] is True
...
def test_category_selection_cache_reuses_project_bound_llm_choice(monkeypatch, tmp_path):
    monkeypatch.delenv("USE_LLM_CATEGORY_SELECT", raising=False)
    monkeypatch.setenv("WORKSPACE_ROOT", str(tmp_path))
    monkeypatch.setenv("PROJECT_ID", "demo_project")

    class FirstLLM(BatchLLM):
        def json_or_none(self, prompt: str, temperature=None):
            self.prompts.append(prompt)
            self.temperatures.append(temperature)
            return {
                "selected_categories": [{"name": "foundation or frontier models, including LLMs", "reason": "matches profile"}],
                "rejected_categories": [{"name": "computer vision", "reason": "not the route"}],
            }

    class SecondLLM(BatchLLM):
        def json_or_none(self, prompt: str, temperature=None):
            self.prompts.append(prompt)
            self.temperatures.append(temperature)
            return {"selected_categories": [{"name": "computer vision", "reason": "should not be used"}]}
    ...
    first = FirstLLM()
    first_selection = select_relevant_categories(category_summary, cfg, first)
    second = SecondLLM()
    second_selection = select_relevant_categories(category_summary, cfg, second)

    assert len(first.prompts) == 1
    assert second.prompts == []
    assert [row["name"] for row in first_selection["selected_categories"]] == ["foundation or frontier models, including LLMs"]
    assert [row["name"] for row in second_selection["selected_categories"]] == ["foundation or frontier models, including LLMs"]
    assert (tmp_path / "projects" / "demo_project" / "planning" / "finding" / "cache" / "category_selection.json").exists()


def test_category_selection_cache_key_changes_with_profile(monkeypatch, tmp_path):
    monkeypatch.delenv("USE_LLM_CATEGORY_SELECT", raising=False)
    monkeypatch.setenv("WORKSPACE_ROOT", str(tmp_path))
    monkeypatch.setenv("PROJECT_ID", "demo_project")
    ...
    class CategoryLLM(BatchLLM):
        ...
        def json_or_none(self, prompt: str, temperature=None):
            self.prompts.append(prompt)
            self.temperatures.append(temperature)
            return {"selected_categories": [{"name": self.category_name, "reason": "matches this profile"}]}
    ...
    first = CategoryLLM("foundation or frontier models, including LLMs")
    select_relevant_categories(category_summary, AppConfig(provider="openai", api_key="test-key", model="test-model", research_interest="LLM-assisted retrieval benchmark"), first)
    second = CategoryLLM("computer vision")
    selection = select_relevant_categories(category_summary, AppConfig(provider="openai", api_key="test-key", model="test-model", research_interest="vision segmentation benchmark"), second)

    assert len(first.prompts) == 1
    assert len(second.prompts) == 1
    assert [row["name"] for row in selection["selected_categories"]] == ["computer vision"]




def test_category_selection_supplements_too_narrow_llm_choice_for_recall(monkeypatch):
    monkeypatch.delenv("USE_LLM_CATEGORY_SELECT", raising=False)

    class NarrowLLM(BatchLLM):
        def json_or_none(self, prompt: str, temperature=None):
            self.prompts.append(prompt)
            self.temperatures.append(temperature)
            return {"selected_categories": [{"name": "foundation or frontier models, including LLMs", "reason": "direct profile match"}]}
    ...
    category_summary = {
        ...
        "paper_count": 800,
        "category_summary": [
            {"name": name, "count": 100 - index, "sample_titles": [name + " recommendation"], "sample_keywords": [name, "recommendation"]}
            for index, name in enumerate(categories)
        ],
    }

    selection = select_relevant_categories(category_summary, AppConfig(provider="openai", api_key="test-key", model="test-model", research_interest="LLM recommender systems with user feedback"), NarrowLLM(), max_categories=8)

    selected = [row["name"] for row in selection["selected_categories"]]
    assert selected[0] == "foundation or frontier models, including LLMs"
    assert len(selected) >= 6
    assert any("High-recall deterministic supplement" in row["reason"] for row in selection["selected_categories"][1:])



def test_official_category_selection_recalls_locally_relevant_quality_category(monkeypatch):
    def narrow_category_selection(_summary, _config, _llm, max_categories=8):
        return {
            "selected_categories": [{"name": "direct category", "reason": "LLM selected direct category"}],
            "rejected_categories": [
                {"name": "quality recall category", "reason": "LLM omitted broad category"},
                {"name": "unrelated quality category", "reason": "LLM omitted unrelated category"},
            ],
        }

    monkeypatch.setattr(find_pipeline, "select_relevant_categories", narrow_category_selection)
    monkeypatch.setenv("VENUE_CATEGORY_QUALITY_SUPPLEMENT_MAX", "2")
    papers = [
        {
            "id": "direct",
            "title": "Direct selected baseline",
            "abstract": "A baseline that belongs to the selected official category.",
            "category": "direct category",
            "classification_source": "official",
            "metadata": {"category_status": "official_categories"},
        },
        {
            "id": "quality_recall",
            "title": "Adaptive preference learning from user feedback",
            "abstract": "This work studies adaptive preference learning with user feedback and evaluation signals.",
            "category": "quality recall category",
            "classification_source": "official",
            "track": "ICLR 2026 Oral",
            "metadata": {"category_status": "official_categories"},
        },
        {
            "id": "unrelated_quality",
            "title": "Compiler scheduling for accelerator kernels",
            "abstract": "This work studies compiler scheduling and low-level optimization.",
            "category": "unrelated quality category",
            "classification_source": "official",
            "track": "ICLR 2026 Oral",
            "metadata": {"category_status": "official_categories"},
        },
    ...
    filtered, reports = find_pipeline._select_official_category_title_index(
        {"id": "iclr", "name": "ICLR"},
        [2026],
        papers,
        {"adapter": "openreview", "has_official_categories": True, "category_status": "official_categories"},
        AppConfig(provider="mock", research_interest="adaptive preference learning with user feedback"),
        BatchLLM(),
        logs.append,
    )

    assert {item["id"] for item in filtered} == {"direct", "quality_recall"}
    assert reports[0]["quality_category_supplements"] == ["quality recall category"]
    assert reports[0]["selected_category_papers"] == 2
    assert any("high-quality presentation category recall added 1" in message for message in logs)


def test_official_category_selection_adds_paper_level_recall_from_rejected_categories(monkeypatch):
    def narrow_category_selection(_summary, _config, _llm, max_categories=8):
        return {
            "selected_categories": [{"name": "direct category", "reason": "LLM selected direct category"}],
            "rejected_categories": [{"name": "miscellaneous", "reason": "venue-level bucket looked too broad"}],
        }

    monkeypatch.setattr(find_pipeline, "select_relevant_categories", narrow_category_selection)
    monkeypatch.setenv("VENUE_CATEGORY_PAPER_RECALL_LOCAL_SCORE_FLOOR", "0.01")
    monkeypatch.setenv("VENUE_CATEGORY_PAPER_RECALL_MAX", "5")
    papers = [
        {
            "id": "direct",
            "title": "Direct selected baseline",
        ...
        {
            "id": "paper_level_recall",
            "title": "Graph contrastive learning for clinical time-series forecasting",
            "abstract": "This work studies graph contrastive learning for clinical time-series forecasting with longitudinal patient signals.",
            "keywords": ["graph contrastive learning", "clinical time-series forecasting"],
    ...
    filtered, reports = find_pipeline._select_official_category_title_index(
        {"id": "test", "name": "TestVenue"},
        [2026],
        papers,
        {"adapter": "openreview", "has_official_categories": True, "category_status": "official_categories"},
        AppConfig(provider="mock", research_interest="graph contrastive learning clinical time-series forecasting"),
        BatchLLM(),
        logs.append,
    )

    filtered_ids = {item["id"] for item in filtered}
    recalled = next(item for item in filtered if item["id"] == "paper_level_recall")
    assert filtered_ids == {"direct", "paper_level_recall"}
    assert recalled["category_paper_recall_supplement"] is True
    assert recalled["category_paper_recall_original_category"] == "miscellaneous"
    assert reports[0]["selected_category_papers"] == 1
    assert reports[0]["title_filter_input_papers"] == 2
    assert reports[0]["adaptive_paper_recall_supplement_count"] == 1
    assert reports[0]["adaptive_paper_recall_supplement_categories"] == {"miscellaneous": 1}
    assert any("adaptive paper-level recall added 1" in message for message in logs)



def test_final_display_score_uses_diversity_and_gated_quality_bonus():
    llm = BatchLLM()
    cfg = AppConfig(provider="mock", research_interest="LLM-assisted retrieval benchmark", max_recommended_papers=5, llm_concurrency=1)
    items = [
        {
    ...
    assert first["score_source"] == "final_llm_relevance_diversity_plus_gated_quality_bonus"
    assert first["score"] == second["score"]
    assert first["llm_fit_score"] == 8.0
    assert first["score"] == first["recommendation_score"]
    assert first["combined_score"] == 7.75
    assert first["score"] == 7.75
    assert first["combined_score"] == first["recommendation_score"]
    assert first["stable_source_base_score"] <= first["stable_source_score"]

    oral = _evaluate_items([{**items[0], "id": "paper_003", "track": "ICLR 2026 Oral"}], cfg, llm, "articles", log=lambda _msg: None)[0]
    assert oral["quality_bonus"] == 0.53
    assert oral["combined_score"] == 7.75
    assert oral["recommendation_score"] == 8.28
    assert oral["score"] == 8.28
...
def test_contradictory_passed_topic_evidence_is_not_strong():
    ...
    assert recommended == []
    assert items[0]["find_recommendation_reject_reason"] == "contradictory_final_topic_evidence"
...
def test_foundation_borrowing_route_is_not_user_visible_recommendation():
    ...
    assert evaluated[0]["find_recommendation_reject_reason"] == "background_or_foundation_not_user_visible_recommendation"
    assert evaluated[0]["not_positive_support"] is True
...
def test_weak_topic_evidence_high_fit_is_calibrated_before_ranking():
    class InconsistentWeakFitLLM(BatchLLM):
        def json_or_error(self, prompt: str, temperature=None):
            self.prompts.append(prompt)
            self.temperatures.append(temperature)
            rows = []
            for item_id in re.findall(r"paper_(?:weak|passed)", prompt):
                if item_id == "paper_weak":
                    rows.append(
                        {
                            "id": item_id,
                            "category": "Generic preference learning",
                            "fit_score": 8.4,
                            "diversity_score": 8.0,
                            "hit_directions": ["generic preference learning"],
                            "topic_evidence": "weak: missing adaptive topic evidence",
                            "topic_evidence_supported": False,
                            "matched_topic_route": "",
                            "topic_evidence_basis": "abstract mentions only a generic neighboring method",
                            "missing_topic_evidence": ["target problem", "profile-specific entities", "evaluation setting"],
                            "fit_explanation_zh": "摘要只说明一个邻近的通用方法，没有把方法落到当前画像的具体目标、实体或评测设置上。",
                            "fit_explanation_en": "The abstract covers a neighboring generic method but not the profile-specific target problem, entities, or evaluation setting.",
                            "reason_zh": "这篇论文只能提供邻近方法背景，摘要没有显示它能直接支撑当前研究画像中的具体目标、实体、数据设置或评测协议，因此不能被当作高优先级推荐。",
                            "reason_en": "This paper is only neighboring background; the abstract does not support the profile-specific target problem, entities, data setting, or evaluation protocol.",
                        }
                    )
                else:
                    rows.append(
                        {
                            "id": item_id,
                            "category": "Target-domain adaptive systems",
                            "fit_score": 7.2,
                            "diversity_score": 6.0,
                            "hit_directions": ["target-domain adaptive systems"],
                            "topic_evidence": "passed: target-domain adaptive systems",
                            "topic_evidence_supported": True,
                            "matched_topic_route": "target-domain adaptive systems",
                            "topic_evidence_basis": "title and abstract name the target domain, adaptive system, data setting, and evaluation protocol",
                            "missing_topic_evidence": [],
                            "fit_explanation_zh": "摘要明确给出目标域、自适应系统、数据设置和评测协议，能支撑当前画像。",
                            "fit_explanation_en": "The abstract names the target domain, adaptive system, data setting, and evaluation protocol.",
                            "reason_zh": "这篇论文把方法、数据设置和评测协议都落到当前研究画像的目标域中，可直接为后续科研流程提供可复用设计。",
                            "reason_en": "This paper grounds its method, data setting, and evaluation protocol in the profile's target domain, making it reusable for the research workflow.",
                        }
                    )
            return {"ok": True, "error": "", "data": {"evaluations": rows}}

    llm = InconsistentWeakFitLLM()
    cfg = AppConfig(
        provider="openai",
        api_key="test-key",
        model="test-model",
        research_interest="target-domain adaptive systems with data settings and evaluation protocols",
    ...
    items = [
        {
            "id": "paper_weak",
            "title": "Generic Preference Learning with Neighboring Signals",
            "abstract": "This paper studies a generic neighboring preference-learning method without a target-domain problem, data setting, or evaluation protocol.",
            "classification_source": "llm_inferred",
        },
        {
            "id": "paper_passed",
            "title": "Adaptive Systems for Target-Domain Evaluation",
            "abstract": "This paper develops adaptive systems for a target-domain workflow with explicit data settings, evaluation protocols, and reusable method design.",
            "classification_source": "llm_inferred",
        },
    ]
    ...
    assert by_id["paper_weak"]["llm_weak_topic_fit_consistency_repaired"] is True
    assert by_id["paper_weak"]["fit_score"] == 4.8
    assert by_id["paper_weak"]["diversity_score"] == 4.8
    assert by_id["paper_weak"]["quality_bonus"] == 0.0
    assert by_id["paper_weak"]["topic_evidence_supported"] is False
    assert [item["id"] for item in recommended] == ["paper_passed"]


def test_cached_final_llm_weak_topic_score_is_calibrated():
    cfg = AppConfig(
        provider="openai",
        api_key="test-key",
        model="test-model",
    ...
    assert find_pipeline._apply_cached_final_llm_score(item, entry, cfg.research_interest) is True
    assert item["llm_weak_topic_fit_consistency_repaired"] is True
    assert item["fit_score"] == 4.8
    assert item["diversity_score"] == 4.8
...
def test_self_contradictory_foundation_explanation_is_demoted_from_strong():
    ...
    assert recommended == []
    assert items[0]["find_recommendation_reject_reason"] == "background_or_foundation_not_user_visible_recommendation"
    assert items[0]["not_positive_support"] is True
...
def test_llm_audit_booleans_do_not_create_hidden_find_gate():
    ...
    assert [item["id"] for item in recommended] == ["llm_low_score_audit_true"]
    assert items[0]["find_recommendation_reject_reason"] == "unsupported_final_topic_evidence"
    assert "find_recommendation_reject_reason" not in items[1]
...
def test_final_scoring_prompt_clarifies_profile_boundary_without_topic_specific_rules():
    llm = BatchLLM()
    cfg = AppConfig(
        provider="mock",
        research_interest="Adaptive systems for a concrete user-defined application domain",
        max_recommended_papers=5,
        llm_concurrency=1,
    )
    items = [
        {
            "id": "paper_001",
            "title": "Generic Method for Ambiguous Shared Terminology",
            "abstract": "This paper improves a broad method using a shared term but does not describe the target problem, entities, data setting, protocol, or application from the profile.",
            "classification_source": "llm_inferred",
        }
    ]
    ...
    _evaluate_items(items, cfg, llm, "articles", log=lambda _msg: None)
    final_prompt = next(prompt for prompt in llm.prompts if "Candidate items" in prompt)
    rules_block = final_prompt.split("Rules:", 1)[1]

    assert "Shared surface terms are not enough" in rules_block
    assert "concrete target problem" in rules_block
    assert "profile-specific evidence" in rules_block
    assert "possible application, benchmark/dataset domain" in rules_block
    assert "concretely reusable for the research profile" in rules_block
    assert "missing profile evidence" in rules_block
    assert "one decimal place" in rules_block
    assert "Do not default to integer or x.5 scores" in rules_block
    assert "recommender" not in rules_block.lower()
    assert "user-item" not in rules_block.lower()
    assert "collaborative filtering" not in rules_block.lower()
    ...
    items = [
        {
            ...
            "reason_zh": "太短",
            "reason_en": "Too short.",
            "topic_evidence": "passed: recommendation evaluation protocols",
            "topic_evidence_supported": True,
            "missing_topic_evidence": [],
        }
    ]
    ...
    items = [
        {
            ...
            "score": 8.0,
            "reason_source": "llm abstract evaluation",
            "topic_evidence": "passed: semantic conditioning retrieval benchmark",
            "topic_evidence_supported": True,
...
def test_llm_title_scoring_retains_ranked_budget_not_sparse_selected(monkeypatch):
    ...
    missing_ranked = [item for item in selected if item.get("title_llm_missing")]
    assert 0 < len(llm_ranked) < len(selected)
    assert len(missing_ranked) >= 8
    assert len(llm_ranked) + len(missing_ranked) == len(selected)
    assert llm_ranked[0]["title_llm_fit_score"] >= llm_ranked[-1]["title_llm_fit_score"]
    assert all("title_llm_fit_score" not in item for item in missing_ranked)
