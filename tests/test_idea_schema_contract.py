...
ensure_current_find_research_plan = load_script("ensure_current_find_research_plan")
...
def test_current_find_idea_ready_accepts_scored_blocked_backlog_candidate():
    approved = _ready_scored_idea(1)
    blocked = _ready_scored_idea(2)
    blocked.update({
        "status": "blocked_with_reason",
        "selected_for_execution": False,
        "execute_next": False,
        "block_reason": "独立评分认为复杂度与新颖性比例不足，因此作为 backlog 保留，不进入执行计划。",
        "score": 5,
        "idea_score": 5,
        "objective_scores": {
            "novelty": 5,
    ...
    assert ensure_current_find_research_plan._idea_three_part_ready(approved)
    assert ensure_current_find_research_plan._idea_three_part_ready(blocked)
    assert ensure_current_find_research_plan._ideas_three_part_ready([approved, blocked], 2)

    blocked_selected = dict(blocked, selected_for_execution=True)
    assert "objective_score_below_threshold" in ensure_current_find_research_plan._idea_contract_issues(blocked_selected)


def test_current_find_contract_rejects_pre_environment_base_binding():
    ...
    contaminated = _with_scored_idea_contract(
        {
            "id": "idea-rsir-bound",
            "title": "Bound to stale base",
            "new_method": "本方法在RSIR的自进化合成数据框架基础上改造训练损失，并声称与RSIR基底兼容。",
            "initial_experiment": "基于RSIR基库（USTC-StarTeam/RSIR）在amazon-beauty数据集上运行现有训练脚本，对比原始RSIR和新损失。",
            "inspired_by": [{"title": "Current paper", "reason": "方法启发"}],
        },
        9,
    )
    assert "stale_or_preselected_base_binding_detected" in ensure_current_find_research_plan._idea_contract_issues(contaminated)

    plan = {
        "plan_id": "plan-rsir-bound",
        "idea_id": "idea-rsir-bound",
        "title": "Bound plan",
        "steps": ["先复现基线，再实现新损失。"],
        "implementation_details": {
            "base_repo": "USTC-StarTeam/RSIR",
            "training_script": "RSIR现有训练脚本",
        },
        "execution_selection": {
            "selected": True,
            "selected_by": "main_claude_code_after_deep_read",
            "reason": "与当前基底RSIR集成最自然。",
        },
    }
    plan_issues = ensure_current_find_research_plan._plan_rows_contract_issues([plan], 1)
    assert plan_issues
    assert "stale_or_preselected_base_binding_detected" in plan_issues[0]["issues"]
    observed = {
        "plans": 1,
        "contract_plan_count": 1,
        "raw_artifact_plan_count": 1,
        "plan_contract_issues": plan_issues,
    }
    assert ensure_current_find_research_plan.current_find_contract_failure_type({"valid": True}, observed, idea_count=1) == "plan_contract_failed"
...
def test_patch_idea_syncs_project_current_find_state(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert "### 启发来源" in markdown
        assert "### Inspired by" not in markdown
...
def test_current_find_markdown_hides_empty_machine_fields_and_keeps_scores():
    ...
    assert "selected_plan_id" not in plan_md
    assert "selected_idea_id" not in plan_md
    assert "plan_id:" not in plan_md
    assert "idea_id:" not in plan_md


def test_current_find_markdown_resolves_internal_supporting_paper_ids_to_titles():
    paper_index = ensure_current_find_research_plan._paper_reference_index(
        [
            {
                "id": "paper_cb880328e490",
                "paper_id": "paper_cb880328e490",
                "title": "R$^2$ec: Towards Large Recommender Models with Reasoning",
                "venue": "NeurIPS",
                "year": 2025,
                "url": "https://openreview.net/forum?id=VYdzGigCBC",
            }
        ]
    )
    idea = _ready_scored_idea(5, source_title="")
    idea["inspired_by"] = []
    idea["supporting_papers"] = ["paper_cb880328e490"]
    plan = {
        "plan_id": "plan-005",
        "idea_id": idea["id"],
        "title": "可执行计划",
        "selected_for_execution": True,
        "supporting_papers": ["paper_cb880328e490"],
        "steps": ["等待环境阶段选择基底并执行同协议实验。"],
    }

    idea_md = ensure_current_find_research_plan.render_idea_md([idea], "find_demo", paper_index)
    plan_md = ensure_current_find_research_plan.render_plan_md([plan], "find_demo", paper_index)

    assert "R$^2$ec: Towards Large Recommender Models with Reasoning" in idea_md
    assert "R$^2$ec: Towards Large Recommender Models with Reasoning" in plan_md
    assert "paper_cb880328e490" not in idea_md
    assert "paper_cb880328e490" not in plan_md
...
def test_deep_read_fragment_merges_top_level_contract_fields(tmp_path):
    ...
    rows = ensure_current_find_research_plan._current_find_deep_read_fragment_rows(taste_dir, run_id)
    selected = ensure_current_find_research_plan._select_current_find_readings_from_candidates(
        rows,
        {"run_id": run_id, "strong_recommendations": [{"id": "paper-1", "title": title}]},
        {"run_id": run_id, "papers": [{"paper_id": "paper-1", "title": title, "pdf_url": "https://example.test/paper.pdf", "text_path": "texts/paper.txt", "text_chars": 50000}]},
    )

    assert len(rows) == 1
    assert rows[0]["abstract_zh"] == fields["abstract_zh"]
    assert rows[0]["method_advantages_zh"] == fields["method_advantages_zh"]
    assert rows[0]["deep_read_audit"]["fragment_path"].endswith("01_paper-1_repair_attempt2.json")
...
def test_load_claude_outputs_keeps_fragment_readings_when_ideas_and_plans_are_stale(tmp_path):
    ...
    stale_epoch = ensure_current_find_research_plan.dt.datetime.fromisoformat(stale_time).timestamp()
    os.utime(taste_dir / "ideas.json", (stale_epoch, stale_epoch))
    os.utime(taste_dir / "plans.json", (stale_epoch, stale_epoch))
...
def test_reading_quality_prefers_pdf_read_fragment_over_pending_status_clone():
    fields = _v4_deep_read_fields("Status Preference Paper")
    pending = {
        "paper_id": "paper-status",
        "title": "Status Preference Paper",
        "url": "https://example.test/status",
        **fields,
        "method_details_zh": fields["method_details_zh"] + "额外字段长度用于模拟旧 pending 行只是在字数上略长。",
        "full_text_available": False,
        "full_text_status": "full_text_packet_ready_pending_deep_read_synthesis",
        "pdf_text_chars": 92518,
        "source_evidence": {"full_text_status": "pdf_text_read", "text_chars": 92518, "text_path": "texts/status.txt"},
        "subagent_deep_read": True,
        "deep_read_audit": {"mode": "task_subagent", "subagent_used": True, "status": "completed", "fragment_path": "old.json", "evidence_chars": 92518},
    }
    repair = dict(pending)
    repair.update({
        "full_text_available": True,
        "full_text_status": "pdf_text_read",
        "pdf_text_read": True,
        "method_details_zh": fields["method_details_zh"],
        "deep_read_audit": {"mode": "task_subagent", "subagent_used": True, "status": "completed", "fragment_path": "repair.json", "evidence_chars": 92518},
    })

    assert ensure_current_find_research_plan._reading_quality_score(repair) > ensure_current_find_research_plan._reading_quality_score(pending)

    candidate_index = {}
    for candidate in [pending, repair]:
        for identity in ensure_current_find_research_plan._reading_identity_values(candidate):
            ensure_current_find_research_plan._index_better_candidate(candidate_index, identity, candidate)
    selected = ensure_current_find_research_plan._best_candidate_for_find_row(
        candidate_index,
        [pending, repair],
        {"id": "paper-status", "title": "Status Preference Paper", "url": "https://example.test/status"},
    )
    assert selected is repair


def test_normalize_preserves_pdf_read_status_when_deep_read_content_needs_revision():
    title = "Readable But Short Method Paper"
    row = {
        "paper_id": "paper-readable-short",
        "title": title,
        "url": "https://example.test/readable-short",
        "pdf_url": "https://example.test/readable-short.pdf",
        "full_text_available": True,
        "full_text_status": "pdf_text_read",
        "pdf_text_read": True,
        "pdf_text_chars": 12000,
        "subagent_deep_read": True,
        "deep_read_audit": {"mode": "task_subagent", "subagent_used": True, "status": "completed", "text_path": "texts/readable-short.txt", "evidence_chars": 12000},
        **_v4_deep_read_fields(title),
    }
    row["method_details_zh"] = "模型结构包含输入输出、训练目标和推理流程。" * 20
    row["method"] = row["method_details_zh"]
    packet_entry = {
        "title": title,
        "paper_id": "paper-readable-short",
        "pdf_url": "https://example.test/readable-short.pdf",
        "text_path": "texts/readable-short.txt",
        "text_chars": 12000,
        "pdf_text_chars": 12000,
    }
    ...
    assert ensure_current_find_research_plan._reading_deep_read_content_gaps(normalized)
    assert normalized["full_text_status"] == "pdf_text_read"
    assert normalized["full_text_available"] is True
    assert normalized["pdf_text_read"] is True
    assert "reading_status_note_zh" not in normalized
...
def test_claude_tool_policy_blocks_secret_env_probes():
    ...
    readonly_current_find_mapping = (
        "/home/fmh/workspace/miniforge/envs/ar_taste/bin/python3.11 -c "
        '"titles=[\'FACE: Mapping Collaborative Filtering Embeddings into LLM Tokens\']; '
        "common=set(titles[0].lower()); print(len(common))\""
    )
    assert claude_project_session.bash_command_tool_policy_issue(
        readonly_current_find_mapping,
        "sample_project",
...
def test_runtime_artifact_state_file_tools_and_redirects_are_wrapper_owned():
    import importlib.util
    ...
    blocked_paths = [
        "/home/fmh/workspace/TASTE/projects/demo/state/experiment_registry.json",
        "/home/fmh/workspace/TASTE/projects/demo/state/experiment_record_table.json",
        "/home/fmh/workspace/TASTE/projects/demo/experiments/experiment_records.csv",
        "/home/fmh/workspace/TASTE/projects/demo/artifacts/run_a/run_contract.json",
        "/home/fmh/workspace/TASTE/projects/demo/artifacts/run_a/running_status.json",
        "/home/fmh/workspace/TASTE/projects/demo/artifacts/run_a/bad_cases.json",
        "/home/fmh/workspace/TASTE/projects/demo/artifacts/run_a/FAILED_DO_NOT_IMPORT.txt",
    ]
    for blocked_path in blocked_paths:
        assert claude_project_session.runtime_artifact_state_tool_policy_issue(
            "Write",
            {"file_path": blocked_path},
            stage,
        ) == claude_project_session.RUNTIME_ARTIFACT_STATE_WRITE_POLICY

    assert claude_project_session.bash_command_tool_policy_issue(
        "echo '{}' > /home/fmh/workspace/TASTE/projects/demo/state/experiment_registry.json",
        "demo",
        stage,
    ) == claude_project_session.RUNTIME_ARTIFACT_STATE_WRITE_POLICY
    assert claude_project_session.bash_command_tool_policy_issue(
        "artifact=/home/fmh/workspace/TASTE/projects/demo/artifacts/run_a/running_status.json; echo '{}' > $artifact",
        "demo",
        stage,
    ) == claude_project_session.RUNTIME_ARTIFACT_STATE_WRITE_POLICY
    ...
    assert claude_project_session.runtime_artifact_state_tool_policy_issue(
        "Write",
        {"file_path": "/home/fmh/workspace/TASTE/projects/demo/reports/status.md"},
        stage,
    ) == ""
    assert claude_project_session.runtime_artifact_state_tool_policy_issue(
        "Write",
        {"file_path": "/home/fmh/workspace/TASTE/projects/demo/artifacts/run_a/notes.md"},
        stage,
    ) == ""
...
"""
    ...
    launcher_training = "/opt/conda/envs/env/bin/python modules/experimenting/scripts/launch_experiment_run.py --project sample_project --artifact-name demo --cwd /tmp -- /opt/conda/envs/project_env/bin/python -u train_diffusion.py --data ATV"
    assert claude_project_session.bash_command_tool_policy_issue(
        launcher_training,
        "sample_project",
        "experiment",
...
def test_main_blocks_failed_current_find_takeover_without_normalized_artifacts(tmp_path, monkeypatch, capsys):
    ...
    assert "Read-stage full-text packet evidence is ready" in "\n".join(validation.get("blockers") or [])
...
def test_current_find_full_text_preflight_accepts_read_stage_replacement(tmp_path, monkeypatch):
    ...
    assert validation["pending_without_evidence_titles"] == []
...
def test_refresh_current_find_claude_outputs_rejects_replacement_readings(tmp_path):
    ...
    assert [row["title"] for row in readings] == ["Readable Paper"]
    assert ideas and plans and targeted_queries == ["query one", "query two", "query three"]
    assert validation["valid"] is False
    assert persisted["valid"] is False
    assert persisted["recommended_reading_count"] == 1
    assert persisted["expected_recommendation_count"] == 2
    assert persisted["actual_reading_count"] == 1
    assert persisted["full_text_evidence_count"] == 1
    assert persisted["pending_without_evidence_count"] == 1
    assert persisted["pending_without_evidence_titles"] == ["Missing Paper"]
    assert persisted["extra_reading_count"] == 0
    assert persisted["extra_reading_titles"] == []
    assert persisted["reading_replacement_count"] == 0
    assert persisted["reading_replacement_titles"] == []
    assert persisted["positive_anchor_count"] == 1
    assert persisted["critique_or_boundary_count"] == 0
...
def test_deep_read_contract_rejects_scientific_numbers_spelled_as_chinese_words_and_placeholder_leaks():
    row = {
        "title": "Scientific notation leak example",
        **_v4_deep_read_fields("Scientific notation leak example"),
    }
    row["method_details_zh"] = row["method_details_zh"] + " 论文还错误泄漏了 @@TASTE_INLINE_1@@ 这样的内部公式占位符。"
    row["experiments_zh"] = (
        "实验使用 Amazon Instruments、CDs 和 Games 数据集，比较 GRU4Rec、SASRec、TIGER、BigRec、LLaRA 和推理增强推荐基线。"
        "指标包括 HR@5、HR@10、NDCG@10 和 p 值，主结果却把 HR@5 写成零点零二六四、把相对提升写成百分之五十点八六，"
        "还把学习率写成一乘以十的负五次方，这些都不是可发布的科学记号写法。"
        "消融覆盖去除推理、替换推荐头、只用离散奖励和只用连续奖励，并讨论延迟、显存、top-K 和温度设置。"
        "该字段长度足够，但科学数字格式不合格，必须触发 TASTE 重新生成而不是在网页上发布。"
    )

    gaps = ensure_current_find_research_plan._reading_deep_read_content_gaps(row)

    assert any("占位符" in item or "placeholder" in item for item in gaps)
    assert any("科学数字" in item for item in gaps)
...
def test_current_find_stage_blocks_source_code_file_writes():
    ...
    assert "only use Claude file tools" in claude_project_session.current_find_tool_policy_issue(
        "Edit",
        {"file_path": "/workspace/taste/web/backend/auto_research/web/project_bridge.py"},
        stage,
    )
    assert not claude_project_session.current_find_tool_policy_issue(
        "Edit",
...
def test_import_experiment_artifacts_recognizes_structured_completed_marker(tmp_path, monkeypatch):
    import importlib.util
    import json
    ...
    monkeypatch.setattr(importer, "live_process_for_artifact", lambda artifact_dir, config: False)
    metric_line = "[runner] {'epoch': 29, 'ndcg@20': tensor(0.0585), 'ndcg@10': tensor(0.0508)}"
    log_text = metric_line + "\n[runner] " + json.dumps({"status": "completed", "executed_epochs": 30})

    completion = importer.completion_evidence(tmp_path, {}, log_text, [{"epoch": 29, "ndcg_at_10": 0.0508, "ndcg_at_20": 0.0585}])

    assert completion["completed"] is True
    assert completion["explicit_training_complete_marker"] is True
    assert completion["last_epoch_seen"] == 29
    assert completion["planned_epochs"] == 30
    assert completion["planned_epochs_source"] == "structured_completion_marker"


def test_import_experiment_artifacts_treats_launcher_contract_runner_as_live(tmp_path, monkeypatch):
    import importlib.util
    import json
    ...
    artifact_dir.mkdir()
    (artifact_dir / "run_contract.json").write_text(
        json.dumps({"pid": 4242, "command": ["/project/env/bin/python", "-u", "custom_research_runner.py"], "command_display": "/project/env/bin/python -u custom_research_runner.py"}),
        encoding="utf-8",
    )
    monkeypatch.setattr(importer, "process_alive", lambda pid: str(pid) == "4242")
    monkeypatch.setattr(importer, "proc_cmdline", lambda pid: "/project/env/bin/python -u custom_research_runner.py")

    assert importer.live_process_for_artifact(artifact_dir, {}) is True
