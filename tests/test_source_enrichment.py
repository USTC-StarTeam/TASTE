...
def test_full_title_index_uses_dblp_as_base_and_enriches_from_openreview():
    ...
    with patch("auto_research.auto_find.sources.fetch_dblp_venue", return_value=dblp_papers) as fetch_dblp, patch("auto_research.auto_find.sources.fetch_openreview_venue", return_value=openreview_papers), patch("auto_research.auto_find.sources.fetch_pmlr_index", return_value=[]):
...
def test_full_title_index_falls_back_when_dblp_base_empty():
    ...
    with patch("auto_research.auto_find.sources.fetch_dblp_venue", return_value=[]), patch("auto_research.auto_find.sources.fetch_pmlr_index", return_value=pmlr_papers), patch("auto_research.auto_find.sources.fetch_openreview_venue", return_value=[]):
        papers, adapter = fetch_venue_title_index_all(venue, [2025])
...
def test_full_title_index_promotes_pmlr_url_when_base_url_missing():
    venue = {
        "id": "ccf_ai_conference_a_icml_international_conference_on_machine_learning",
        "name": "ICML",
        "full_name": "International Conference on Machine Learning",
    ...
    with patch("auto_research.auto_find.sources.fetch_dblp_venue", return_value=dblp_papers), patch("auto_research.auto_find.sources.fetch_openreview_venue", return_value=[]), patch("auto_research.auto_find.sources.fetch_pmlr_index", return_value=pmlr_papers):
        papers, adapter = fetch_venue_title_index_all(venue, [2025])

    assert adapter == "dblp+pmlr:1"
    ...
    assert papers[0]["metadata"]["pmlr_url"] == "https://proceedings.mlr.press/v267/example.html"
