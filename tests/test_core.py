...
def test_markdown_hides_internal_recommendation_debug_fields():
    ...
    assert "- **推荐分数**: 8.00 / 10" in content
    assert "- **URL**" not in content
    assert "- **PDF**:" not in content


def test_markdown_preserves_public_latex_text_commands_for_renderer():
    content = paper_markdown([
        {
            "id": "p1",
            "title": r"$\textit{S}$-SPPO and $\texttt{G1}$",
            "venue": "ICLR",
            "year": 2026,
            "track": "ICLR 2026 Poster",
            "abstract_zh": r"摘要中出现\textit{收敛不一致}，也可能因为 JSON 转义显示成 extbf{重点贡献}，还包括\textbf{\underline{S}}tructure、\textsc{NeedleInATable}、\textbf{\justask{}} 与 $\textbf{11$^{\circ}$}$ 以及 Compositional-ARC\textemdash{}。",
            "reason_zh": r"推荐理由包含\emph{可复用方法}和 extit{风险边界}。",
        }
    ])

    assert r"## 1. \textit{S}-SPPO and \texttt{G1}" in content
    assert r"\textit{收敛不一致}" in content
    assert r"\textbf{重点贡献}" in content
    assert r"\textbf{\underline{S}}tructure" in content
    assert r"\textsc{NeedleInATable}" in content
    assert r"\textbf{justask}" in content
    assert r"\justask" not in content
    assert "Compositional-ARC --" in content
    assert r"\emph{可复用方法}" in content
    assert r"\textit{风险边界}" in content
    assert "justask{}" not in content
    assert "textemdash" not in content
    assert re.search(r"(?<!t)extit\{", content) is None
    assert re.search(r"(?<!t)extbf\{", content) is None
    assert "- **会议/年份**: ICLR 2026 / Poster" in content
    assert "- **Track/类型**" not in content
...
def test_markdown_contains_quality_labels_and_score_bonus_details():
    ...
    assert "- **会议/年份**: NeurIPS 2026 / Oral" in content
    assert "- **Track/类型**" not in content
...
def test_icml_downloads_records_metadata_completeness_audit(monkeypatch):
    ...
    def fake_request(url, timeout=30):
        if str(url).endswith("/Downloads/2026"):
            return Response()
        raise RuntimeError("event page unavailable")
...
def test_icml_downloads_enriches_presentation_from_official_event_pages(monkeypatch):
    ...
    <html><body>
      <a href="/virtual/2026/poster/1">Diffusion Recommendation with Semantic Signals</a>
      <a href="/virtual/2026/poster/2">LLM Conditioned Sequential Recommendation</a>
      <a href="/virtual/2026/poster/3">Regular Poster Baseline</a>
    </body></html>
    """
    oral = """
    <html><body>
      <a href="/virtual/2026/oral/91">Diffusion Recommendation with Semantic Signals</a>
      <a href="/virtual/2026/oral/91">View full details</a>
    </body></html>
    """
    spotlight = """
    <html><body>
      <a href="/virtual/2026/poster/1">Diffusion Recommendation with Semantic Signals</a>
      <a href="/virtual/2026/poster/2">LLM Conditioned Sequential Recommendation</a>
      <a href="/virtual/2026/events/spotlight">Schedule</a>
    </body></html>
    """

    def fake_request(url, timeout=30):
        text = str(url)
        if text.endswith("/Downloads/2026"):
            return Response(downloads)
        if text.endswith("/events/oral"):
            return Response(oral)
        if text.endswith("/events/2026SpotlightPosters"):
    ...
    oral_paper = by_title["Diffusion Recommendation with Semantic Signals"]
    spotlight_paper = by_title["LLM Conditioned Sequential Recommendation"]
    poster_paper = by_title["Regular Poster Baseline"]
    assert oral_paper["presentation_type"] == "oral"
    assert oral_paper["track"] == "ICML 2026 Oral"
    assert oral_paper["metadata"]["presentation_source"] == "icml_events_presentation"
    assert oral_paper["metadata"]["presentation_url"].endswith("/virtual/2026/oral/91")
    assert spotlight_paper["presentation_type"] == "spotlight"
    assert spotlight_paper["track"] == "ICML 2026 Spotlight"
    assert poster_paper["presentation_type"] == "poster"
    audit = sources.venue_metadata_audit_from_papers(papers)
    assert audit["presentation_counts"] == {"oral": 1, "poster": 1, "spotlight": 1}
    assert audit["presentation_audits"][0]["counts"] == {"oral": 1, "spotlight": 2}
    assert audit["presentation_audits"][0]["applied_counts"] == {"oral": 1, "spotlight": 1}
...
def test_quality_bonus_ignores_venue_level_presentation_audit_text():
    row = {
        "title": "Poster Paper with Venue Audit Metadata",
        "abstract": "This paper studies recommendation with language model preference signals, adaptive personalization, offline evaluation, and robust user modeling.",
        "venue": "ICML",
        "year": 2026,
        "track": "ICML 2026 Poster",
        ...
        "metadata": {
            "presentation_type": "poster",
            "presentation_label": "ICML 2026 Poster",
            "detail_url": "https://icml.cc/virtual/2026/poster/42",
            "venue_metadata_audit": {
                "source_yeaudits": [
                    {
                        "presentation_audit": {
                            "sources": [
                                {
                                    ...
                                    "source_url": "https://icml.cc/virtual/2026/events/oral",
                                    "matched_titles": 168,
                                }
                            ],
                            "counts": {"oral": 168},
    ...
    assert "oral" not in row["presentation_labels"]
    assert row["quality_bonus"] == 0.08
    assert row["quality_bonus_reason"] == "普通顶级会议小加分: ICML +0.08"
...
def test_final_llm_score_cache_reuses_valid_profile_bound_scores():
    ...
    item = {
        "title": "Self Improving Recommendation with Synthetic Feedback",
        "abstract": "This paper studies recommender systems with synthetic user feedback, iterative self-improvement, fidelity control, and offline evaluation.",
        "category": "recommendation",
        "fit_score": 8.0,
        "diversity_score": 7.0,
        "reason_source": "llm abstract evaluation",
        "hit_directions_zh": ["自改进推荐", "合成反馈"],
        "hit_directions_en": ["self-improving recommendation", "synthetic feedback"],
        "fit_explanation_zh": "该论文在题名和摘要中同时覆盖推荐系统、合成用户反馈、自改进流程和保真控制，能够为当前研究画像提供直接的方法结构、数据构造和评价协议参考。摘要还暴露了需要继续核查的迁移风险。",
        "reason_zh": "对当前 LLM 推荐系统研究来说，这篇论文的价值在于把合成反馈、自改进推荐和保真控制放在同一个可评测框架中。它可以帮助设计用户序列生成、过滤机制、离线评价和消融实验，并提示摘要层面仍需核查真实用户行为分布、长期反馈偏差、冷启动场景以及生成数据与真实点击序列之间的分布差异。该信息足以作为后续方案选择的重点参考。",
        "reason_en": "For the current LLM recommender project, the paper is useful because it combines synthetic feedback, self-improving recommendation, and fidelity control in one evaluable framework. It can inform sequence generation, filtering mechanisms, offline evaluation, and ablations, while leaving user-behavior distribution shift as an abstract-level risk.",
        "topic_evidence": "passed: direct recommendation feedback route",
        "topic_evidence_supported": True,
        "matched_topic_route": "LLM recommender systems with synthetic feedback",
        "topic_evidence_basis": "title and abstract mention recommender systems, synthetic user feedback, self-improvement, and fidelity control",
        "missing_topic_evidence": [],
    }
    key = pipeline._final_llm_score_cache_key(item, cfg, interest, "")
    entry = pipeline._final_llm_score_cache_entry(item, key, cfg)
    fresh = {
        "title": item["title"],
    ...
    assert pipeline._apply_cached_final_llm_score(fresh, entry, interest) is True
    assert fresh["fit_score"] == 8.0
    assert fresh["reason_source"] == "llm abstract evaluation"
    assert fresh["llm_final_scoring_cache_hit"] is True
    assert fresh["topic_evidence_source"] == "llm_adaptive"
    assert pipeline._final_llm_score_cache_key(item, cfg, interest + " with graph retrieval", "") != key
...
def test_final_llm_score_cache_accepts_concise_valid_reason_but_rejects_internal_text():
    ...
    cfg = AppConfig(provider="openai_compatible", base_url="https://llm.example/v1", api_key="key", model="judge-model")
    base = {
        "title": "Concise Recommendation Reason",
        "abstract": "A recommender systems abstract with user feedback and evaluation evidence.",
        "fit_score": 7.0,
        "diversity_score": 6.0,
        "reason_source": "llm abstract evaluation",
        "reason_zh": "可复用用户反馈建模和评测协议。",
    }
    key = pipeline._final_llm_score_cache_key(base, cfg, "LLM recommender systems", "")

    entry = pipeline._final_llm_score_cache_entry(base, key, cfg)
    assert entry["cache_key"] == key
    assert pipeline._final_llm_cache_entry_valid(entry) is True

    bad = dict(base, reason_zh="passed: adaptive_llm_topic_route")
    assert pipeline._final_llm_score_cache_entry(bad, key, cfg) == {}
...
def test_title_filter_uses_zero_temperature_for_stable_candidate_recall(monkeypatch):
    ...
    def fake_json(_llm, _prompt, *, temperature=None, max_tokens=None, timeout_sec=0):
        calls.append({"temperature": temperature, "max_tokens": max_tokens, "timeout_sec": timeout_sec})
        return {
            "ok": True,
            "data": {
                "scored": [
                    {
                        ...
                        "category": "recommendation",
                        "reason": "题名明确包含 LLM 推荐和用户反馈，适合进入详情评分。",
                    }
                ]
            },
        }
    ...
    monkeypatch.setattr(pipeline, "_json_or_error_wall_timeout", fake_json)
    cfg = AppConfig(research_interest="LLM recommender systems with user feedback", provider="openai_compatible", api_key="key", model="model")

    rows = pipeline._prefilter_titles(
        [{"id": "p1", "title": "LLM Recommendation with User Feedback", "abstract": ""}],
        cfg,
        FakeLLM(),
        "TestVenue",
        lambda *_args: None,
    ...
    assert [call["temperature"] for call in calls] == [pipeline.FIND_TITLE_FILTER_TEMPERATURE]
...
def test_title_filter_reuses_project_bound_cache_without_llm_call(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("PROJECT_ID", "demo_project")
    cfg = AppConfig(
        research_interest="LLM recommender systems with user feedback",
    ...
    item = {"id": "p1", "title": "LLM Recommendation with User Feedback", "abstract": "", "venue": "ICLR", "year": 2026}
    scoring_interest = pipeline._compact_scoring_interest(cfg, pipeline._topic_interest_text(cfg))
    key = pipeline._title_llm_score_cache_key(item, cfg, scoring_interest, "batch context 1/100")
    assert key == pipeline._title_llm_score_cache_key(item, cfg, scoring_interest, "different batch context 2/101")
    cached_item = {
        **item,
        "fit_score": 8.5,
        ...
        "hit_directions": ["LLM recommendation"],
        "category": "recommendation",
        "title_reason": "题名明确命中 LLM 推荐和用户反馈。",
        "reason_source": "llm title filter",
    }
    legacy_key = "legacy-context-sensitive-key"
    entry = pipeline._title_llm_score_cache_entry(cached_item, legacy_key, cfg)
    cache_path = pipeline._title_llm_score_cache_path()
    cache_path.parent.mkdir(parents=True, exist_ok=True)
    cache_path.write_text(json.dumps({"schema": pipeline.TITLE_LLM_SCORE_CACHE_SCHEMA_VERSION, "entries": {legacy_key: entry}}, ensure_ascii=False), encoding="utf-8")

    def fail_if_called(*_args, **_kwargs):
        raise AssertionError("title LLM should not be called on a cache hit")

    monkeypatch.setattr(pipeline, "_json_or_error_wall_timeout", fail_if_called)
    rows = pipeline._prefilter_titles([dict(item)], cfg, llm, "ICLR", lambda *_args: None, lambda: False, scan_all=True, result_limit=1)

    assert rows[0]["id"] == "p1"
    assert rows[0]["title_llm_fit_score"] == 8.5
    assert rows[0]["llm_title_filter_cache_hit"] is True
    assert rows[0]["llm_title_filter_cache_migrated"] is True
    refreshed = json.loads(cache_path.read_text(encoding="utf-8"))
    assert key in refreshed.get("entries", {})
...
def test_title_filter_cache_policy_separates_abstract_snippet_prompts(monkeypatch, tmp_path):
    ...
    title_only = {"id": "p1", "title": "LLM Recommendation with User Feedback", "abstract": "", "venue": "ICLR", "year": 2026}
    with_snippet = {**title_only, "abstract": "This paper studies recommender systems with online user feedback."}
    scoring_interest = pipeline._compact_scoring_interest(cfg, pipeline._topic_interest_text(cfg))

    title_key = pipeline._title_llm_score_cache_key(title_only, cfg, scoring_interest, "batch context")
    snippet_key = pipeline._title_llm_score_cache_key(with_snippet, cfg, scoring_interest, "batch context")

    assert title_key != snippet_key
    assert pipeline._title_llm_score_cache_policy(title_only) == pipeline.TITLE_LLM_SCORE_CACHE_POLICY_TITLE_ONLY
    assert pipeline._title_llm_score_cache_policy(with_snippet) == pipeline.TITLE_LLM_SCORE_CACHE_POLICY_WITH_SNIPPETS
    ...
    entry = pipeline._title_llm_score_cache_entry(cached_title_only, title_key, cfg)

    title_target = dict(title_only)
    snippet_target = dict(with_snippet)

    assert pipeline._apply_cached_title_llm_score(
        title_target,
        entry,
        scoring_interest,
        expected_policy=pipeline._title_llm_score_cache_policy(title_target),
    ) is True
    assert pipeline._apply_cached_title_llm_score(
        snippet_target,
        entry,
        scoring_interest,
        expected_policy=pipeline._title_llm_score_cache_policy(snippet_target),
    ) is False
...
def test_openreview_venue_preserves_official_primary_area(monkeypatch):
    ...
    assert papers[0]["presentation_type"] == "poster"
    assert papers[0]["presentation_label"] == "ICLR 2026 Poster"
    assert papers[0]["metadata"]["presentation_source"] == "openreview_venue_text"
...
def test_openreview_venue_normalizes_oral_presentation_from_track(monkeypatch):
                ...
                "notes": [
                    {
                        ...
                        "forum": "oral123",
                        "content": {
                            "title": {"value": "Preference Modeling for Personalized Language Systems"},
                            "authors": {"value": ["Alice Example"]},
                            "abstract": {"value": "This paper studies personalized language systems with preference modeling and evaluation."},
    ...
    assert papers[0]["track"] == "ICLR 2026 Oral"
    assert papers[0]["presentation_type"] == "oral"
    assert papers[0]["presentation_label"] == "ICLR 2026 Oral"
    assert papers[0]["metadata"]["presentation_type"] == "oral"
    assert papers[0]["metadata"]["presentation_source"] == "openreview_venue_text"
