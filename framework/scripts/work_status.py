...
def append_safe_unblock_status(project: str, payload: dict[str, Any]) -> None:
    lines = [
        "### 当前结论",
        f"- 项目: {project}",
        f"- 目标会议/期刊: {_clean(payload.get('target_venue'))}",
        f"- 当前环境阶段选出的基底/仓库: {_clean(payload.get('main_base'))}",
        f"- Find run: {_clean(payload.get('find_run_id'))}",
        f"- full_cycle 状态: {_clean(payload.get('full_status') or payload.get('status'))}",
        f"- 当前 blocker: {_clean(payload.get('blocker'))}",
        f"- 修复路线: {_clean(payload.get('top_route'))}",
        "",
        "### Fresh-base / 参考复现门控",
        f"- 数据: status={_clean(payload.get('data_status'))}；decision={_clean(payload.get('data_decision'))}；latest_download_rc={_clean(payload.get('latest_download_rc'), '-')}",
        f"- Loader: status={_clean(payload.get('loader_status'))}；decision={_clean(payload.get('loader_decision'))}",
        f"- 可用数据集: {_clean(payload.get('ready_datasets'), '无')}",
        f"- 阻塞数据集: {_clean(payload.get('blocked_datasets'), '无')}",
        f"- Reference protocol: {_clean(payload.get('reference_protocol_status'))}",
        f"- Reference smoke: {_clean(payload.get('reference_smoke_status'))}",
        f"- Reference audit: status={_clean(payload.get('reference_audit_status'))}；decision={_clean(payload.get('reference_audit_decision'))}；artifact={_clean(payload.get('reference_audit_artifact'))}",
        _pid_line("full reference reproduction", {"status": payload.get("reference_full_job_status"), "pid": payload.get("reference_full_job_pid"), "log_path": payload.get("reference_full_job_log"), "alive": None}),
        "",
        "### 本次 safe-unblock 执行步骤",
        *_step_lines(payload.get("steps")),
        "",
        "### 安全约束",
        f"- {_clean(payload.get('guardrail'))}",
    ]
    append_entry(f"TASTE safe-unblock / {project}", lines)
