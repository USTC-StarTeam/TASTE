...
from project.project_paths import PROJECT_TEMPLATE, ROOT, build_paths, load_project_config, validate_project_name
from runtime.framework_io import write_json_raw as _write_json

from runtime.taste_pythonpath import ensure_taste_pythonpath, resolve_script_path
ensure_taste_pythonpath(ROOT)
from policies.source_selection import enabled_sources_for, normalize_source_selection, save_canonical_source_selection
...
def _sync_discovery_booleans(discovery: dict[str, Any], selection: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, enabled in {
        ...
        out[key] = child
    out["enabled_sources"] = enabled_sources_for(selection)
    return out
...
def _apply_project_patch(cfg: dict[str, Any], patch: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any] | None]:
    ...
    if "max_read_papers" in patch:
        try:
            max_read_papers = int(patch.get("max_read_papers"))
        except (TypeError, ValueError) as exc:
            raise ValueError("max_read_papers must be a positive integer") from exc
        if max_read_papers < 1:
            raise ValueError("max_read_papers must be a positive integer")
        cfg["max_read_papers"] = max_read_papers
        reading = dict(cfg.get("reading") or {}) if isinstance(cfg.get("reading"), dict) else {}
        reading.update({
            "max_read_papers_source": "user",
            "max_read_papers_updated_at": datetime.now(timezone.utc).isoformat(),
        })
        cfg["reading"] = reading
...
def create_project_settings(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    proc = subprocess.run([sys.executable, str(resolve_script_path("init_project.py", ROOT)), "--project", name], cwd=ROOT, text=True, capture_output=True)
    ...
    if not activate.exists():
        activate.write_text(
            "\n".join([
                ...
                f"exec \"$ROOT/framework/scripts/launchers/run_in_conda.sh\" \"{name}\" \"$@\"",
                "",
            ]),
            encoding="utf-8",
        )
