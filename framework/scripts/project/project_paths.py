...
def _ensure_framework_pythonpath() -> None:
    scripts_root = ROOT / "framework" / "scripts"
    entries: list[Path] = [
        ROOT / "framework",
        ROOT / "web" / "backend",
        scripts_root,
...
def require_project_root(project: str, projects_root: Path) -> Path:
...
def configured_max_read_papers(
    name: str = '',
    cfg: dict[str, Any] | None = None,
    *,
    explicit: Any = None,
    default: int = DEFAULT_MAX_READ_PAPERS,
) -> int:
    """Resolve the per-project Read input cap.

    ``max_read_papers`` is intentionally a Framework/project setting.  The
    Reading module receives only the resolved integer and a generic paper
    input, so it never needs to inspect project or Find state.
    """
    ...
    for payload in payloads:
        value = _positive_config_int(payload.get('max_read_papers'))
        if value:
            return value
    fallback = _positive_config_int(default)
    return fallback or DEFAULT_MAX_READ_PAPERS
