...
def main() -> None:
    ...
    if not args.skip_taste:
        taste_cmd = [
            sys.executable, 'framework/scripts/run_frontend.py',
            '--project', args.project,
        ...
        research_commands.append(([sys.executable, 'framework/scripts/sync_outputs.py', '--project', args.project, '--allow-empty'], min(180, args.command_timeout_sec)))
    research_cmd = [sys.executable, 'framework/scripts/run_autoscientist_supervisor.py', '--project', args.project, '--venue', args.venue, '--cycles', '1', '--fast-cycle', '--timeout-sec', str(args.cycle_timeout_sec), '--command-timeout-sec', str(args.command_timeout_sec), '--audit-limit', '3', '--stop-on-blocker']
    ...
    if active_blocked(paths):
        ...
        append_phase(record, paths, 'code_execute', 'coder_debugger', [
            ([sys.executable, 'framework/scripts/run_autonomous_research.py', '--project', args.project, '--venue', args.venue, '--iterations', '1', '--execute-plan', '--max-launches', '2'], args.cycle_timeout_sec),
        ], env)
