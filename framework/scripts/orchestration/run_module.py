...
from bridges.reading_bridge import (  # noqa: E402
    extract_last_json_object,
    prepare_current_find_read_input,
    sync_current_find_read_outputs,
)
from bridges.ideation_bridge import (  # noqa: E402
...
from bridges.planning_bridge import (  # noqa: E402
...
from bridges.environment_bridge import (  # noqa: E402
...
from runtime.resource_locks import crawl_resource_lease  # noqa: E402
from runtime.taste_pythonpath import resolve_script_path  # noqa: E402


def _python_env() -> dict[str, str]:
    env = os.environ.copy()
    entries = [str(ROOT / "framework"), str(SCRIPTS), str(ROOT / "web" / "backend"), str(ROOT)]
    for stage in STAGES:
...
def _ensure_framework_imports() -> None:
    entries = [str(ROOT), str(ROOT / "framework"), str(SCRIPTS), str(ROOT / "web" / "backend")]
...
def _load_framework_script_module(relative_path: str, module_name: str):
    ...
    if not module_path.is_file():
        module_path = resolve_script_path(relative_path, ROOT).resolve(strict=False)
    try:
...
def _run_current_find_read_bridge(action: str, rest: Sequence[str]) -> int:
    ...
    if not project:
        ...
        return 2
    requested_limit = _arg_value(args, "--read-limit") or _arg_value(args, "--max-papers")
    read_limit = configured_max_read_papers(project, explicit=requested_limit)
    if not _has_arg(args, "--input-json"):
        try:
            prepared = prepare_current_find_read_input(project, read_limit=read_limit)
    ...
    args = _without_arg(args, "--max-papers")
    args.extend(["--max-papers", str(read_limit)])
