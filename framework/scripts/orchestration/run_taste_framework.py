...
from bridges.environment_bridge import prepare_environment_invocation, sync_environment_outputs
from contracts.module_catalog import STAGE_ORDER, contracts_payload, load_all_contracts
from orchestration.workflow_decision import Decision, choose_decision
from reporting.workflow_status import render_markdown
from runtime.framework_io import append_text, read_json, write_json
from runtime.workflow_context import FrameworkContext
from runtime.workflow_executor import run_module
from runtime.workflow_state import WorkflowState, record_from_result, save_state
...
def run_workflow(args: argparse.Namespace) -> int:
    ...
    for _step in range(max(1, args.max_steps)):
        ...
        if decision.stage == "environment" and not (ctx.mode == "dry-run" and not state.project):
