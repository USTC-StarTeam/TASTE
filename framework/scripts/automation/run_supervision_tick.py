...
from bridges.reading_bridge import reading_validation_is_ready
from policies.current_find_route import current_find_run_id, repo_path_from_mapping, selected_title_in_current_find
from runtime.agent_state import append_agent_log
from project.project_config import project_source_selection, project_target_venue, update_project_settings
from project.project_paths import ROOT, build_paths
from runtime.framework_io import read_json as load_json
from runtime.framework_io import positive_int as _int_value
from runtime.framework_io import utc_now as now_iso
from runtime.framework_io import write_json_raw as save_json
from runtime.processes import command_text
from runtime.taste_pythonpath import resolve_script_path
from reporting.work_status import append_supervision_status
...
def audit_current_find_claude_state(paths, run_id: str) -> dict[str, Any]:
    ...
    if isinstance(validation, dict) and validation.get('valid') is True and not reading_validation_is_ready(validation, run_id):
        ...
        payload = {
            ...
            'policy': 'A Read-stage packet validation is usable only when policy_version=full_text_required_v5_detailed_deep_read and the reading packet has title/author-verified full-text/PDF/page evidence or eligible same-run replacements plus Chinese deep-read synthesis. Older or metadata-only validations are audit history only.',
        }
...
def current_impl_repo_path(paths) -> str:
    viability = selected_base_viability_current_selection(paths, current_find_run_id(paths))
...
def refresh_gates(project: str, venue: str) -> list[dict[str, Any]]:
    py = sys.executable
    return [
        run_step('audit_reference_reproduction', [py, str(ROOT / 'framework' / 'scripts' / 'main.py'), 'module', 'experimenting', '--action', 'reference_reproduction', '--project', project, '--venue', venue], timeout=180),
        ...
        run_step('audit_framework_content_coupling', [py, str(resolve_script_path('audit_framework_content_coupling.py', ROOT)), '--project', project], timeout=180),
        run_step('audit_obsolete_baseline_cleanup', [py, str(ROOT / 'framework' / 'scripts' / 'main.py'), 'module', 'environment', '--action', 'chat', '--project', project, '--message', 'Must only audit project-local obsolete Environment baseline candidates against the current selected route. Must record protected paths, eligible paths, and exact evidence in the project before reporting completion.'], timeout=None),
    ]
...
def launch_project_full_cycle(paths, project: str, venue: str) -> dict[str, Any] | None:
    ...
    cmd = [sys.executable, str(resolve_script_path('run_full_research_cycle.py', ROOT)), '--project', project, '--venue', venue]
...
def generic_supervision_tick(project: str, venue: str, *, supervise: bool) -> dict[str, Any]:
    ...
    claude_state = audit_current_find_claude_state(paths, current_find_run_id(paths))
    ...
    if supervise:
        ...
        elif current_find_run_id(paths) and isinstance(find_plan, dict) and str(find_plan.get('run_id') or '') == current_find_run_id(paths):
            steps.extend(refresh_gates(project, venue))
            lit_gate = literature_gate_shortfall(paths)
            action_rc = 0 if all(step.get('return_code') in SAFE_BLOCKED_RC for step in steps) else 2
    ...
    payload = {
        ...
        'action': action,
        'action_rc': action_rc,
        'find_run_id': current_find_run_id(paths),
