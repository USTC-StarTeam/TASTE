...
from runtime.agent_state import append_agent_log, mark_agent, upsert_agent
from runtime.framework_paths import ROOT
from bridges.reading_bridge import current_find_read_gate_status
from project.project_paths import build_paths, management_python
...
def experimenting_cmd(action: str, project: str, venue: str = "") -> list[str]:
    cmd = [sys.executable, str(ROOT / "framework" / "scripts" / "main.py"), "module", "experimenting", "--action", action, "--project", project]
    if venue:
...
def stop_for_current_find_full_text_gate(args, paths, agent_id: str) -> None:
    ...
    gate = current_find_read_gate_status(paths)
...
def stop_for_selected_base_switch_gate(args, paths, agent_id: str, *, stage: str) -> None:
    ...
    state_path.write_text(json.dumps({
        ...
        "policy": "Autonomous research must not run candidate/alternative main-route experiments or claim repair while selected_base_viability_gate.decision=base_switch_gate_required and base_switch_gate is not authorized. Current selected-base evidence repair may continue via framework/scripts/main.py module experimenting --action launch --route-scope selected_base_current_route; bounded candidate base-switch evidence collection may use --route-scope base_switch_evidence_collection and cannot promote claims or switch active route.",
    }, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    if isinstance(reference_gate, dict) and reference_gate and not gate_passed(reference_gate, decision="continue_base"):
        if reference_gate.get("decision") == "switch_base":
            ...
            env_cmd = [sys.executable, str(ROOT / "framework" / "scripts" / "main.py"), "module", "environment", "--action", "deploy_from_plan", "--project", args.project]
