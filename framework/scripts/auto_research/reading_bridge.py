...
def _recommendation_rows(payload: dict[str, Any]) -> list[dict[str, Any]]:
    for key in ("strong_recommendations", "recommendations", "read_candidates", "articles", "input_articles", "papers"):
...
def prepare_current_find_read_input(
    ...
    *,
    ...
    if not recommendations:
        raise ValueError("Current Find results contain no recommended papers.")
    ...
    payload = {
        ...
        "policy": "Framework converts current Find rows to generic Reading input. A title is sufficient; supplied URL, PDF, DOI, arXiv, OpenReview, venue, presentation type, author, and metadata fields are optional same-paper hints.",
    }
...
def _project_packet_entry(
    ...
    *,
    ...
    packet.update({
        ...
        "reading_packet_role": "original_recommendation",
    })
    return packet
...
def _build_validation(
    *,
    ...
    read_quality_complete = bool(expected_count and len(readings) == expected_count and len(full_text_rows) == expected_count and len(deep_rows) == expected_count)
    warnings: list[str] = []
    if limited or len(readings) < expected_count:
    ...
    return {
        ...
        "recommended_reading_count": expected_count,
    ...
    *,
    ...
    readings = [
        _project_reading_row(
            item,
            source_rows[index - 1] if index <= len(source_rows) else {},
    ...
    expected_count = max(len(source_rows), _as_int(input_payload.get("total_current_find_recommendation_count"), len(source_rows)))
    ...
    read_payload = {
        ...
        "recommendation_count": expected_count,
        "processed_recommendation_count": len(source_rows),
        "readings": readings,
    ...
    plan_payload = {
        ...
        "recommended_count": expected_count,
