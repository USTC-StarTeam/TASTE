...
from agent_state import append_agent_log, list_agents, refresh_process_flags, upsert_agent
...
def _module_cmd(py: str, stage: str, action: str = "", *args: str) -> list[str]:
    cmd = [py, str(FRAMEWORK_SCRIPTS / "run_module.py"), stage]
    if action:
...
def _project_conda_env(project: str, root: Path, cfg: dict[str, Any], payload: dict[str, Any]) -> str:
    ...
    if env_name:
        return env_name
    configured = str(cfg.get("conda_env") or "").strip()
    if configured:
        return configured
    handoff = _project_environment_handoff(root)
...
def _is_current_find_read_worker_cmd(cmd: str) -> bool:
    ...
    return "framework/scripts/run-module.py" in lowered and "current-find-research-plan" in lowered
...
def _latest_claude_receipt(root: Path, session: Any, last_result: Any, *, include_live: bool = True) -> dict[str, Any]:
    ...
    explicit_response = str(last_result.get("response_markdown") or "").strip() if last_result.get("web_visible_response") is True else ""
    stdout_text = str(last_result.get("stdout") or "").strip()
    if explicit_response:
        raw_reply = _redact_secrets(explicit_response)
        response_source = str(last_result.get("response_source") or "response_markdown")
    elif clean_result:
    ...
    receipt = {
        ...
        "web_visible_response": last_result.get("web_visible_response") is True,
        "kind": last_result.get("kind", ""),
        "controller_turn": last_result.get("controller_turn", 0),
    }
...
def _human_supervision_summary(root: Path, compact: dict[str, Any], raw_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    reference_smoke_script = f"framework/scripts/run_module.py environment --action deploy_from_plan --project {project_id}"
...
def _current_find_pipeline_public_blocker(pipeline: dict[str, Any]) -> dict[str, Any]:
    ...
    expected = _as_int(pipeline.get("recommended_reading_count") or pipeline.get("expected_recommendation_count") or recommended, recommended)
...
def _lightweight_project_summary(project: str, root: Path, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    source_status_totals = _venue_metadata_counts(venue_rows or source_status or verified_venue_rows)
    source_metadata_count_keys = {'metadata_complete_venue_count', 'metadata_venue_count', 'venues_without_official_categories'}
    source_fallback_count_keys = {
        'raw_title_index_papers',
        'venue_total_papers_available',
        'venue_corpus_audited_papers',
        'venue_category_selected_papers',
    ...
    has_find_summary_funnel_counts = any(
        find_summary.get(key) not in (None, '', 0)
        for key in pipeline_stage_count_keys | {'raw_title_index_papers', 'venue_total_papers_available', 'venue_corpus_audited_papers'}
    )
    for key, value in source_status_totals.items():
        if value in (None, ''):
            continue
        if key in source_metadata_count_keys or (not has_find_summary_funnel_counts and key in source_fallback_count_keys):
            find_summary[key] = value
    if source_status_totals:
        find_summary['source_status_totals'] = source_status_totals
    ...
    current_find_route_ready = bool(
        current_find_ready_for_environment
        or pipeline_contract.get('content_ready')
        or pipeline_contract.get('read_idea_plan_ready')
    ...
    if status == 'running' and pid and full_job_live:
        ...
        summary = str(env_bootstrap_blocker.get('summary') or 'Environment handoff 未通过；等待 Environment 主控 Claude 基于本机回执继续修复。')
    elif status == 'blocked_fresh_base_data_required':
        summary = f'环境阶段已选择当前候选基底：{base_title}；但真实数据/loader 尚未通过，不能进入实验或论文证据。'
    elif status == 'blocked_fresh_base_reference_probe_required':
        protocol_blocker_summary = _reference_protocol_probe_blocker_summary(protocol_probe, base_title)
    ...
    blocker_title = '当前项目门控状态' if env.get('valid') else '等待环境阶段验证并锁定候选基底'
    submission_blockers = _current_submission_blockers(root)
    base_switch_gate_required = bool(selected_base_viability.get('status') == 'blocked' and selected_base_viability.get('decision') == 'base_switch_gate_required')
    base_switch_authorized = bool(
        base_switch_gate.get('status') == 'pass'
    ...
    if fresh_find_running:
        ...
        next_action = selected_base_viability_public.get('next_action') or '等待 Experimenting 主控 Claude 读取当前缺口证据，并给出下一轮实验或修复动作。'
    elif selected_base_viability_blocked:
        blocker_category = selected_base_blocker_category
        blocker_title = selected_base_viability_public.get('title') or '缺少当前主线候选实验证据'
        blocker_summary = selected_base_viability_public.get('summary') or '参考复现已通过；当前主线还缺少可审计、可写入论文的候选实验结果；论文预览可以生成，但不能被标记为投稿通过。'
        next_action = selected_base_viability_public.get('next_action') or '等待 Experimenting 主控 Claude 读取候选实验证据缺口，并给出下一步实验动作。'
    elif reference_full_job_live:
        ...
        blocker_category = 'environment_handoff_not_ready'
        blocker_title = str(env_bootstrap_blocker.get('title') or 'Environment handoff 未通过')
        blocker_summary = str(env_bootstrap_blocker.get('summary') or 'Environment handoff 未通过。')
        next_action = str(env_bootstrap_blocker.get('next_action') or '重新运行环境阶段，让 Environment 主控 Claude 基于本机回执继续修复。')
    elif status == 'blocked_fresh_base_reference_probe_required':
        blocker_category = 'fresh_base_reference_probe_required'
        protocol_blocker_summary = _reference_protocol_probe_blocker_summary(protocol_probe, base_title)
    ...
    if waiting_environment_base_selection and not env_bootstrap_blocker and not current_find_pipeline_blocker:
        blocker_category = 'environment_anchor_selection_required'
        blocker_title = '等待环境阶段验证并锁定候选基底'
        blocker_summary = '当前 Find/Read/Idea/Plan 已准备；必须由 Environment 主控 Claude 基于当前 run 选择基底，不能使用旧 active_repo。'
        next_action = 'Environment 主控 Claude 需要审计当前推荐文章和 Read/Idea/Plan 提出的候选 repo/base，并验证 repo/data/protocol 后锁定候选基底。'
    elif not env.get('valid') and not env_bootstrap_blocker and current_find_route_ready and not current_find_pipeline_blocker:
        blocker_category = blocker_category or 'environment_anchor_selection_required'
        blocker_title = '等待环境阶段验证并锁定候选基底'
        blocker_summary = blocker_summary or '当前 Find/Read/Idea/Plan 已准备；必须由 Environment 主控 Claude 基于当前 run 选择基底，不能使用旧 active_repo。'
        next_action = 'Environment 主控 Claude 需要审计当前推荐文章和 Read/Idea/Plan 提出的候选 repo/base，并验证 repo/data/protocol 后锁定候选基底。'
    ...
    main_route = {
        ...
        'base_selection_status': str(env_bootstrap_blocker.get('status')) if env_bootstrap_blocker else current_find_pipeline_status if current_find_pipeline_blocker else 'waiting_for_current_find_results' if fresh_find_running else 'selected' if env.get('valid') else 'waiting_for_environment_claude_code',
        'selection_stage': _public_internal_names(env.get('selection_stage', '')),
        'selection_gate': _public_internal_names(env.get('selection_gate', '')),
        'readings': read_count,
        'read_artifacts': raw_read_count,
    ...
    artifacts = [item for item in [
        ...
        artifact('plan.md', root / 'planning' / 'finding' / 'plan.md', 'markdown'),
...
def action_gate_blocker(payload: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "allowed_actions": [
            f"{management_python()} framework/scripts/run_module.py finding --action tool_packet --project {project}",
            f"{management_python()} framework/scripts/run_module.py finding --action literature_tool --project {project} --query \"<targeted literature gap query>\" --fast-mode --publish-current-find",
            f"{management_python()} framework/scripts/run_module.py planning --action blocker_action --project {project}",
        ],
...
def build_command(payload: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    if action == "healthcheck":
        ...
        cmd = [py, str(ROOT / "framework" / "scripts" / "run_module.py"), "planning", "--action", "select", "--project", project]
