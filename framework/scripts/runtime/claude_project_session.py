...
from runtime.agent_state import append_agent_log, consume_guidance, mark_agent, upsert_agent
from runtime.runtime_env import find_binary, interactive_env
...
from project.project_paths import CLAUDE_SKILL_ROOT, ROOT, build_paths, load_project_config, management_python, project_experiment_python_from_config
...
CURRENT_FIND_SELECTION_ONLY_POLICY = (
    "current-Find plan selection must run through framework/scripts/main.py module planning --action select; "
...
DIRECT_CONDA_COMMAND_POLICY = (
    ...
    "Use the configured project experiment Python for probes and `framework/scripts/main.py module experimenting --action launch` for experiments; "
...
DIRECT_ARTIFACT_SCRIPT_EXECUTION_POLICY = (
    ...
    "Evidence repair that runs code must use a wrapper-managed module action (`framework/scripts/main.py module experimenting --action launch`) so TASTE owns PID, logs, artifacts, and import policy."
)
...
def command_uses_experiment_launcher(tokens: list[str]) -> bool:
    ...
    if any('launch_experiment_run.py' in token for token in lowered):
        return True
    main_entry = any(token.endswith('framework/scripts/main.py') for token in lowered) and 'module' in lowered
    private_entry = any(token.endswith('framework/scripts/orchestration/run_module.py') for token in lowered)
    if not (main_entry or private_entry):
        return False
...
def bash_command_tool_policy_issue(command: str, project: str, stage: str = '') -> str:
    ...
    nested_environment_stage = (
        re.search(r"framework/scripts/main\.py\s+module\s+environment\b", lowered)
        or re.search(r"framework/scripts/orchestration/run_module\.py\s+environment\b", lowered)
...
def build_context(project: str, instruction: str, stage: str, repo_path: str = '', agent_id: str = 'main') -> str:
    ...
    if current_find_context:
        ...
        } if isinstance(literature_packet, dict) and literature_packet else 'not built yet; read planning/finding/find_results.json and run ' + str(ROOT / 'framework/scripts/main.py') + ' module reading --action current_find_research_plan --project ' + project + ' after a real Find refresh'
    ...
    launcher_template = (
        f"{management_python()} {ROOT / 'framework/scripts/main.py'} module experimenting --action launch --project {project} "
...
- Experiment execution, validation, metrics, and records belong to the project-unique Experimenting module controller. Delegate them through `{management_python()} {ROOT / 'framework/scripts/main.py'} module experimenting --action work --project {project}` and consume its project artifacts afterward.
...
- If a repo training script cannot accept an artifact path or unbuffered logging, write a small repo-local wrapper once, then launch that wrapper through `{ROOT / 'framework/scripts/main.py'} module experimenting --action launch`; do not create ad-hoc background shell jobs.
...
- Use TASTE's native research-direction, evolutionary-memory, evidence-assurance, trajectory-optimization, and paper-production contracts. If the method stack is absent or stale, run `{ROOT / 'framework/scripts/main.py'} module writing --action sync_stack --project {project}` first.
...
- If the packet is missing, stale, too generic, or does not cover the current blocker, refresh the configured Find route with `{management_python()} {ROOT / 'framework/scripts/main.py'} find --project {project} --deep-survey --query "<targeted research query>"`, then rebuild downstream artifacts with `{management_python()} {ROOT / 'framework/scripts/main.py'} module reading --action current_find_research_plan --project {project}`. Do not call deleted Finding literature-tool/tool-packet actions.
...
def run_claude(project: str, instruction: str, stage: str, timeout_sec: int, resume: bool = True, agent_id: str = 'main', repo_path: str = '') -> dict[str, Any]:
    ...
    def inspect_tool_policy(name: str, tool_input: Any) -> None:
        ...
        if is_current_find_gate_state_policy_reason(reason):
            ...
            ) if policy_type == 'current_find_file_write_whitelist' else 'Claude Code may not bypass TASTE control wrappers. New experiment launches must use `framework/scripts/main.py module experimenting --action launch` with the project experiment Python after `--`.'
    ...
    if tool_policy_tripped:
        ...
        if 'trajectory supervisor recursion' in str(tool_policy_report.get('reason') or ''):
            ...
            result['stdout'] = (str(result.get('stdout') or '') + '\n\n[tool policy guard] Blocked unsafe current-Find artifact writing. This is recoverable: repair Read fragments or Ideas through their Framework routes. Planning candidates, plan.md, and selection must use framework/scripts/main.py module planning; project Claude sessions may not write them directly.').strip()[-20000:]
            ...
            result['stdout'] = (str(result.get('stdout') or '') + '\n\n[tool policy guard] Blocked naked experiment launch; relaunch through framework/scripts/main.py module experimenting --action launch.').strip()[-20000:]
