...
def taste_script_search_dirs(root: Path | str | None = None) -> list[Path]:
    repo = resolve_repo_root(root)
    dirs: list[Path] = []
...
def taste_script_import_dirs(root: Path | str | None = None) -> list[Path]:
    repo = resolve_repo_root(root)
    framework_scripts = repo / "framework" / "scripts"
    dirs: list[Path] = [framework_scripts]
    for directory in taste_script_dirs(repo)[1:]:
        dirs.append(directory)
        if directory.is_dir():
            dirs.extend(
                path
...
def resolve_script_path(name: str, root: Path | str | None = None) -> Path:
    ...
    for directory in taste_script_search_dirs(root):
...
class ScriptPathResolver:
    ...
    def glob(self, pattern: str):
        for directory in taste_script_search_dirs(self.root):
