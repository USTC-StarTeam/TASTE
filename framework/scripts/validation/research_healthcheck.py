...
from policies.pipeline_guard import current_environment_selection as guard_current_environment_selection
from reporting.paper_state import active_paper_state
from reporting.report_status import (
    _as_dict,
    _public_summary,
    _selected_base_viability_public_status,
    _state_run_id,
    find_claude,
    load_json,
)
...
def main() -> None:
    ...
    if not llm_ready:
        ...
        notes.append("Run framework/scripts/validation/check_llm_ready.py --project <project> after setting LLM API configuration.")
    ...
    if not taste_state and not current_find_plan:
        issues.append("The workflow has not been run for this project; run framework/scripts/main.py find")
    else:
    ...
    paper_state = active_paper_state(paths.root, args.project, cfg, venue=args.venue)
    if not paper_state:
