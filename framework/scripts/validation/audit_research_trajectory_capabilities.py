...
from project.project_paths import CLAUDE_SKILL_ROOT, ROOT, build_paths
...
from validation.checks import check_result as check
...
def audit_end_to_end_verification(paths) -> dict[str, Any]:
    ...
    checks = [
        ...
        check("framework_exposes_end_to_end_verification", source_contains("bridges/project_bridge.py", ["research_trajectory_end_to_end_verification", "end_to_end_verification_status"]), evidence=[str(ROOT / "framework" / "scripts" / "bridges" / "project_bridge.py")]),
    ]
    return {
...
def audit_third_party_research_stack(paths) -> dict[str, Any]:
    ...
    checks = [
        ...
        check("third_party_framework_exposed", source_contains("bridges/project_bridge.py", ["third_party_research_stack", "third_party_stack_status"]), evidence=[str(ROOT / "framework" / "scripts" / "bridges" / "project_bridge.py")]),
    ]
    return {
        ...
        "evidence_files": [str(stack_path), str(report_path), str(ROOT / "framework" / "scripts" / "main.py")],
