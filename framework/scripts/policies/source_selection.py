...
from runtime.framework_paths import CONFIG_PATH
...
def _workspace_root() -> Path:
    ...
    return Path(os.environ.get("WORKSPACE_ROOT") or ROOT).expanduser()
...
def enabled_sources_for(selection: dict[str, Any]) -> list[str]:
...
def save_canonical_source_selection(selection: Any, config_path: Path = CONFIG_PATH, project_config_path: Path | None = None) -> dict[str, Any]:
    ...
    if project_path:
        ...
        discovery["enabled_sources"] = enabled_sources_for(normalized)
