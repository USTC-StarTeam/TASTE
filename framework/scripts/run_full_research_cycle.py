...
def current_find_plan_bridge_gate_status(paths, bridge_summary: Any | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "next_action": "Repair the Read-stage full-text packet and rerun framework/scripts/run_module.py reading --action current_find_research_plan --project <project>; do not continue environment, experiment, paper, or Claude repair stages with stale or blocked plan artifacts.",
    }
...
6. Re-run `{management_python()} framework/scripts/run_module.py environment --action fresh_base_plan --project {self.args.project}`, `{management_python()} framework/scripts/run_module.py experimenting --action reference_reproduction --project {self.args.project} --venue {self.args.venue}`, and `{management_python()} framework/scripts/run_module.py planning --action blocker_action --project {self.args.project} --venue {self.args.venue}` after changes.
        ...
        blockers = [
            {
                ...
                "next_action": plan_gate.get("next_action") or "Repair the current-Find bridge and rerun framework/scripts/run_module.py reading --action current_find_research_plan --project <project>.",
            }
        ]
        blocker_action_plan = read_json(self.paths.state / "blocker_action_plan.json", {})
        gate["blocker_action_plan"] = {
        ...
        blockers = [
            {
                ...
                "next_action": "Rerun framework/scripts/run_module.py reading --action current_find_research_plan --project <project> so the main Claude Code compares the five plans from full readings and writes exactly one selected_for_execution/execute_next plan; all other plans must remain backlog.",
            }
        ]
        blocker_action_plan = read_json(self.paths.state / "blocker_action_plan.json", {})
        gate["blocker_action_plan"] = {
