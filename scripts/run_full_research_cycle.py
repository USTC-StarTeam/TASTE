...
def current_find_full_text_gate_status(paths) -> dict[str, Any]:
    ...
    if expected and (not validation_ready):
        if validation_run and run_id and validation_run != run_id:
            blockers.append("current Find full-text validation is stale for this run_id")
        if not blockers:
            blockers.append("current Find recommended readings have not all passed full-text/PDF/page evidence validation")
        return {
            ...
            "next_action": "Acquire or prove the missing full-text/PDF/HTML/page evidence for the current user-visible recommendation, or use the Find/recommendation acceptance path to replace recommendations that cannot be fully read; do not rerun Claude over the same missing evidence.",
        }
    return {
...
class FullCycle:
    ...
    def run(self, cmd: list[str], *, stage: str, required: bool = False, timeout: int | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if "--skip-discovery" in {str(part) for part in cmd}:
                ...
                # still be able to launch scripts/run_literature_tool.py unless a
    ...
    def authoritative_gate_context(self, stage: str) -> str:
        ...
        hard_lines = [
            ...
            "P0: Claude may repair a literature shortfall only through scripts/run_literature_tool.py. That wrapper may launch a controlled targeted Find unless DISABLE_NEW_FIND=1/--record-only is explicitly set; do not call raw finding-module commands or run duplicate concurrent Finds.",
...
- If the current packet is stale, empty, or not specific enough for the current blocker, run `{management_python()} scripts/run_literature_tool.py --project {self.args.project} --query "<targeted query>" --fast-mode --venue {self.args.venue}`; use `--deep-survey` only when broad venue/arXiv coverage is necessary.
...
- If `selected_base_viability_gate.decision` is `base_switch_gate_required`, treat candidate/alternative main-route launches, paper/claim promotion, and route execution as blocked until deterministic base-switch evidence passes. Current selected-base evidence repair may use `scripts/launch_experiment_run.py --route-scope selected_base_current_route`; bounded candidate gate evidence collection may use `--route-scope base_switch_evidence_collection`. Keep alternatives as proposals until the dedicated gate passes, then execute the switch only through `scripts/execute_authorized_base_switch.py`.
...
5. Do not run raw `gdown`, `curl`, or `wget` yourself. Data acquisition must go through `{management_python()} scripts/probe_fresh_base_data_acquisition.py --project {self.args.project} --attempt-download --timeout-sec 45`; if it fails or times out, record the exact blocker and stop before training.
6. Re-run `{management_python()} scripts/build_fresh_base_implementation_plan.py --project {self.args.project}`, `{management_python()} scripts/audit_reference_reproduction.py --project {self.args.project} --venue {self.args.venue}`, and `{management_python()} scripts/build_blocker_action_plan.py --project {self.args.project} --venue {self.args.venue}` after changes.
...
- If `selected_base_viability_gate.decision=base_switch_gate_required`, stop candidate/alternative main-route training and paper/claim promotion until deterministic gate evidence passes. Current selected-base evidence repair may continue via `scripts/launch_experiment_run.py --route-scope selected_base_current_route`; bounded candidate gate evidence collection may use `--route-scope base_switch_evidence_collection`. If the dedicated gate passes, execute route identity changes only through `scripts/execute_authorized_base_switch.py`; evidence still remains non-promotable until downstream audits pass.
...
- For every new training launch, use `{management_python()} scripts/launch_experiment_run.py --project {self.args.project} --artifact-name <unique_slug> --cwd <project_or_repo_dir> -- <project-experiment-python> -u <training_script.py> ...` so TASTE owns the PID, lock, artifact contract, project experiment Python, and stdout/stderr log.
...
""".strip()
    ...
    def finish_current_find_full_text_gate_block(self, cycle: dict[str, Any], pdf_before: dict[str, Any], full_text_gate: dict[str, Any]) -> dict[str, Any]:
        ...
        gate["paper_pipeline_skipped_reason"] = "Current Find Read gate blocks base selection, experiments, paper writing, citation repair, figure repair, and claim promotion until every user-visible recommendation has full-text evidence"
        blockers = [
            {
                "category": "current_find_full_text_reading_gate",
                "severity": "block",
                "issue": (
                    f"Current Find recommended papers are not fully read: "
                ...
                "next_action": full_text_gate.get("next_action") or "Acquire or prove the missing current-Find full-text/PDF/HTML/page evidence, or replace the unreadable recommendation through the Find/recommendation acceptance path.",
            }
        ]
        blocker_action_plan = read_json(self.paths.state / "blocker_action_plan.json", {})
        ...
        self.state["continuation_reason"] = "current Find full-text deep reading is incomplete because one or more recommendations lack full-text evidence; acquire evidence or repair the recommendation packet before any Claude rewrite"
        self.state["current_goal"] = (
            f"complete current Find full-text reading gate "
        ...
        blockers = [
            {
                ...
                "next_action": "Rerun scripts/ensure_current_find_research_plan.py --project <project> so the main Claude Code compares the five plans from full readings and writes exactly one selected_for_execution/execute_next plan; all other plans must remain backlog.",
            }
        ]
        blocker_action_plan = read_json(self.paths.state / "blocker_action_plan.json", {})
        gate["blocker_action_plan"] = {
        ...
        blockers = [
            {
                ...
                "next_action": (
                    ...
                    "Run scripts/audit_deterministic_base_switch_gate.py and require state/base_switch_gate.json to pass before any switch."
                ),
                "human_summary": (
            ...
            blockers.append(
                {
                    ...
                    "next_action": "Run scripts/audit_deterministic_base_switch_gate.py before any route switch.",
                }
            )
