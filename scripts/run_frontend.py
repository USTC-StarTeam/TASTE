...
def _load_backup_articles(limit):
    ...
    runs_root = taste_root / "auto_research" / "runs"
...
def latest_taste_run_hint() -> dict:
    runs = sorted((WORKSPACE_ROOT / "auto_research" / "runs").glob("find_*"), key=lambda p: p.stat().st_mtime if p.exists() else 0, reverse=True)
    if not runs:
        return {}
    latest = runs[0]
