...
def build_prompt(agent_name: str, args: argparse.Namespace, repo: Path, trial_context: str) -> str:
    ...
    return (
        ...
        f"If the packet is missing or unrelated, run `{management_python()} scripts/run_literature_tool.py --project {args.project} --query \"<targeted query>\" --fast-mode` from the module root, then rebuild the packet. "
