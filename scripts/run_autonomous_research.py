...
def stop_for_current_find_full_text_gate(args, paths, agent_id: str) -> None:
    ...
    state_path.write_text(json.dumps({
        ...
        "gate": gate,
        "policy": "Standalone autonomous research must not enter reference repair, experiment iteration, trajectory, paper writing, or claim repair until every current user-visible Find recommendation has both full-text evidence and non-placeholder deep-read synthesis.",
    }, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    mark_agent(args.project, agent_id, "blocked", current_step="current Find full-text deep reading gate blocked autonomous research before experiments")
...
def stop_for_selected_base_switch_gate(args, paths, agent_id: str, *, stage: str) -> None:
    ...
    state_path.write_text(json.dumps({
        ...
        "policy": "Autonomous research must not run candidate/alternative main-route experiments or claim repair while selected_base_viability_gate.decision=base_switch_gate_required and base_switch_gate is not authorized. Current selected-base evidence repair may continue via launch_experiment_run.py --route-scope selected_base_current_route; bounded candidate base-switch evidence collection may use --route-scope base_switch_evidence_collection and cannot promote claims or switch active route.",
    }, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
