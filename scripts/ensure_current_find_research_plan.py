...
def claude_output_payloads_or_files_are_current(payloads: list[Any], file_paths: list[Path], current_revision: dt.datetime | None) -> bool:
    ...
    for payload, file_path in zip(payloads, file_paths):
        ...
        use_mtime = generated is None or bool(re.fullmatch(r"\d{4}-\d{2}-\d{2}", generated_raw))
        candidates: list[dt.datetime] = []
...
def _normalize_inspired_refs(value: Any, fallback: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for item in as_list(value):
        ref = _public_ref_from_source(item)
        if ref:
            rows.append(ref)
    if not rows:
...
def _current_find_evidence_preflight_validation(run_id: str, report: dict[str, Any], *, status: str, blockers: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "pending_full_text_reading_count": missing_count if missing_count else expected,
        "full_text_evidence_titles": evidence_titles[:20],
        "pending_deep_read_synthesis_titles": [] if missing_count else evidence_titles[:20],
        "pending_without_evidence_titles": missing_titles[:20],
        "pending_full_text_reading_titles": missing_titles[:20] if missing_count else evidence_titles[:20],
        "blockers": blockers,
...
def ensure_current_find_full_text_evidence_before_claude(project: str, paths: Any, taste_dir: Path, run_id: str, find_results: dict[str, Any]) -> dict[str, Any]:
    ...
    validation = _current_find_evidence_preflight_validation(
        ...
        blockers=["current Find full-text packet is missing or stale; The workflow must acquire same-run PDF/HTML evidence before Claude deep reading"],
    )
    save_json(paths.state / "current_find_claude_reading_validation.json", validation)
    try:
    ...
    if missing_after:
        validation_after = _current_find_evidence_preflight_validation(
            ...
            report_after,
            status="blocked_current_find_full_text_evidence_pending",
            blockers=["current Find full-text evidence still misses at least one user-visible recommendation; The workflow must acquire same-paper publisher, repository, or title/author-verified preprint evidence before Claude deep reading"],
        )
    ...
    validation_after = _current_find_evidence_preflight_validation(
        ...
        report_after,
        status="current_find_full_text_evidence_ready_pending_claude_deep_read",
        blockers=["current Find full-text evidence is ready after repair; Claude Code must now synthesize detailed per-paper deep readings"],
    )
    validation_after["repair_return_code"] = repair_rc
    validation_after["full_text_repair_status"] = str((repair_receipt if isinstance(repair_receipt, dict) else {}).get("status") or "")
    save_json(paths.state / "current_find_claude_reading_validation.json", validation_after)
...
def validate_claude_readings_against_current_find(readings: list[dict[str, Any]], find_results: dict[str, Any], read_limit: int, paths: Any = None, run_id: str = "") -> tuple[bool, dict[str, Any]]:
    ...
    if expected_count and actual_count != expected_count:
        valid = False
        blockers.append("Claude readings count must equal the current user-visible recommendation count")
    if extra_readings:
        valid = False
        blockers.append("readings include non-recommended papers outside the user-visible recommendation pool")
    if duplicate_readings:
        valid = False
        blockers.append("readings contain duplicate current recommendations")
    if invalid_positive:
        valid = False
    ...
    if missing_recommendations:
        valid = False
        blockers.append("not all current user-visible recommendations were read")
    if missing_positive:
        valid = False
    ...
    report = {
        ...
        "blockers": blockers,
        "policy": "Every user-visible current recommendation must have both full-text/PDF/page evidence, auditable Claude Task/subagent delegation, and non-placeholder deep-read synthesis before it counts as completed deep reading. Only strict positive anchors may be labelled positive; weak/boundary recommended papers remain reading, critique, or search-expansion evidence.",
    }
    return valid, report
...
def build_ideas(readings: list[dict[str, Any]], repo: dict[str, Any], fresh_plan: dict[str, Any], cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "supporting_papers": [_paper_ref(row) for row in readings[:8]],
            "claude_code_tasks": ["读取当前 Find 的推荐论文、边界审计和来源状态。", "围绕缺口生成补充检索主题；如推荐数量短缺，通过 scripts/run_literature_tool.py 受控补检索并刷新 packet。", "复核评分/降级原因并刷新 literature gate，禁止用未评分/无摘要论文凑推荐数量。"],
...
def render_idea_md(ideas: list[dict[str, Any]], run_id: str) -> str:
    lines = ["# 当前 Find 驱动 Ideas\n\n", f"- run_id: `{run_id}`\n", f"- ideas: {len(ideas)}\n\n"]
    for idx, idea in enumerate(ideas, 1):
        inspired = _normalize_inspired_refs(idea.get("inspired_by"), as_list(idea.get("supporting_papers") or idea.get("positive_anchor_papers")))
        metadata = [
...
- artifact_parse_failures 表示上一轮写出的 JSON artifact 已损坏，TASTE 无法解析其内容；本轮精读必须改用逐篇分片协议：每篇论文用 Claude Code 文件工具写一个 `planning/finding/current_find_deep_read_fragments/<rank>_<paper_id>.json` 或返修分片，每个分片只含该论文的 reading 对象；ideas.json 和 plans.json 仍必须用 Write 完整重写整个 JSON。不要分段 Edit/MultiEdit 大 JSON；即使只改一个字段，也要用 Write 重写对应完整 JSON artifact。不要写 read.md/idea.md/plan.md，Markdown 由 wrapper 渲染；不得用 Bash/Python/cat/heredoc 批量修写科研 artifact 或分片。
...
- 不准用 Bash/Python/cat/heredoc/json.dump/open(..., 'w') 生成或修补 current-Find artifact；Bash 只能做只读检查，例如 `rg`、`sed -n`、`head`、`tail`、`wc`、`python -m json.tool planning/finding/plans.json >/dev/null`。
...
def _idea_contract_issues(row: dict[str, Any]) -> list[str]:
    ...
    if _contains_stale_execution_binding(row):
    ...
    elif scores["overall"] < IDEA_OBJECTIVE_SCORE_MIN_OVERALL:
...
def _valid_claude_plan(row: dict[str, Any]) -> bool:
    ...
    return has_id and (has_action or has_title)
...
def _reading_quality_score(row: dict[str, Any]) -> tuple[int, int, int, int]:
    if not isinstance(row, dict):
        return (-999, 0, 0, 0)
    ...
    return (-len(gaps), field_len, list_items, evidence)
...
def ensure_claude_plan_state(project: str, paths, run_id: str, readings: list[dict[str, Any]], ideas: list[dict[str, Any]], plans: list[dict[str, Any]], takeover: dict[str, Any], idea_count: int = 5) -> dict[str, Any]:
    ...
    literature_repair_commands = [
        f"python3 scripts/build_literature_tool_packet.py --project {project} --venue {venue}",
        f"python3 scripts/run_literature_tool.py --project {project} --venue {venue} --query \"<targeted literature gap query>\" --fast-mode",
        f"python3 scripts/audit_submission_readiness.py --project {project} --venue {venue}",
        f"python3 scripts/build_blocker_action_plan.py --project {project} --venue {venue}",
    ]
...
def _contains_stale_execution_binding(value: Any) -> bool:
    text = json.dumps(value, ensure_ascii=False, sort_keys=True).lower() if isinstance(value, (dict, list)) else str(value or "").lower()
...
def normalize_claude_outputs_to_current_find_policy(project: str, paths, run_id: str, find_results: dict[str, Any], takeover: dict[str, Any], read_limit: int, idea_count: int) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    ...
    if not validation_ok:
        ...
        blocked_payload = {
            ...
            "policy": "Normalization preserves same-run Claude readings and full-text packet evidence, but must not mark takeover ready unless every current recommendation has both full-text evidence and non-placeholder deep-read synthesis.",
            "full_text_packet": full_text_packet_summary(full_text_packet),
            **selection_fields,
        }
...
def _render_plan_refs(lines: list[str], refs: Any) -> None:
    rows = as_list(refs)[:8]
    if not rows:
        return
    ...
    for ref in rows:
        if isinstance(ref, dict):
            ...
            reason = compact(ref.get("reason") or ref.get("evidence_role") or ref.get("source") or ref.get("venue"), 260)
            ...
            lines.append(f"- {compact(ref, 240)}\n")
    lines.append("\n")
...
def render_plan_md(plans: list[dict[str, Any]], run_id: str) -> str:
    lines = ["# 当前 Find 驱动 Plans\n\n", f"- run_id: `{run_id}`\n", f"- plans: {len(plans)}\n"]
    selected_plan = next((plan for plan in plans if isinstance(plan, dict) and plan.get("selected_for_execution") is True), None)
    if selected_plan:
        lines.extend([
            f"- selected_plan_id: `{selected_plan.get('plan_id') or ''}`\n",
            f"- selected_idea_id: `{selected_plan.get('idea_id') or ''}`\n",
            "- execution_policy: 后续环境、实验、写作和 claim 只能消费该 selected plan；其他 plan 是候选池。\n",
        ])
    lines.append("\n")
    for idx, plan in enumerate(plans, 1):
        ...
        metadata = [
            f"- plan_id: `{plan.get('plan_id')}`\n" if str(plan.get("plan_id") or "").strip() else "",
            f"- idea_id: `{plan.get('idea_id')}`\n" if str(plan.get("idea_id") or "").strip() else "",
        ...
        _render_plan_refs(lines, plan.get("inspired_by") or plan.get("supporting_papers") or plan.get("positive_anchor_papers"))
        if steps:
            lines.append("### 执行步骤\n")
            for step in steps:
...
def build_execution_plan(project: str, cfg: dict[str, Any], run_id: str, readings: list[dict[str, Any]], ideas: list[dict[str, Any]], plans: list[dict[str, Any]], fresh_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    gate_refresh_commands = [
        f"python3 scripts/audit_reference_reproduction.py --project {project} --venue {venue}",
        f"python3 scripts/audit_experiment_iteration.py --project {project}",
        f"python3 scripts/build_research_trajectory_system.py --project {project} --venue {venue}",
        f"python3 scripts/build_blocker_action_plan.py --project {project} --venue {venue}",
    ]
    literature_repair_commands = [
        f"python3 scripts/build_literature_tool_packet.py --project {project} --venue {venue}",
    ...
    data_command = f"python3 scripts/build_fresh_base_implementation_plan.py --project {project}"
    failure_type = ""
    next_required_action = "environment_base_selection_and_repo_data_protocol_audit"
    ...
    return {
        ...
        **selection_fields,
        "guardrails": [
            "Do not launch raw or duplicate Find jobs from this script; use scripts/run_literature_tool.py for controlled targeted literature repair.",
...
def update_literature_packet(paths, run_id: str, readings: list[dict[str, Any]], ideas: list[dict[str, Any]], plans: list[dict[str, Any]]) -> None:
    ...
    section = [
        ...
        f"- selected_idea_id: `{selection_fields.get('selected_idea_id') or ''}`\n",
        f"- selected_plan_id: `{selection_fields.get('selected_plan_id') or ''}`\n",
        ...
        "- 规则：主控 Claude Code 可以生成多个 idea/plan 候选，但后续环境、实验、写作和 claim 只能消费 selected_plan_id；未选中候选只能留作 backlog，不得驱动主线。\n",
...
def update_frontend_state(paths, run_id: str, readings: list[dict[str, Any]], ideas: list[dict[str, Any]], plans: list[dict[str, Any]]) -> None:
    ...
    state.update({
        "taste_root": str(root / "modules" / "taste"),
        "taste_run_id": run_id,
...
def main() -> int:
    ...
    if not args.skip_claude:
        ...
        if existing_readings:
            ...
            # prevents stale or test-run read_results.json from masking the real
            # Claude/subagent progress; the validation payload below remains the
            # gate of record.
            validation_for_artifact = positive_validation if isinstance(positive_validation, dict) else reading_validation
            write_current_find_structured_artifacts(paths, taste_dir, run_id, existing_readings, existing_ideas, existing_plans, takeover, validation_for_artifact)
        ...
        incomplete = {
            "status": "blocked_current_find_full_text_evidence_pending" if failure_type == "full_text_evidence_missing" else "blocked_current_find_idea_contract_failed" if idea_failure else "blocked_ambiguous_selected_plan" if failure_type == "ambiguous_selected_plan" else "blocked_missing_selected_plan" if selection_failure else "blocked_claude_current_find_takeover_incomplete",
            "failure_type": failure_type,
            "next_required_action": next_action,
            "next_required_stage": "acquire_current_find_full_text_evidence" if failure_type == "full_text_evidence_missing" else next_action if (selection_failure or idea_failure) else "repair_current_find_claude_read_idea_plan",
            "base_selection_status": "blocked_by_current_find_full_text_evidence" if failure_type == "full_text_evidence_missing" else "blocked_by_current_find_idea_contract" if idea_failure else "blocked_by_current_find_execution_selection" if selection_failure else "blocked_by_current_find_read_idea_plan_contract",
            "content_ready": bool(len(existing_readings) == min_readings and validation_ok and len(existing_ideas) >= args.idea_count and _ideas_three_part_ready(existing_ideas, args.idea_count) and len(existing_plans) >= args.idea_count and len(targeted_queries) >= 3),
            "read_idea_plan_ready": bool(len(existing_readings) == min_readings and validation_ok and len(existing_ideas) >= args.idea_count and _ideas_three_part_ready(existing_ideas, args.idea_count) and len(existing_plans) >= args.idea_count and len(targeted_queries) >= 3),
            "execution_ready": False,
            "takeover_ready": False,
            "claude_current_find_ready": bool(len(existing_readings) == min_readings and validation_ok and len(existing_ideas) >= args.idea_count and _ideas_three_part_ready(existing_ideas, args.idea_count) and len(existing_plans) >= args.idea_count and len(targeted_queries) >= 3),
            ...
            "claude_takeover": takeover,
            **selection_fields,
            "guardrail": "Claude Code takeover has executed, but TASTE cannot mark current-Find reading complete until every user-visible recommendation has full-text/PDF/HTML evidence." if failure_type == "full_text_evidence_missing" else "Claude Code takeover is mandatory for current Find Read/Idea/Plan; deterministic compatibility templates are disabled unless --skip-claude is explicitly supplied. Claude must read every current user-visible recommendation and record at least three supplemental search topics.",
        }
    ...
    if already_current and not args.force:
        ...
        if downstream_source == CLAUDE_TAKEOVER_SOURCE:
            ...
            content_ready = bool(validation_ready and idea_schema_ready and len(plans) >= args.idea_count and targeted_query_count >= 3)
            ...
            if validation_ready and not idea_schema_ready:
                observed = {
                    "readings": len(readings),
                    "ideas": len(ideas),
                    "plans": len(plans),
                    "raw_artifact_idea_count": len(raw_ideas),
                    "idea_schema_ready": False,
