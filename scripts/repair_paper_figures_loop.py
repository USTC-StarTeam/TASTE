...
Goal: keep iterating until the generated PDF can be shown as an accepted conference preview. The current PDF must remain viewable as a blocked preview, but your job is to repair it so `scripts/audit_paper_figures.py` passes.
...
def main() -> int:
    ...
    for round_index in range(1, args.max_rounds + 1):
        audit = run([sys.executable, str(ROOT / "scripts" / "audit_paper_figures.py"), "--project", args.project, "--venue", venue])
        state = get_active_paper_state(args.project, venue=venue)
        if state.get("paper_figure_quality_ready"):
            preview = run([sys.executable, str(ROOT / "scripts" / "build_conference_preview_paper.py"), "--project", args.project, "--venue", venue, "--title", title])
