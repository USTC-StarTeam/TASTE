...
def main() -> None:
    ...
    if args.project:
        ...
        if not (project_root / 'project.json').exists():
            ...
            run([sys.executable, str(ROOT / 'scripts' / 'init_project.py'), '--project', project])
        run([sys.executable, str(ROOT / 'scripts' / 'detect_machine_profile.py'), '--project', project])
        run([sys.executable, str(ROOT / 'scripts' / 'generate_handoff.py'), '--project', project])
