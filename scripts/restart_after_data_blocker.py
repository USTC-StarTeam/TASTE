...
def main() -> None:
    ...
    for query in queries:
        ...
        if selection.get('include_github'):
            commands.append([sys.executable, 'scripts/discover_github_repos.py', '--project', args.project, '--query', query, '--limit', str(args.limit)])
        if selection.get('include_arxiv'):
            commands.append([sys.executable, 'scripts/discover_arxiv.py', '--project', args.project, '--query', query, '--max-results', '5'])
        if not commands:
    ...
    followups = [
        ([sys.executable, 'scripts/ingest_discovery.py', '--project', args.project, '--limit', str(args.limit)], min(120, args.command_timeout_sec)),
        ([sys.executable, 'scripts/assess_repo_candidates.py', '--project', args.project], min(120, args.command_timeout_sec)),
        ([sys.executable, 'scripts/assess_paper_quality.py', '--project', args.project], min(120, args.command_timeout_sec)),
        ([sys.executable, 'scripts/assess_idea_candidates.py', '--project', args.project], min(120, args.command_timeout_sec)),
        ([sys.executable, 'scripts/audit_repo_candidate_pool.py', '--project', args.project, '--limit', str(max(1, min(3, args.limit)))], max(120, min(360, args.command_timeout_sec * 3))),
        ([sys.executable, 'scripts/reconcile_active_and_pool_candidates.py', '--project', args.project], min(120, args.command_timeout_sec)),
        ([sys.executable, 'scripts/data_unavailability_policy.py', '--project', args.project], min(120, args.command_timeout_sec)),
    ]
