...
def ensure_writing_vendor(venue: str, repo_dir: Path, *, skip_clone: bool = False) -> dict[str, Any]:
    script = ROOT / "scripts" / "sync_writing_vendor.py"
    payload: dict[str, Any] = {"status": "missing_script", "required_ready": False, "warnings": []}
    if not script.exists():
        payload["warnings"].append("scripts/sync_writing_vendor.py is missing; writing vendor references cannot be checked.")
        return payload
    ...
    if result.get("return_code") != 0 and not payload.get("required_ready"):
        ...
        payload["warnings"].append(
            "writing vendor dependencies are missing. Run `python scripts/sync_writing_vendor.py --venue "
            + str(venue or "<venue>")
...
def prepare_workspace(project: str, venue: str, title: str, repo_dir: Path, force: bool = True) -> tuple[Path, dict[str, Any]]:
    ...
    init_script = repo_dir / "skills" / "paper-orchestra" / "scripts" / "init_workspace.py"
    commands: list[dict[str, Any]] = []
...
def ensure_venue_contract(project: str, venue: str, *, refresh_current_venue: bool = False) -> dict[str, Any]:
    ...
    if refresh_current_venue or not requirements:
        cmd = [sys.executable, str(ROOT / "scripts" / "resolve_venue_requirements.py"), "--project", project, "--venue", venue]
    ...
    if not template_ok:
        result = run([sys.executable, str(ROOT / "scripts" / "fetch_latex_template.py"), "--project", project, "--venue", venue], cwd=ROOT, required=False)
        commands.append(result)
        if result.get("return_code") != 0:
            raise RuntimeError(
...
def run_deterministic_gates(project: str, venue: str, workspace: Path, repo_dir: Path, *, min_references: int) -> dict[str, Any]:
    ...
    payload["venue_template_validation"] = venue_validation
    figure_audit_cmd = [sys.executable, str(ROOT / "scripts" / "audit_paper_figures.py"), "--project", project, "--venue", venue]
...
def sync_preview(project: str, venue: str, title: str, workspace: Path, *, force_refresh: bool = False) -> dict[str, Any]:
    ...
    if force_refresh and not workspace_artifacts.get("ready"):
        ...
        return payload
    cmd = [sys.executable, str(ROOT / "scripts" / "build_conference_preview_paper.py"), "--project", project, "--venue", venue, "--title", title]
    result = run(cmd, cwd=ROOT, required=False)
...
def main() -> int:
    ...
    try:
        ...
        if not vendor_status.get("required_ready"):
            raise RuntimeError("writing vendor references are not ready for the requested venue; run scripts/sync_writing_vendor.py for this workspace.")
