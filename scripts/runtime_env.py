...
def interactive_env(project: str | None = None, cfg: dict[str, Any] | None = None) -> dict[str, str]:
    ...
    taste_root = ROOT / "modules" / "taste"
    pythonpath_entries = [str(taste_root), str(ROOT), str(ROOT / "scripts")]
    existing_pythonpath = env.get("PYTHONPATH", "")
    if existing_pythonpath:
        pythonpath_entries.extend(item for item in existing_pythonpath.split(os.pathsep) if item)
    seen_pythonpath: set[str] = set()
    env["PYTHONPATH"] = os.pathsep.join(item for item in pythonpath_entries if not (item in seen_pythonpath or seen_pythonpath.add(item)))
