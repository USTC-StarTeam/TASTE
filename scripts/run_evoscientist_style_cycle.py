...
def main() -> None:
    ...
    append_phase(record, paths, 'intake', 'planner', [
        ([sys.executable, 'scripts/report_status.py', '--project', args.project, '--venue', args.venue], args.command_timeout_sec),
        ([sys.executable, 'scripts/check_llm_ready.py', '--project', args.project] + (['--live'] if args.use_llm else []), min(90, args.command_timeout_sec)),
    ], env)
    append_phase(record, paths, 'plan', 'planner', [
        ([sys.executable, 'scripts/propose_next_actions.py', '--project', args.project], args.command_timeout_sec),
        ([sys.executable, 'scripts/build_stagnation_report.py', '--project', args.project, '--venue', args.venue], args.command_timeout_sec),
    ], env)
    ...
    if not args.skip_taste:
        ...
        research_commands.append(([sys.executable, 'scripts/sync_outputs.py', '--project', args.project, '--allow-empty'], min(180, args.command_timeout_sec)))
    research_cmd = [sys.executable, 'scripts/run_autoscientist_supervisor.py', '--project', args.project, '--venue', args.venue, '--cycles', '1', '--fast-cycle', '--timeout-sec', str(args.cycle_timeout_sec), '--command-timeout-sec', str(args.command_timeout_sec), '--audit-limit', '3', '--stop-on-blocker']
    ...
    append_phase(record, paths, 'evaluate', 'analyst', [
        ([sys.executable, 'scripts/audit_paper_evidence.py', '--project', args.project, '--venue', args.venue], args.command_timeout_sec),
        ([sys.executable, 'scripts/build_blocker_resolution_packet.py', '--project', args.project, '--venue', args.venue], args.command_timeout_sec),
        ([sys.executable, 'scripts/build_stagnation_report.py', '--project', args.project, '--venue', args.venue], args.command_timeout_sec),
    ], env)
    append_phase(record, paths, 'write', 'writer', [
        ([sys.executable, 'scripts/run_paper_pipeline.py', '--project', args.project, '--venue', args.venue, '--title', str(cfg.get('title') or cfg.get('topic') or args.project)], min(180, args.cycle_timeout_sec)),
    ], env)
    append_phase(record, paths, 'verify', 'debugger_analyst', [
        ([sys.executable, 'scripts/audit_pipeline_runnability.py', '--project', args.project, '--venue', args.venue], min(120, args.cycle_timeout_sec)),
        ([sys.executable, 'scripts/research_healthcheck.py', '--project', args.project, '--venue', args.venue], args.command_timeout_sec),
        ([sys.executable, 'scripts/lint_wiki.py', '--project', args.project], args.command_timeout_sec),
        ([sys.executable, 'scripts/report_status.py', '--project', args.project, '--venue', args.venue], args.command_timeout_sec),
    ], env)
