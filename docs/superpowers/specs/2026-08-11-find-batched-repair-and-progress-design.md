...
- Find 的职责是从已经完成真实标题+摘要 LLM 评价的候选中产生配置数量的排序精读清单；Read 才负责全文证据验证。
- 硬资格条件只有：存在真实摘要、存在有效有限数值的最终 LLM 评分、中英文推荐理由通过质量检查、没有最终评分失败标记。
- `topic_evidence_supported`、`matched_topic_route`、`missing_topic_evidence` 和 `recommend_for_deep_reading` 是解释与审计字段，不得在最终 LLM 排序后再形成第二道资格门。
- 评分提示必须区分精确中心、强匹配、可迁移/部分相关、弱相关和无关；最终列表按综合评分排序取 Top-N。只要有效候选不少于目标数，就必须返回目标数量。
- 中文负面判断不得用裸 `无关` 子串匹配；`梯度无关优化` 等正向技术表述不能被误判成“与主题无关”。
- 最终推荐政策变化不得使稳定的标题评分缓存失效；标题缓存身份只由标题提示合同控制，最终标题+摘要缓存则必须随最终提示合同变化而失效。
...
- 具备足够多真实摘要、有效最终分数和合格理由的唯一候选时，必须稳定返回前 `max(网页配置值, 来源数×5)` 篇；该规则对任意合法网页值一致生效，弱主题证据审计状态不减少数量。
- 缺失摘要、最终评分失败或理由质量失败的高分行仍必须排除，不能用于凑数。
