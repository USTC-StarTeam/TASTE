...
## Source Files
- `third_party/ARIS/skills/research-wiki/SKILL.md`
...
- Absorb the source `research-wiki` protocol into the matching native phase.
...
|---------|--------|---------|
| Project direction | full sections | Structured extraction from `BRIEF.md` by `## ` heading (Problem / Constraints / Direction / Background / Non-Goals / Domain Knowledge / Existing Results), in priority order. No per-field char cap — the 8000-char assembly loop is the only safety net. Falls back to a flat 600-char slice if the brief uses no known headings. |
