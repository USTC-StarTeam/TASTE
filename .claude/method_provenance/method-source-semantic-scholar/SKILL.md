...
## Source Files
- `third_party/ARIS/skills/semantic-scholar/SKILL.md`
...
- Absorb the source `semantic-scholar` protocol into the matching native phase.
...
- **S2_FETCHER** — canonical name `semantic_scholfetch.py`, resolved per
...
S2_FETCHER=".aris/tools/semantic_scholfetch.py"
[ -f "$S2_FETCHER" ] || S2_FETCHER="tools/semantic_scholfetch.py"
[ -f "$S2_FETCHER" ] || { [ -n "${ARIS_REPO:-}" ] && S2_FETCHER="$ARIS_REPO/tools/semantic_scholfetch.py"; }
