...
def api_artifacts(run_id: str) -> dict:
    ...
    for name in ["find_results.json", "stage0_profile.json", "venue_health_report.json", "category_scan_report.json", "title_filter_report.json", "arxiv_raw.json", "arxiv_prefiltered.json", "read_results.json", "ideas.json", "plans.json", "config.json", "selection.json", "email_report.json"]:
        path = directory / name
        if path.exists():
