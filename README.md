...
| Find | LLM 评估并发数 | 控制 Find 标题 LLM 批次并发。 | 默认 8；慢速或限流 API 建议 4-8，标题+摘要综合评分使用独立并发预算。 |
...
| Paper | 缺 LaTeX 依赖时尝试自动安装 | 网页会保存该开关，但当前 Writing 运行入口不读取它。 | 生成 PDF 前自行准备 LaTeX 和 `latexmk`。 |
...
左侧栏负责“项目级上下文”和“运行工具”。研究主题、研究兴趣和研究者画像会直接参与 Find、Ideas、Plan 和后续模块主控 Claude 判断，Find 推荐结果再决定 Read 的论文输入；其中研究画像越详细，TASTE 越能区分“真正匹配的论文”和“只是泛泛相关的论文”。网页中的 LLM 配置只供 Find 做关键词抽取、分类排序、标题筛选和标题+摘要评分；后续模块使用 Claude Code。运行环境用于让网页后端找到 `node`、`npm`、`claude` 和管理 Python。
...
| `framework/scripts/run_module.py environment --action deploy_from_plan --project <project>` | 校验当前执行合同并调用 Environment。 |
...
| `framework/scripts/run_module.py writing --action work --project <project> --venue <venue>` | 将论文生成、修复和审计任务交给项目唯一的 Writing 主控 Claude。 |
...
正常项目流程请使用网页或 `framework/scripts/run_module.py`；Framework 负责校验输入、调用七个阶段模块并同步 `projects/<project>/`，Web 只传递配置和命令、展示任务状态与产物。需要独立使用某个模块时，从对应 `modules/<stage>/main.py` 进入，并按该模块 `README.md` 提供显式输入；模块不会替代 Framework 发现或同步项目状态。
