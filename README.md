...
├── framework/scripts/      # TASTE 编排、运行时和共享入口脚本
├── modules/finding/        # Find 独立模块边界，含 scripts/
├── modules/reading/        # Read 独立模块边界，含 scripts/
├── modules/ideation/       # Ideas 独立模块边界，含 scripts/
├── modules/planning/       # Plan 独立模块边界，含 scripts/
├── modules/environment/    # Environment 独立模块边界，含 scripts/
├── modules/experimenting/  # Experiment 独立模块边界，含 scripts/
├── modules/writing/        # Paper/Writing 独立模块边界，含 scripts/
├── framework/              # TASTE 运行时、任务队列、共享模型和跨阶段编排
...
| 路径 | 作用 |
| --- | --- |
| `framework/scripts/create_project.py` | 创建项目。 |
| `framework/scripts/start_web.sh` | 启动 Web/API。 |
| `framework/scripts/runtime_env.py` | 检测和构造 Python、Node、Claude Code 运行环境。 |
| `modules/reading/scripts/ensure_current_find_research_plan.py` | 让 Claude Code 接管当前 Find 的 Read/Ideas/Plan。 |
| `framework/scripts/claude_project_session.py` | Claude Code 项目会话和 guidance 队列。 |
| `modules/environment/scripts/run_environment_stage.py` | 环境配置阶段。 |
| `modules/experimenting/scripts/run_coding_agent.py` | 实验代理入口。 |
| `modules/writing/scripts/run_paper_orchestra_bridge.py` | 论文生成、修复和门控。 |
| `modules/finding/` 到 `modules/writing/` | 七个独立科研阶段模块，每个目录包含合同、CLI 适配入口和脚本归属 manifest。 |
| `framework/` | TASTE 运行时、共享模型、任务队列、项目状态和跨阶段编排；不承载某个科研阶段的私有逻辑。 |
| `web/` | FastAPI bridge 与 React/Vite 前端；只负责人类交互、配置、任务触发和产物展示。 |
...
`modules/` 只放七个科研阶段模块。`framework/` 负责运行时、任务队列、项目状态、共享模型和跨阶段编排；`web/` 负责 FastAPI bridge 与 React/Vite 前端。Find 不负责全文抓取；全文证据、阅读包和同轮可读 replacement 属于 Read-stage packet。Environment/Experiment/Writing 只能消费显式产物和选择合同，不能隐式读取历史阶段状态来替代当前模块输入。
