...
def active_repo_candidate(active: dict[str, Any], fresh_find_run_id: str = '') -> dict[str, Any] | None:
    if not isinstance(active, dict) or not active.get('repo_path'):
        return None
    repo_path = str(active.get('repo_path'))
    if not Path(repo_path).exists():
        return None
    claim_ready = active.get('claim_ready_datasets') if isinstance(active.get('claim_ready_datasets'), list) else []
    if active.get('claim_ready_dataset') and active.get('claim_ready_dataset') not in claim_ready:
        claim_ready = [active.get('claim_ready_dataset'), *claim_ready]
    return {
        ...
        'claim_ready_dataset': active.get('claim_ready_dataset', ''),
        'claim_ready_datasets': claim_ready,
        'literature_base_title': active.get('selected_base_title') or active.get('literature_base_title', ''),
        'fresh_find_run_id': fresh_find_run_id,
        'source': 'current_active_repo_reaudit' if fresh_find_run_id else 'active_repo',
        '_source': 'active_repo',
...
def main() -> int:
    ...
    skip_active_for_current_source_pool = bool(args.selection_stage == 'environment_claude_code' and args.fresh_find_run_id and args.candidate_source)
    active_candidate = None if (args.exclude_active_repo or skip_active_for_current_source_pool) else active_repo_candidate(active, args.fresh_find_run_id if args.selection_stage == 'environment_claude_code' else '')
    if active_candidate:
    ...
    if args.selection_stage == 'environment_claude_code' and args.fresh_find_run_id:
        current_rows = [row for row in candidates if str(row.get('fresh_find_run_id') or '') == args.fresh_find_run_id]
    ...
    active_only_current_reaudit = bool(
        selection_stage == 'environment_claude_code'
        and args.fresh_find_run_id
        and audited
        and all(item.get('source') == 'active_repo' for item in audited)
    )
    ...
    if active_only_current_reaudit:
        ...
        payload['selected'] = {}
        payload['active_repo_reaudit_blocked'] = True
        payload['blocker'] = 'Only the legacy active_repo was audited for this current Find run; it remains historical/control evidence until fresh current-run candidates are compared.'
        selected = {}
    elif args.use_claude_review:
