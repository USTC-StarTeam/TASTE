...
def ensure_reference_full_job(paths, project: str, venue: str, py: str) -> dict[str, Any]:
    ...
    cmd = [
        py,
        str(ROOT / "modules" / "environment" / "scripts" / "run_selected_base_reference_reproduction_audit.py"),
        "--project", project,
        "--mode", "full",
        "--epoch", "30",
...
def main() -> int:
    ...
    if isinstance(fresh_plan, dict) and fresh_plan.get('status') == 'implementation_ready_for_reference_probe':
        if not reference_protocol_passed(paths):
            results.append(run_step('selected_base_reference_protocol_probe', [probe_py, str(ROOT / 'modules' / 'environment' / 'scripts' / 'probe_selected_base_reference.py'), '--project', args.project, '--mode', 'protocol', '--timeout-sec', '240'], timeout=300))
    ...
    for name, cmd, timeout in [
        ('reference_reproduction_gate', [py, str(ROOT / 'modules' / 'experimenting' / 'scripts' / 'audit_reference_reproduction.py'), '--project', args.project, '--venue', venue], 180),
        ('blocker_action_plan', [py, str(ROOT / 'modules' / 'planning' / 'scripts' / 'build_blocker_action_plan.py'), '--project', args.project, '--venue', venue], 180),
    ]:
