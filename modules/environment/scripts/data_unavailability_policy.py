...
def normalize_repo_candidate(row: dict) -> dict:
    ...
    execution_ready = bool(
        ...
        or row.get('repo_execution_ready')
        or row.get('execution_ready_after_audit')
        or ready_datasets
        or loader_probe.get('success')
    )
    return {
        'name': str(row.get('name') or row.get('repo') or row.get('full_name') or ''),
        'url': str(row.get('url') or row.get('html_url') or row.get('repo_url') or ''),
        'local_path': str(row.get('local_path') or row.get('repo_path') or row.get('path') or ''),
        'score': float(row.get('score', row.get('repo_reuse_score', row.get('selection_score', 0))) or 0),
        'bucket': str(row.get('bucket') or row.get('repo_selection_bucket') or row.get('decision') or ''),
        'support_signals': row.get('support_signals') or row.get('repo_support_signals') or [],
        'execution_ready': execution_ready,
        'ready_datasets': [str(item) for item in ready_datasets if item],
    }


def normalize_repo_candidates(rows) -> list[dict]:
    if not isinstance(rows, list):
        return []
    return [normalize_repo_candidate(row) for row in rows if isinstance(row, dict)]
...
def main() -> None:
    ...
    evidence_ready_alternatives = normalize_repo_candidates(pool_ready) or candidate_alternatives(paths, str(active.get('repo_path', '')), floor, require_execution_ready=True)
