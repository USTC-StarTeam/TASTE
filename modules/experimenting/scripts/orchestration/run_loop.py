...
def main() -> int:
    ...
    if guidance:
        ...
        args.prompt = ((args.prompt or args.topic or "") + "\n\nQueued external guidance:\n" + guidance_text).strip()
    init_cmd = [sys.executable, str(ROOT / "framework" / "scripts" / "init_workspace.py"), "--project", args.project, "--conda-env", args.conda_env]
    ...
    for idx in range(args.iterations):
        run_cmd = [sys.executable, str(ROOT / "framework" / "scripts" / "run_project.py"), "--project", args.project]
        if topic:
        ...
        if not args.skip_trajectory_supervisor and args.trajectory_rounds > 0:
            supervisor_cmd = [
                sys.executable, str(ROOT / "framework" / "scripts" / "run_research_trajectory_supervisor.py"),
                "--project", args.project,
    ...
    run([sys.executable, str(ROOT / "framework" / "scripts" / "generate_handoff.py"), "--project", args.project], paths.logs / "run_loop_generate_handoff.log", args.project, agent_id, "handoff")
