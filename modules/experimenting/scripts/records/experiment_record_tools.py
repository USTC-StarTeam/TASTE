...
def run_record_table(argv: list[str] | None = None) -> int:
...
# ---- experiment log tool ----
def _log_load_json(path: Path):
    return json.loads(path.read_text(encoding='utf-8')) if path.exists() else []

def _log_save_json(path: Path, data) -> None:
    path.write_text(json.dumps(data, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')

def _log_load_metrics_blob(metrics_json: str, metrics_path: str) -> dict:
    if metrics_json:
...
def _log_git_snapshot(repo_path: str) -> dict[str, object]:
    if not repo_path:
        return {}
    repo = Path(repo_path)
    ...
    return {'git_commit': run(['git', 'rev-parse', 'HEAD']), 'git_branch': run(['git', 'rev-parse', '--abbrev-ref', 'HEAD']), 'repo_dirty': bool(status), 'git_status_short': status}

def _log_experiment_identity(row: dict) -> tuple[str, str, str]:
    return (str(row.get('experiment_id') or row.get('name') or ''), str(row.get('repo_path') or ''), str(row.get('dataset') or ''))

def _log_upsert(rows: list[dict], item: dict) -> list[dict]:
    identity = _log_experiment_identity(item)
    legacy_identity = (identity[0], '', '')
    for index, row in enumerate(rows):
        row_identity = _log_experiment_identity(row)
...
def _log_rewrite_markdown(path: Path, rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        lines.append(f"| {row.get('timestamp', '')} | {row.get('name', '')} | {row.get('method', '')} | {row.get('status', '')} | {metric_bits} | {row.get('result', '')} | {row.get('decision', '')} | {row.get('claim_verdict', '')} | {audit_bits} | {row.get('git_commit', '')[:8]} |\n")
    lines.append('\n## Details\n')
    for row in rows[:20]:
...
def run_log(argv=None) -> int:
    ...
    rows = _log_load_json(registry_path)
    metrics = _log_load_metrics_blob(args.metrics_json, args.metrics_path)
    ...
    git_info = _log_git_snapshot(args.repo_path)
    if args.git_commit:
    ...
    item = {'timestamp': timestamp, 'started_at': args.started_at, 'finished_at': args.finished_at, 'experiment_id': args.experiment_id or args.name, 'name': args.name, 'repo': args.repo, 'repo_path': args.repo_path, 'dataset': args.dataset, 'benchmark': args.benchmark, 'method': args.method, 'method_slug': args.method_slug or args.method, 'method_role': args.method_role, 'comparison_role': args.comparison_role or args.method_role, 'human_label': args.human_label, 'human_goal': args.human_goal, 'config_summary': args.config_summary, 'status': args.status, 'metric_name': args.metric, 'metric_value': metric_value, 'metrics': metrics, 'result': args.result, 'notes': args.notes, 'artifact_path': args.artifact_path, 'env_name': args.env_name, 'command': args.command, 'return_code': args.return_code, 'duration_sec': args.duration_sec, 'trial_index': args.trial_index, 'priority': args.priority, 'decision': args.decision, 'bad_case_path': args.bad_case_path, 'bad_case_summary': bad_case_summary, 'bad_case_slices': bad_case_summary.get('slices', []), 'failure_analysis_path': args.failure_analysis_path, 'claim_verdict': args.claim_verdict, 'novelty_note': args.novelty_note, 'counterexample_outcome': args.counterexample_outcome, 'audit_path': args.audit_path, 'audit_ready': str(args.audit_ready).lower() in {'1', 'true', 'yes'}, 'missing_audit_fields': [item for item in args.missing_audit_fields.split(',') if item]}
    item.update(git_info)
    rows = _log_upsert(rows, item)
    _log_save_json(registry_path, rows)
    md_path = paths.experiments / 'experiment_log.md'
    _log_rewrite_markdown(md_path, rows)
...
TOOL_ACTIONS = {
    "log": run_log,
    "record_table": run_record_table,
}


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Experimenting module private record/log tools.")
    parser.add_argument("--tool-action", default="record_table", choices=sorted(TOOL_ACTIONS))
    ns, rest = parser.parse_known_args(argv)
    return int(TOOL_ACTIONS[ns.tool_action](rest) or 0)
