...
def build_prompt(agent_name: str, args: argparse.Namespace, repo: Path, trial_context: str) -> str:
    ...
    return (
        ...
        f"If the packet is missing or unrelated, run `{management_python()} modules/finding/scripts/run_literature_tool.py --project {args.project} --query \"<targeted query>\" --fast-mode` from the module root as an internal project-agent survey, read the packet path printed under `state/internal_literature_runs/...`, and do not publish to the web-facing current Find unless the TASTE wrapper/user explicitly passes `--publish-current-find`. "
