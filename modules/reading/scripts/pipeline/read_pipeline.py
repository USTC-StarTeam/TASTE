...
from common.read_ranking import sort_readings_by_ranking
...
def _doi_from_paper(paper: dict) -> str:
    for key in ["doi", "published_doi", "url", "abs_url", "pdf_url", "input_article"]:
        doi = _doi_from_text(paper.get(key))
        if doi:
            return doi
    return ""
...
def _openalex_headers() -> dict[str, str]:
    return {"User-Agent": READ_USER_AGENT, "Accept": "application/json"}
...
def _openalex_api_params(extra: dict[str, str] | None = None) -> dict[str, str]:
    ...
    mailto = str(os.environ.get("OPENALEX_MAILTO") or os.environ.get("CROSSREF_MAILTO") or "").strip()
    if mailto:
...
def _openalex_pdf_candidates(paper: dict, limit: int = 5) -> list[dict]:
    ...
    try:
        if doi:
            ...
            response = requests.get(url, params=_openalex_api_params(), timeout=30, headers=_openalex_headers())
            if response.status_code == 200:
        ...
        if len(title.split()) >= 3:
            ...
            response = requests.get("https://api.openalex.org/works", params=params, timeout=30, headers=_openalex_headers())
            if response.status_code == 200:
                payload = response.json()
...
def _unpaywall_pdf_candidates(paper: dict) -> list[dict]:
    ...
    try:
        ...
        response = requests.get(url, params={"email": email}, timeout=30, headers={"User-Agent": READ_USER_AGENT, "Accept": "application/json"})
        if response.status_code != 200:
            return []
        payload = response.json()
    except Exception:
...
def _pdf_candidates_for_reading(paper: dict) -> list[dict]:
    ...
    for key in ["pdf_urls", "oa_pdf_urls", "candidate_pdf_urls"]:
        for value in _iter_pdf_url_values(paper.get(key)):
            add(key, value)
    for candidate in semantic_scholar_pdf_candidates(paper):
...
"""
    ...
    _raise_if_cancelled(should_cancel)
    readings, reading_ranking = sort_readings_by_ranking(readings, request.run_id, source="reading_module_post_read_rerank")
    lines = ["# 当前推荐论文精读", "", f"- run_id: `{request.run_id}`", f"- readings: {len(readings)}", ""]
    ranked_items = [row for row in reading_ranking.get("ranked_readings", []) if isinstance(row, dict)]
    if ranked_items:
        lines.extend(["## 读后重评分排序", "", str(reading_ranking.get("comparative_summary_zh") or ""), "", "| 排名 | 分数 | 论文 | 评分依据 |", "|---:|---:|---|---|"])
        for row in ranked_items:
            title = str(row.get("title") or "").replace("|", " ")
            reason = str(row.get("why_ranked_here_zh") or "").replace("|", " ")
            lines.append(f"| {row.get('rank')} | {row.get('read_score')} | {title} | {reason} |")
        lines.append("")
