...
def build_ideas(readings: list[dict[str, Any]], repo: dict[str, Any], fresh_plan: dict[str, Any], cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "supporting_papers": [_paper_ref(row) for row in readings[:8]],
            "claude_code_tasks": ["读取当前 Find 的推荐论文、边界审计和来源状态。", "围绕缺口生成补充检索主题；如推荐数量短缺，通过 framework/scripts/run_module.py finding --action literature_tool 受控补检索并刷新 packet。", "复核评分/降级原因并刷新 literature gate，禁止用未评分/无摘要论文凑推荐数量。"],
...
def _render_method_overview(readings: list[dict[str, Any]]) -> list[str]:
    lines = ["## 方法差异、优缺点总览\n\n"]
    lines.append("| 排名 | 读后分 | 论文 | 机制类别 | 主要优点 | 主要局限 |\n")
    lines.append("|---:|---:|---|---|---|---|\n")
    families: list[str] = []
    recurring_advantages: list[str] = []
    recurring_risks: list[str] = []
    for idx, row in enumerate(readings, 1):
        rank = row.get("read_rank") or idx
        score = row.get("read_score")
        score_text = _score_display(score) if score not in (None, "") else "未评分"
        ...
        advantages_list = [_ensure_zh_sentence_end(item) for item in _read_list(row, "method_advantages_zh")[:3]]
        disadvantages_list = [_ensure_zh_sentence_end(item) for item in _read_list(row, "method_disadvantages_zh", "limitations_zh", "limitations")[:3]]
        recurring_advantages.extend(advantages_list[:1])
        recurring_risks.extend(disadvantages_list[:1])
        advantages = "；".join(advantages_list[:2]) or "机制优点缺失，需项目代理返修。"
        disadvantages = "；".join(disadvantages_list[:2]) or "方法局限缺失，需项目代理返修。"
        lines.append(f"| {rank} | {score_text} | {title} | {_protect_inline_math(family).replace('|', ' ')} | {_protect_inline_math(advantages).replace('|', ' ')} | {_protect_inline_math(disadvantages).replace('|', ' ')} |\n")
    lines.append("\n### 总体差异\n\n")
    if families:
        lines.append(_ensure_zh_sentence_end("这些论文覆盖" + "、".join(families[:8]) + "等机制路线；排序靠前的论文通常同时具备更完整的全文证据、更可迁移的方法细节和更清晰的实验/局限边界") + "\n")
    else:
        lines.append("机制类别仍缺失，需项目代理重新完成逐篇精读和方法对比。\n")
    if recurring_advantages:
        lines.append("\n### 可借鉴优点\n")
        for item in recurring_advantages[:6]:
            lines.append(f"- {_protect_inline_math(item)}\n")
    if recurring_risks:
        lines.append("\n### 共性不足与风险\n")
        for item in recurring_risks[:6]:
            lines.append(f"- {_protect_inline_math(item)}\n")
    lines.append("\n")
    return lines
...
- `planning/finding/idea_scoring.json`：允许作为独立评分子任务的受控审计文件写入；评分结论必须同步回填到 `ideas.json`。
...
- 不要用 Bash/Python/cat/heredoc 读取、生成或修补 `ideas.json`、`idea_scoring.json`、`plans.json`、`read_results.json` 或 deep-read fragments。内容查看必须用 Claude Read；写入必须用 Claude Write/Edit/MultiEdit。Bash 只允许 `{management_python()} -m json.tool planning/finding/ideas.json >/dev/null` 和同类 JSON 语法检查。
...
4. 每个 idea 必须有 `objective_scores={{"novelty":...,"evidence_alignment":...,"feasibility":...,"experimentability":...,"risk_control":...,"overall":...}}`，同时写 `score` 和 `idea_score`，且 `idea_score_audit={{"mode":"task_subagent","subagent_used":true,"status":"completed","criteria":"TASTE-like objective idea scoring"}}`。可以先把独立评分子任务完整结论写入 `planning/finding/idea_scoring.json` 作审计，但最终必须把每个 idea 的评分和审计字段同步回填到 `ideas.json`；不要停留在 `needs_scoring`。若当前 Claude 工具面板没有 Task/subagent，仍必须诚实写 `status:"blocked_task_subagent_unavailable"` 并说明，不能伪造已调用。
...
- idea_contract_issues 表示上一轮 idea 不是合格科研 idea：缺 `objective_scores`、`score/idea_score` 为 None、没有独立评分子任务、初步实验仍绑定历史基底/旧 base switch gate，或 `new_method`/`initial_experiment`/`inspired_by` 三段缺失。必须重新基于精读结果生成 {idea_plan_count} 个 idea；每个 idea 都要先由主控 Claude 写出详细 `new_method`、`initial_experiment`、`inspired_by`，再调用独立 Task/subagent 客观评分。可以用 Claude 文件工具写 `planning/finding/idea_scoring.json` 作为评分子任务审计中间件，但最终必须把 `objective_scores={{novelty,evidence_alignment,feasibility,experimentability,risk_control,overall}}`、`score`、`idea_score` 和 `idea_score_audit={{mode:"task_subagent",subagent_used:true,status:"completed"}}` 回填到 `planning/finding/ideas.json`。`overall < 7.0` 的 idea 必须由主控 Claude 深思修改后重新评分；不得把 None/空分数或旧路线记忆当成通过。
...
def build_execution_plan(project: str, cfg: dict[str, Any], run_id: str, readings: list[dict[str, Any]], ideas: list[dict[str, Any]], plans: list[dict[str, Any]], fresh_plan: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        **selection_fields,
        "guardrails": [
            "Do not launch raw or duplicate Find jobs from this script; use framework/scripts/run_module.py finding --action literature_tool for controlled targeted literature repair.",
...
def run_llm_current_find_fallback(project: str, paths, cfg: dict[str, Any], taste_dir: Path, run_id: str, find_results: dict[str, Any], papers: list[dict[str, Any]], effective_read_limit: int, idea_count: int, find_revision: dt.datetime | None) -> int:
    ...
    readings, reading_ranking = sort_readings_by_ranking(readings, run_id, source=LLM_CURRENT_FIND_FALLBACK_SOURCE)
