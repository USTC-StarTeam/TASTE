...
for candidate in (CURRENT_FILE, *CURRENT_FILE.parents):
    if (candidate / "framework").is_dir() and (candidate / "modules").is_dir() and (candidate / "web").is_dir():
        WORKSPACE_ROOT = candidate
        break
else:
    WORKSPACE_ROOT = CURRENT_FILE.parents[6]
FRAMEWORK_SCRIPTS = WORKSPACE_ROOT / "framework" / "scripts"
...
def _missing_abstract_count(papers: list[dict[str, Any]]) -> int:
    return sum(1 for row in papers if isinstance(row, dict) and not _clean_text(row.get("abstract")))
...
def _normalize_metadata_audit(
    ...
    *,
    ...
    if official_scope:
        ...
        if official_scope in {"official_icml_downloads_title_index", "official_openreview_metadata"}:
...
def _write_cache(venue_id: str, venue: dict[str, Any], year: int, papers: list[dict[str, Any]], adapter: str, audit: dict[str, Any], output_root: Path) -> Path:
    ...
    (directory / "papers.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    summary_payload = {
        "schema_version": 1,
        "venue_id": venue_id,
        "venue": venue.get("name") or venue_id,
    ...
    (directory / "category_summary.json").write_text(json.dumps(summary_payload, ensure_ascii=False, indent=2), encoding="utf-8")
...
def _audit_existing_cache(directory: Path, venue_id: str, year: int) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], list[dict[str, Any]]]:
    ...
    audit["papers_path"] = str(papers_path)
    audit["category_summary_path"] = str(summary_path)
...
def build_cache(venue_id: str, year: int, output_root: Path = LOCAL_DATABASE_DIR, max_items: int = 12000) -> Path:
    catalog = catalog_by_id()
    venue = catalog.get(venue_id)
    if not venue:
        raise SystemExit(f"Unknown venue id: {venue_id}")
    papers, adapter = fetch_venue_title_index(venue, [year], max_items)
...
def main() -> None:
    ...
    if args.from_existing:
        ...
        directory = build_cache(args.venue_id, args.year, Path(args.output_root), max_items=max(1, args.max_items))
