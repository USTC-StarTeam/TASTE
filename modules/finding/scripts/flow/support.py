    ...
    for paper in papers[:limit]:
        ...
        try:
            if doi:
                ...
                response = finding_service_get(doi_url, params=params, headers=HEADERS, timeout=_metadata_timeout(6))
            ...
            if not item and query and not rate_limited and (max_network_requests <= 0 or network_requests < max_network_requests):
                ...
                network_requests += 1
                last_request_at = time.monotonic()
                response = finding_service_get(search_url, params=params, headers=HEADERS, timeout=_metadata_timeout(6))
                if response.status_code == 429:
                    metadata = paper.setdefault("metadata", {})
                    metadata["openalex_lookup_error"] = "http_429"
                    metadata["openalex_lookup_retryable"] = True
...
def _dblp_request(
    ...
    *,
    ...
    for attempt in range(attempts_allowed):
        ...
        try:
            response = finding_service_get(candidate, params=params, headers=HEADERS, timeout=timeout)
