...
FINAL_LLM_SCORE_CACHE_PROMPT_POLICY = "final_title_abstract_prompt_v29_source_guard_positive_core_component"
...
- The workflow selects user-visible recommendations by sorting eligible final-scored rows. Eligibility requires real title+abstract judgment and topic_evidence_supported=true for the current core route; do not add a separate fixed numeric cutoff after eligibility.
...
def _title_abstract_evidence_text(item: dict) -> str:
    return " ".join(
        part
        for part in [
...
def _route_anchor_source_support(item: dict, route: str) -> tuple[bool, list[str], list[str]]:
    if not _has_real_abstract(item):
        return False, [], ["real abstract"]
    source_text = _title_abstract_evidence_text(item)
    terms = _route_anchor_terms(route)
    if not terms:
        return False, [], ["complete route anchor"]
    matched = [term for term in terms if _source_has_route_term(source_text, term)]
...
def _best_complete_source_supported_route(item: dict, interest: str) -> tuple[str, list[str], list[str]]:
...
def _apply_complete_route_guard(item: dict, interest: str) -> None:
    ...
    if not routes:
        return
    if any("一" <= char <= "鿿" for char in " ".join(routes)):
        return
    supported_route, matched_terms, missing_terms = _best_complete_source_supported_route(item, interest)
    if supported_route:
...
def _find_cache_path(filename: str) -> Path:
    ...
    if project:
        return root / "projects" / project / "planning" / "finding" / "cache" / filename
    return WORKFLOW_RUNTIME_DIR / "state" / filename


def _final_llm_score_cache_enabled(config: AppConfig | None = None, llm: LLMClient | None = None) -> bool:
    raw = os.environ.get("FIND_FINAL_SCORE_CACHE", "1").strip().lower()
...
def _final_llm_score_cache_path() -> Path:
    return _find_cache_path("final_llm_scores.json")


def _stage0_profile_cache_path() -> Path:
    return _find_cache_path("stage0_profiles.json")


def _venue_title_index_cache_path() -> Path:
...
def _latest_project_find_results_title_index(venue: dict, years: list[int], limit: int | None) -> tuple[list[dict], str]:
    finding_dir = _venue_title_index_cache_path().parent.parent
...
def _store_venue_title_index_cache(venue: dict, years: list[int], papers: list[dict], adapter: str) -> None:
    ...
    cache = read_json(path, {}) if path.exists() else {}
    if cache.get("schema") != VENUE_TITLE_INDEX_CACHE_SCHEMA_VERSION:
...
def _load_stage0_profile_cache(config: AppConfig) -> dict[str, Any] | None:
    path = _stage0_profile_cache_path()
    cache = read_json(path, {}) if path.exists() else {}
    entries = cache.get("entries") if isinstance(cache.get("entries"), dict) else {}
...
def _title_llm_score_cache_enabled(config: AppConfig | None = None, llm: LLMClient | None = None) -> bool:
    raw = os.environ.get("FIND_TITLE_SCORE_CACHE", "1").strip().lower()
    if raw in {"0", "false", "no", "off"}:
        return False
    if llm is not None and not isinstance(llm, LLMClient):
        return False
...
def _title_llm_score_cache_path() -> Path:
    return _find_cache_path("title_llm_scores.json")
...
def _load_title_llm_score_cache() -> tuple[Path, dict]:
    path = _title_llm_score_cache_path()
    data = read_json(path, {})
...
def _title_llm_cache_title_lookup_key(item: dict) -> str:
    return _cache_normalized_text(item.get("title"), limit=2000).lower()
...
def _load_final_llm_score_cache() -> tuple[Path, dict]:
    path = _final_llm_score_cache_path()
    data = read_json(path, {})
    if not isinstance(data, dict):
        data = {}
    entries = data.get("entries")
...
def _venue_title_fetch_timeout_sec() -> float:
    return _timeout_env_value(("FIND_VENUE_TITLE_FETCH_TIMEOUT_SEC", "VENUE_TITLE_FETCH_TIMEOUT_SEC"), 120.0)


def _venue_year_probe_timeout_sec() -> float:
    return _timeout_env_value(("FIND_VENUE_YEAR_PROBE_TIMEOUT_SEC", "VENUE_YEAR_PROBE_TIMEOUT_SEC"), 8.0)
...
def _fetch_venue_title_index_for_find(
    ...
    *,
    ...
    if prefer_cache:
        ...
        if cached:
            return cached, cached_adapter
    timeout_sec = _venue_title_fetch_timeout_sec() if timeout_sec is None else max(0.0, float(timeout_sec or 0.0))
    if timeout_sec > 0:
            ...
            for batch, context in batches_with_context:
                ...
                for item in batch:
                    ...
                    if not cache_hit:
                        ...
                        title_key = _title_llm_cache_title_lookup_key(item)
...
""")
        ...
        if title_cache_path is not None and title_cache_keys:
            ...
            if stored_title_cache_entries:
                write_json(title_cache_path, title_score_cache)
    ...
    *,
    ...
    def probe(candidate: int) -> tuple[bool, str]:
        ...
        if local and int(local.get("paper_count") or 0) > 0:
            ...
            return probe_cache[candidate]
        try:
            titles, adapter = _fetch_venue_title_index_for_find(venue, [candidate], 1, timeout_sec=_venue_year_probe_timeout_sec(), prefer_cache=True)
        except Exception:
...
"""
        ...
        if cache_path is not None and score_cache_keys:
            ...
            if stored_cache_entries:
                write_json(cache_path, score_cache)
...
def _looks_complete_chinese_translation(text: str, source: str, item: dict | None = None) -> bool:
    return not _chinese_translation_reject_reason(text, source, item)
...
def _translation_text_from_data(data: object, item: dict, *, allow_any_id: bool = False) -> str:
    ...
    for text in candidates:
        if _looks_complete_chinese_translation(text, source, item):
            return text
    return ""
        ...
        for batch_index, (batch, result) in enumerate(zip(prompt_batches, results, strict=False), 1):
            ...
            if isinstance(rows, list):
                for row in rows:
                    ...
                    if item and text and _looks_complete_chinese_translation(text, str(item.get("abstract_en") or ""), item):
...
def run_find(
    ...
    # Read starts from the same user-facing recommendation pool. Full-text
    # acquisition and any same-run replacement selection are owned by Read.
    ...
    def _build_find_artifacts(translation_status: str) -> dict:
        ...
        artifacts = {
            ...
            "artifact_semantics": {
                "articles": "Backward-compatible alias for strong_recommendations and the source of article.md; contains final LLM title+abstract recommendations with real abstracts and supported current-topic evidence.",
                "strong_recommendations": "User-facing recommended papers: supported-topic, real-abstract rows ranked by final title+abstract LLM fit plus a small gated official-presentation bonus.",
                "screened_ranking": "Uncapped eligible final-LLM recommendation ranking for inspection; article.md/read_candidates take the top configured rows from the same list.",
                "read_candidates": "Backward-compatible alias for the user-facing recommended-paper pool. Read owns full-text acquisition and builds its own full-text reading packet from this pool plus same-run ranked replacements when needed.",
