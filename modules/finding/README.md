...
> 维护者请先读同目录的 **`工作状态.txt`**：里面有架构全量梳理、框架边界契约、测试基线和正在进行的
> 分阶段重构计划。本 README 描述"稳定的对外口径与用法"，`工作状态.txt` 描述"当前进度与决策"。
...
（已知弱点：HuggingFace papers 流不支持日期窗；medRxiv/PubMed 未实现。详见 `工作状态.txt` 源覆盖矩阵。）
...
```bash
# 全部在远程：ssh hidimension_5090_1，工作根 /home/fmh/workspace/TASTE
...
WORKSPACE_ROOT=/home/fmh/workspace/TASTE \
...
WORKFLOW_RUNTIME_DIR=/home/fmh/workspace/TASTE/runtime \
...
> 运营约定（见根 `工作状态.txt`）：端到端验证走网页 UI，CLI 只用于调试/测试/查状态；
...
- 修改本模块必须先读相关脚本、`script_manifest.json` 与 `工作状态.txt`，找到根因再改；
...
- **任何结构改动（脚本分子目录、巨型文件拆分、产物落点迁移）都受框架"模块边界契约"约束**
  ...
  动结构前务必读 `工作状态.txt` 的"框架模块边界契约"与"分阶段重构计划"两节，并让全部测试保持绿。
