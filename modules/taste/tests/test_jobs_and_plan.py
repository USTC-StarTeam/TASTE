...
def test_running_claude_environment_job_uses_panel_stage_and_public_progress():
    ...
    row = server._compact_job_for_list({
        ...
        "logs": [
            ...
            "Workflow command: /tmp/taste-local/miniforge/bin/python /tmp/taste-local/TASTE/scripts/claude_project_session.py --project demo --stage environment --message secret details",
...
def test_runtime_env_separates_management_and_experiment_python(tmp_path, monkeypatch):
    ...
    assert str(tmp_path / "modules" / "taste") in env["PYTHONPATH"].split(":")
    assert str(tmp_path / "scripts") in env["PYTHONPATH"].split(":")
...
def test_create_project_cli_preserves_prompt_and_conda_env(tmp_path, monkeypatch):
    ...
    script_path = server.WORKSPACE_ROOT / "scripts" / "create_project.py"
...
def test_compact_job_list_hides_raw_historical_command():
    ...
    item = {
        ...
        "result": {
            "project": "demo_project",
            "phase": "idea",
            "command": "/env/bin/python scripts/run_full_research_cycle.py --project demo_project --venue Nature",
            "status": "stale",
            "process_alive": False,
        },
        "progress": {},
...
def test_compact_job_list_hides_internal_json_log_chunks():
    ...
    assert "read blocked" in compact["logs"]
    assert all("idea_contract" not in line for line in compact["logs"])
    assert all("guardrail" not in line for line in compact["logs"])
    assert "{" not in compact["logs"]
...
def test_full_cycle_gate_blocks_duplicate_live_worker(monkeypatch, tmp_path):
    ...
    write_json(
        state_dir / "full_cycle_job.json",
        {
            ...
            "process_alive": True,
            "command": "/env/bin/python scripts/run_full_research_cycle.py --project demo_project",
...
def test_cancel_prefers_active_project_child_over_stale_full_cycle_pid(monkeypatch, tmp_path):
    ...
    def active_child(seen_project, seen_root, phase_hint=""):
        ...
        return {
            ...
            "phase": "paper",
            "kind": "paper_pipeline",
            "cmd": "/env/bin/python scripts/run_paper_pipeline.py --project demo_project --venue Nature",
        }
...
def test_live_jobs_lists_active_worker_while_full_cycle_controller_runs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "list_projects", lambda: [{"id": project, "path": str(root)}])
    monkeypatch.setattr(server, "_pid_alive_local", lambda pid: str(pid) in {"100", "202"})
    monkeypatch.setattr(server, "_ps_row_for_pid", lambda pid: {"pid": str(pid), "elapsed": "00:20", "cmd": "python scripts/run_full_research_cycle.py --project demo_project"} if str(pid) == "100" else {})
    monkeypatch.setattr(server, "_active_project_child_processes", lambda seen_project, seen_root, phase_hint="": [
        {"pid": "100", "ppid": "1", "stat": "S", "elapsed": "00:20", "pcpu": "1.0", "pmem": "0.1", "cmd": "python scripts/run_full_research_cycle.py --project demo_project", "cwd": str(root), "kind": "full_cycle", "phase": "full-cycle"},
...
def test_cancel_exact_environment_job_does_not_fallback_to_stale_full_cycle(monkeypatch, tmp_path):
    ...
    job.status = "running"
    job.logs = ["Workflow command: /env/bin/python scripts/run_environment_stage.py --project demo_project --venue ICLR"]
    ...
    def active_child(seen_project, seen_root, phase_hint=""):
        ...
        if phase_hint == "environment":
            return {"pid": "202", "phase": "environment", "kind": "environment_stage", "cmd": "/env/bin/python scripts/run_environment_stage.py --project demo_project"}
        return {}
...
def test_environment_repo_search_ignores_find_source_toggles(monkeypatch):
    ...
    github = next(cmd for cmd in calls if "scripts/discover_github_repos.py" in cmd)
    arxiv = next(cmd for cmd in calls if "scripts/discover_arxiv.py" in cmd)
...
def test_find_survey_panel_stays_between_source_config_and_task_artifacts():
    ...
    assert "literature_survey_preview" in (Path(__file__).resolve().parents[1] / "auto_research" / "web" / "client" / "src" / "api.ts").read_text(encoding="utf-8")
    assert "_light_find_survey_from_progress" in (Path(__file__).resolve().parents[1] / "auto_research" / "web" / "project_bridge.py").read_text(encoding="utf-8")
...
def test_frontend_plan_page_uses_compact_controls_and_plan_markdown_artifact():
    ...
    assert 'className="planControlGrid"' in block
    assert 'className="panel planControlPanel"' in block
    assert 'className="planSelect"' in block
    ...
    assert 'runPlanPolish(selectedPlanForControls.plan_id' in block
    assert 'runPlanFinish(selectedPlanForControls.plan_id)' in block
    ...
    assert '.planPickList' in css
    assert '.planSelect' in css
...
def test_frontend_start_read_requests_all_recommendations_by_default():
    ...
    api = Path("modules/taste/auto_research/web/client/src/api.ts").read_text(encoding="utf-8")
...
def test_frontend_paper_self_review_evidence_blockers_are_submission_gate():
    ...
    app = Path("modules/taste/auto_research/web/client/src/App.tsx").read_text(encoding="utf-8")
        ...
        lambda: [
            {
                ...
                "ppid": "1",
                "kind": "claude_session",
                "cmd": "python scripts/ensure_current_find_research_plan.py --project demo_project",
                "cwd": str(root),
            },
            {
                "pid": "222",
                "ppid": "111",
                "kind": "claude_session",
                "cmd": "python scripts/claude_project_session.py --project demo_project --stage current-find-claude-read-idea-plan",
                "cwd": str(root),
            },
            {
                "pid": "333",
...
def test_full_text_unavailable_repair_does_not_rewrite_find_topn(tmp_path):
    ...
    scripts_dir = server.WORKSPACE_ROOT / "scripts"
