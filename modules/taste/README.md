...
| 字段 | 说明 |
| --- | --- |
| `research_interest` | 当前想追踪的研究方向和匹配标准。建议写清楚研究问题、方法关键词、应用领域、正例/反例、偏好的 venue/年份、数据和代码要求；不要只写一个宽泛主题。 |
| `researcher_profile` | 你的背景、已有项目、熟悉框架、可用算力、数据访问限制、长期方向、评价标准，以及不希望推荐的方向。 |

建议写清楚什么算“真正匹配”，什么只是“泛泛 AI 相关”。画像越具体，TASTE 的论文匹配、标题筛选、摘要评分、idea 生成和后续计划越稳定。
