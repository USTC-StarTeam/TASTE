...
def main() -> None:
    ...
    payload = {
        ...
        'verification_commands': [
            f"{management_python()} modules/environment/scripts/build_repo_data_requirements.py --project {args.project} --repo-path {active.get('repo_path', '<active_repo>')}",
            f"{management_python()} modules/environment/scripts/probe_repo_dataset.py --project {args.project} --repo-path {active.get('repo_path', '<active_repo>')} --env-name <active_env>",
            f"{management_python()} modules/writing/scripts/audit_paper_evidence.py --project {args.project}" + (f" --venue \"{args.venue}\"" if args.venue else ''),
            f"{management_python()} framework/scripts/report_status.py --project {args.project}" + (f" --venue \"{args.venue}\"" if args.venue else ''),
        ],
