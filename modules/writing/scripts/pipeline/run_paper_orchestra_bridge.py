...
from writing_paths import MODULE_ROOT, SCRIPTS_ROOT, add_script_paths, local_script_resolver, module_action_cmd, script_path  # noqa: E402
...
WRITING_MODULE_ROOT = MODULE_ROOT
WRITING_CONTRACT = WRITING_MODULE_ROOT / "SKILL.md"
WRITING_SKILLS_ROOT = WRITING_MODULE_ROOT / "skills"
DEFAULT_PAPER_ORCHESTRA_DIR = WRITING_MODULE_ROOT
WRITING_ACADEMIC_SKILLS_DIR = WRITING_SKILLS_ROOT / "taste-paper-writing"
WRITING_NATURE_REFERENCE_DIR = WRITING_SKILLS_ROOT / "nature-family-writing"
WRITING_VENDOR_PROVENANCE = WRITING_SKILLS_ROOT / "INTERNAL_SKILL_PROVENANCE.json"
...
def ensure_writing_vendor(venue: str, repo_dir: Path, *, skip_clone: bool = False) -> dict[str, Any]:
    script = script_path("check_internal_assets.py")
    payload: dict[str, Any] = {"status": "missing_script", "required_ready": False, "warnings": []}
    if not script.exists():
        payload["warnings"].append("writing internal asset check is unavailable; writing skills cannot be checked.")
        return payload
    ...
    try:
        ...
        payload["warnings"].append(f"writing internal asset check returned unparsable JSON: {exc}")
    if parsed:
        payload.update(parsed)
    if result.get("return_code") != 0 and not payload.get("required_ready"):
        payload.setdefault("warnings", [])
        payload["warnings"].append(
            "writing internal writing assets are missing. Run `python modules/writing/main.py --action check_assets --venue "
            + str(venue or "<venue>")
...
def clone_or_update(repo_dir: Path, *, skip_clone: bool = False) -> tuple[dict[str, Any], list[str]]:
    ...
    if repo_dir.exists() and (repo_dir / ".git").exists():
        ...
        warnings.append("writing uses internalized PaperOrchestra skills; external clone is disabled.")
    commit = ""
...
def link_skills(repo_dir: Path) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for name in PAPER_ORCHESTRA_SKILLS:
        ...
        rows.append({"name": name, "source": str(source), "status": "local-ready" if source.exists() else "missing_source"})
        if not source.exists():
            warnings.append(f"missing internal writing skill: {source}")
...
def ensure_venue_contract(project: str, venue: str, *, refresh_current_venue: bool = False) -> dict[str, Any]:
    ...
    if refresh_current_venue or not requirements:
        cmd = [sys.executable, str(SCRIPTS_ROOT / "resolve_venue_requirements.py"), "--project", project, "--venue", venue]
    ...
    if not template_ok:
        result = run([sys.executable, str(SCRIPTS_ROOT / "fetch_latex_template.py"), "--project", project, "--venue", venue], cwd=ROOT, required=False)
        commands.append(result)
        if result.get("return_code") != 0:
            raise RuntimeError(
...
Workspace: {workspace}
Read-only internal writing skill root: {repo_dir / "skills"}
Read-only TASTE writing contract skill: {WRITING_ACADEMIC_SKILLS_DIR}
...
def run_deterministic_gates(project: str, venue: str, workspace: Path, repo_dir: Path, *, min_references: int) -> dict[str, Any]:
    ...
    payload["venue_template_validation"] = venue_validation
    figure_audit_cmd = [sys.executable, str(SCRIPTS_ROOT / "audit_paper_figures.py"), "--project", project, "--venue", venue]
...
def sync_preview(project: str, venue: str, title: str, workspace: Path, *, force_refresh: bool = False) -> dict[str, Any]:
    ...
    if force_refresh and not workspace_artifacts.get("ready"):
        ...
        return payload
    cmd = [sys.executable, str(SCRIPTS_ROOT / "build_conference_preview_paper.py"), "--project", project, "--venue", venue, "--title", title]
    result = run(cmd, cwd=ROOT, required=False)
...
def main() -> int:
    ...
    payload: dict[str, Any] = {
        ...
        "source": {
            "repository": "internalized-writing-dev-skills",
            "principle": "writing prepares inputs and Claude Code executes the internalized writing skills; this bridge does not fabricate paper content itself.",
        },
        "warnings": [],
    }
    status = "blocked"
    try:
        ...
        if not vendor_status.get("required_ready"):
            raise RuntimeError("writing internal writing skills are not ready; run python modules/writing/main.py --action check_assets.")
