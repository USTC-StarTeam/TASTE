...
from taste_pythonpath import ensure_taste_pythonpath, taste_pythonpath_string
from writing_paths import MODULE_ROOT, SCRIPTS_ROOT, add_script_paths, local_script_resolver, module_action_cmd, script_path

ensure_taste_pythonpath(ROOT)
add_script_paths()
os.environ['PYTHONPATH'] = taste_pythonpath_string(ROOT, os.environ.get('PYTHONPATH', ''))
SCRIPTS = local_script_resolver()
add_script_paths()

from paper_common import update_pipeline_state
from writing_paths import module_action_cmd, external_stage_cmd
from pipeline_guard import guard_fresh_base_blocker_entry


def module_cmd(stage: str, action: str, *extra: str) -> list[str]:
    if stage in {'writing', 'writing'}:
        return module_action_cmd(action, *extra)
    return external_stage_cmd(stage, action, *extra)
