...
def refresh_current_venue_contract(project: str, venue: str, timeout_sec: int) -> dict[str, Any]:
    ...
    requirements = run(
        [
            sys.executable,
            str(ROOT / "modules" / "writing" / "scripts" / "resolve_venue_requirements.py"),
            "--project",
            project,
            "--venue",
            venue,
    ...
    template = run(
        [
            sys.executable,
            str(ROOT / "modules" / "writing" / "scripts" / "fetch_latex_template.py"),
            "--project",
            project,
            "--venue",
            venue,
...
def main() -> int:
    ...
    for round_index in range(1, max(1, args.max_rounds) + 1):
        preview_before = run([
            sys.executable,
            str(ROOT / "modules" / "writing" / "scripts" / "build_conference_preview_paper.py"),
            "--project",
            args.project,
            "--venue",
            venue,
